...
| ADR-6 | **OpenSearch mapping generated from `@OpenSearchProperty` decorators** | Search schema co-located with the entity; no drift. **Amended 2026-08-19** (`docs/specs/innovation-use/data-model-and-catalog` T-14, finding in that spec's `design.md` §7/§11) — **proposed amendment to this ADR, not a supersession: the decorator mechanism stands; only the stated *location* is wrong.** For the results index, the mapping is generated from the class handed to the base API — `ResultOpensearchDto` (`result.opensearch.api.ts:25`, reflected by `base-open-search-api.ts:316-344`) — never from the entity, so entity-level `@OpenSearchProperty` decoration is inert for that index. The next spec must decorate the DTO the results index actually reflects off, not the entity, or its decoration ships dead. |
...
| ADR-11 | **Filed 2026-08-19** (`docs/specs/innovation-use/data-model-and-catalog` T-14, proposed in that spec's `design.md` §11) — **section completeness (and result lifecycle) is computed by MySQL stored routines.** Green-check completeness lives in MySQL stored functions (`innovation_dev_validation`, `innovation_use_validation`, …); result lifecycle — versioning, snapshot, delete — lives in stored procedures (`SP_versioning`, `SP_delete_result_version`, `full_delete_result_version`, `delete_result`) that enumerate every child table, and on the copy path every column, by name. Retaining the mechanism was chosen over reimplementing completeness in TypeScript (a cross-indicator refactor out of scope for one spec) or making the procedures column-agnostic (changes lifecycle behavior for every indicator at once). | **The checklist's first instruction is how to build the checklist — enumerate the routine set by call site, never by suspected name:** grep the codebase for the literal call sites (`CALL <routine>` and `SELECT <function>(` occurrences under `src`, excluding specs and migrations) or query `information_schema.ROUTINES`, then transcribe each body by reading it, and name each routine by what its body does. Three consecutive review rounds on the filing spec got this set wrong (2 → 3 → 4) by guessing names, and each wrong count was written into an earlier draft of this very checklist — a wrong checklist is worse than none. **Standing rule (current set, call-site-verified 2026-08-19): every spec adding a table or column under `results` must amend all FOUR lifecycle routines above.** **Two blind spots this gate does NOT close:** (i) a schema change hidden under a column name a routine already enumerates — a structural DROP/CREATE body diff cannot see it, only a behavioral fixture can; (ii) redefinition of a shared helper a routine calls (e.g. `valid_text()`) — invisible to that routine's own body diff. **Column-coverage method for copy-path routines:** `SELECT *` against the source and destination rows, then delete the identity/PK column(s) from both sides before comparing — never a hand-enumerated column list, which re-creates the exact enumerate-by-name failure the routines themselves embody (reference implementation: `server/researchindicators/test/fixtures/innovation-use/innovation-dev-lifecycle-routines-unchanged.fixture-spec.ts`'s `fetchFullRow` helper). **SQL logic sits outside the Jest coverage figure** — a green 60% must never be read as validation coverage for the classes of defect this ADR exists to name. **Consequences:** every spec adding a section-completeness rule or a `results` child table/column inherits an untestable-by-default gate and must supply its own execution harness against a disposable MySQL — no unit test in this repository executes a stored routine, so a wrong boolean or a silently dropped column produces no error, log, or metric on its own. **Revisit triggers:** a completeness or data-loss bug reaching production; a third spec paying the harness cost; any move to a database without stored-routine parity. **Related:** ADR-5 (routine bodies are `DROP`/`CREATE`, never edited in place — still append-only at the migration level, not superseded); **ADR-6 amendment** (above, same filing). |
| ADR-13 | **Filed 2026-09-09** (`docs/specs/innovation-use/link-innovation-dev`, T-13; decisions in that spec's `design.md` §3.2/§3.3, DD-2/DD-5) — **a result-to-result link is a soft-deleted row under a role, and its cardinality is a property of the writer, not of a constraint.** An Innovation Use result names its Innovation Dev output through one `link_results` row: `link_result_role_id = 5` (`INNOVATION_USE_LINKED_DEV`, seeded by an append-only migration), `other_result_id` = the target, `is_active` toggled — never `DELETE`, so the history survives. At-most-one-active is achieved by **reusing `LinkResultsService.create`** (DD-2), which deactivates every row in the role before inserting, and which **reactivates the same PK** when a previously-cleared target is re-selected rather than duplicating it. **Consequence, accepted and recorded rather than mitigated:** there is **no** unique index on `(result_id, link_result_role_id, is_active)` — a partial unique index is not expressible in MySQL and a full one would break the soft-delete history — so two concurrent PATCHes can each deactivate what they can see and each insert, committing two active rows. That is tolerable **because** the submission gate uses `EXISTS` (DD-5) rather than `COUNT(*) = 1`: a doubled row leaves the green check correct and the UI showing one link, where a cardinality assertion in SQL would turn an invariant violation into a red check the user cannot clear. **Section completeness follows ADR-11's routine pattern**: rule 16 is appended to the `innovation_use_validation` stored function, applies retroactively with no grandfathering (direct user ruling), and is verified against a disposable scratch schema before any shared environment (ADR-12's baseline makes that possible). |
| ADR-12 | **Added 2026-08-18** (`docs/specs/archive/2026-08-18-bugfix--sp-versioning-roles-id`, finding RB-1d/OQ-3) — **the 303-migration history is not replayable from an empty database; a committed schema-only snapshot, not migration replay, is the de-facto bootstrap for any disposable schema.** Two independent blockers surface in the first 139 migrations: `1751474908040-InsertTemplates.ts` writes into `sec_template`, which no migration creates (MySQL 1146), then `CreateStaffGroups1759786024597` hardcodes a `carnet` FK to `alliance_user_staff`, a table populated only by a runtime staff-sync process (MySQL 1452) — leaving 164 migrations unexercised. The `sec_*` tables predate this repo's adoption of TypeORM migrations and were created directly on the shared database, outside migration control. | **Consequences:** no environment can be stood up from source; CI can never gain a from-scratch schema gate. The disposable-schema path (`server/researchindicators/src/db/baseline/`, npm-scripted as `migration:test:bootstrap`) exists precisely because replay does not work — see `docs/infrastructure.md` *Data & migrations*. **Does not supersede ADR-5** — append-only migrations are upheld, not overturned; this is a newly discovered constraint ADR-5 implies, not a reversal of it. **No ticket id exists yet** for the underlying repo-level fix (`requirements.md` OQ-3 in the spec above): whether the history should be squashed to a baseline migration or the snapshot should become the permanent bootstrap is still open. |
...
| PERF-5 | The project-dashboard route (`project-detail/:id/project-dashboard`) loads | Its load `effect` fires **4** requests on one consistent basis (`reports/full`, `reports/geo-scope`, `results/count`, the results-by-status fetch) — down from **7** before the `reports/full` migration collapsed four separate `reports/top-*` calls into one. **Do not cite "6 → 4"** — that comparison excluded the status call and compares unlike quantities. Trajectory: **3** once the geographic-scope spec retires `reports/geo-scope`; **2** once a follow-up chunk retires the status fetch. Not counted on either side: `initializeProjectDashboardResultsTable` and the executive-overview load, fired by the same effect and untouched by this migration. **Server-side query note, added 2026-07-31 (R-IMC-012 AC.3):** the `indicator-metadata-charts` spec added 10 aggregation sections to `reports/full`'s own response; this is a change to the endpoint's SQL profile, not to the **4 client requests per load** counted above — those two are different quantities and the client count is unchanged by this spec. `reports/full` now issues **10 SQL queries in two sequential batches, peak concurrency 8** (consolidated per DD-1, composed sequentially per DD-11/DD-12 — `docs/specs/project-dashboard/indicator-metadata-charts/design.md` §1, §12), against a pool whose default `connectionLimit` is 10 (`mysql2` default, unconfigured). |
...
**Representative endpoints (via `ApiService`):** `POST /authorization/login`, `POST /authorization/refresh-token`; `GET /indicator-types`, `/indicators`, `/maturity-levels`; `GET /tools/clarisa/*` (institutions, sdgs, levers, sdg-targets, impact-areas, countries, languages, delivery-modality, session-types); `GET|POST /results`, `GET /results/search`, `GET|PATCH /results/:id`, `PATCH /results/:id/submit`, `DELETE /results/:id/:field`; `GET /results-center`, `/dashboards`, `/projects`, `/contracts`, `/metadata`; `PATCH /agresso/contracts/:code/pool-funding-tag`; `GET|PATCH /results/:resultCode/pool-funding-alignment` (PATCH → `409` once synced to PRMS); `GET /results/:resultCode/pool-funding-alignment/hlos-indicators` (level-based ToC catalog, sourced from lambda-toc through a 5-min backend cache); `GET /agresso/contracts/reports/full?contract-id=<encoded>` — **the project dashboard's single analytics contract**: one call returns the `ContractFullReports` payload — **17 fields (16 sections + `contract_id`)**: the original 6 sections (`top_partners`, `top_primary_levers`, `top_main_contact_persons`, `top_contributors`, `staff`, `geo_scope`) plus **10 Indicator-metadata sections** added 2026-07-31 (`innovation_nature`, `innovation_type`, `innovation_readiness`, `oicr_maturity`, `policy_type`, `policy_stage`, `session_format`, `session_type`, `gender_distribution`, `degree` — `docs/specs/project-dashboard/indicator-metadata-charts/requirements.md` R-IMC-012 AC.2), plus the scalar `contract_id` string identifier — verified against `ContractFullReportsDto` (`server/researchindicators/src/domain/entities/agresso-contract/dto/reports-full.dto.ts:34-139`, 7 base properties + 10 added) — mirrored client-side in `shared/interfaces/contract-full-reports.interface.ts` and consumed via `GetFullContractReportsService` (one `payload` signal plus per-section `computed` accessors), same `MainResponse<T>` envelope as everything else through `ApiService`.

`reports/full` replaced four separate per-chart client methods (`GET_TopPartners`, `GET_TopPrimaryLevers`, `GET_TopMainContactPersons`, `GET_TopContributorsContracts`) with the single call above; those four methods and their services were retired from the client (R-PDB-008). `GET_GeoScope` / `GetGeoScopeService` deliberately **survive** — the geographic card still consumes `reports/geo-scope` pending a follow-up spec. **The four `reports/top-*` server endpoints are untouched** — this was a client-side consumption migration only, not a server contract change. Full record: `docs/specs/project-dashboard/full-payload-show-more/`.
...
| **PRMS decision callback** | HTTP (`POST /api/prms-callback/:secret`) | PRMS → ARI | The platform's first inbound **write**. Authenticated by the path secret `ARI_PRMS_WEBHOOK_SECRET` and excluded from `JwtMiddleware` — not a JWT and not a machine token. Every delivery is appended to `result_prms_sync_history` (renamed from `prms_webhook_delivery` by the owner-approved Pivot of 2026-09-23; migration `1790086170692`). The callback module is `domain/entities/prms-webhook/`, not a folder under `domain/tools/`. Registering the destination is the opposite direction: ARI → PRMS through `PrmsNormalizerService` (`POST` / `GET /api/prms-webhook`, `SYSTEM_ADMIN` + JWT). Spec: `docs/specs/bilateral/prms-sync/decision-webhook/`. |
