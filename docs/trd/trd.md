...
> **ADR-11 is reserved, not yet filed.** `docs/specs/innovation-use/data-model-and-catalog/design.md` proposes ADR-11 for *green checks and lifecycle as stored routines*; this spec's own `design.md` §7 already refers to it by that number. Do not assign ADR-11 to anything else until that spec files it.
