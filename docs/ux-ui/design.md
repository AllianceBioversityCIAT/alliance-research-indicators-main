...
- **Ranked-card expand/collapse toggle** (`project-dashboard-card`, §8.1): the "Show more"/"Show less" control is a real `<button>`, reachable by Tab and operable by `Enter`/`Space`, exposes `aria-expanded` reflecting state, and its accessible name **includes the chart's title** so four otherwise-identical toggles are distinguishable (NFR-PDB-003, `docs/specs/project-dashboard/full-payload-show-more/requirements.md`). **Known gap, honestly scoped:** the container that scrolls the expanded list is not itself keyboard-operable yet — tracked as deferred task **T-09** in that spec's `tasks.md`. Do not infer full WCAG 2.1.1 (Keyboard) conformance on this screen until T-09 lands.
