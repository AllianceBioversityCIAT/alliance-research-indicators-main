...
- **Result indicator-6 (Innovation Use) section, `pages/platform/pages/result/pages/innovation-use-details/`:**
  ...
  - **Organization card** — `components/innovation-use-organization-item/innovation-use-organization-item.component.ts` — one organization row: known/unknown identity paths (institution lookup vs. type + cascading sub-type), OTHER name, and a count. **Required fields depend on the active path** (`docs/specs/changes/innovation-use-required-fields`, `R-IUR-006`…`R-IUR-009`): known path — `Organization`; unknown path — `Organization type` always, `Sub-type` only when the selected type has sub-types, `Organization count` required and greater than zero. Every other field on this card stays optional.
...
- **2026-09-09 — Innovation Use names its Innovation Dev output through a required picker in its own card (`docs/specs/innovation-use/link-innovation-dev`).** Decision record, not a claim of visual conformance — the visual check is that spec's T-12 and is **still open**.
  - **The field is its own titled card, `RELATED INNOVATION DEVELOPMENT`, between *INNOVATION USE DETAILS* and *ACTORS*** (Amendment 01, from a user-supplied mock; R-IUL-013). *Rationale:* the mock puts it there; it is not a field appended to the details card.
  - **The required asterisk sits on the section title, not on a field label** (DD-11). *Rationale:* the mock gives the control no label to carry one. The amber border and *"This field is required"* still come from the shared `app-select`'s own `isInvalid()`. **Required is visual and at submit only** — it never blocks a draft save, per `docs/specs/bugfix/innovation-use-draft-save`.
  - **The selected result renders from the GET payload, never from the options list** (DD-6 / C12). *Rationale:* resolving the card from the options list would make a **soft-deleted** target silently vanish while the green check went red — the user would face a red check over a blank field. Reading the payload shows the now-invalid link instead, and rule 16 independently returns `FALSE`, so the UI and the check agree.
  - **One labelled `View innovation detail ↗` anchor, opening a new tab; no remove control** (DD-7 **withdrawn**). *Rationale:* the field is required, so changing the selection is the only supported edit; a labelled action also gives a better accessible name than a card-wide link. The anchor carries `target="_blank"`, `rel="noopener"` and a visually-hidden *"(opens in a new tab)"*.
  - **The anchor's URL carries the platform prefix for every platform — `STAR-284`, `PRMS-284`, `TIP-284`, `AICCRA-284`** (Amendment 04 / DD-13, user ruling). *Rationale:* the options list is deliberately unfiltered by platform, and a bare `/result/284` resolves to **STAR** via the numeric-implies-STAR invariant — so a PRMS target rendered `PRMS 284` beside a link that opened a *different* result. The bare code remains the fallback only when `platform_code` is `NULL`. **Display and URL are different strings:** the label is space-joined, the URL hyphen-joined.
  - **A failed options request surfaces inside that card and reaches nothing outside it** (Amendment 03 / DD-12). *Rationale:* routing it into the page-level load-failure gate — which the requirement's original wording invited — unmounted the whole section, made *Save* a silent no-op and let *Next* discard unsaved edits. The card-scoped banner reuses the section's existing error affordance, the control stays mounted so re-entering the section retries, and the empty-list tooltip is suppressed because a failed load is not an empty catalog.
  - **The picker carries the placeholder `Select the innovation development`** (Amendment 07, user-requested at the visual check). *Rationale:* without one an unselected control reads as blank. Wording follows the established house convention — `Select the <thing>`, as in *"Select the actor type"* / *"Select the organization type"* / *"Select the year"* — rather than inventing a phrasing for this one field.
  - **The anchor carries its own `--ac-white-1` fill rather than sitting on the row's grey.** *Rationale:* measured — on the grey surface its resting state is **1.46:1** in dark theme (`#23466b` on `#2b2b2b`), i.e. invisible. With its own fill it is **6.83:1** light and **7.72:1** dark, and it is also closer to the mock. **No visual-consistency-versus-AA tradeoff arose here.**
...
- **2026-08-21 — Innovation Use (indicator 6) details page ships.** New lazy route `innovation-use-details`; two sidebar rows (`Innovation use details`, `IP rights`) render for indicator 6. Spec: `docs/specs/innovation-use/details-page`. Three of its decisions are user-visible and recorded here (component patterns are in §8.1):
  ...
  - **Empty-state affordance: exactly one blank Actor card; Organizations and Other quantitative measures are deliberately left empty** (spec `DD-10` — **SUPERSEDED 2026-09-04**, see the 2026-09-04 entry below). *Rationale:* actors are required, so a blank starter card helps completion; organizations and quantifications are optional, and a blank organization card is exactly the identity-less row whose `400` a prior chunk of this feature added server-side validation to stop.
