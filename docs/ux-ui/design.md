...
- **2026-08-13 — The Results Center URL query vocabulary is a durable, cross-package contract.** Spec: `docs/specs/results-center/url-filters`. `/results-center` filter state is expressed in **exactly six human-readable query parameters** — `indicator`, `contract`, `status`, `year`, `source`, `tab` — read on load and written back on every filter change (`replaceUrl`, so history depth is unchanged). Binding rules, all contractual rather than incidental:
