...
- **Chart explainers (`chart-explainer`, D-CXP-1…10, `docs/specs/changes/chart-explainers/`):** shared "?" trigger + non-modal `p-popover` (not `all-modals` — reading help must not trap focus or dim the page) paired with an always-rendered `sr-only` description, so the explanation reaches assistive tech via a host's `aria-describedby` whether or not the popover is ever opened. Wired at **section level only** — one instance beside each Act's `<h2>` in `project-dashboard.component.html` (6 total: `act-1-identity` … `act-6-depth`), not per individual chart; the originally-approved per-chart design (33 surfaces, 38 keys) was superseded mid-spec (D-CXP-10) before shipping. `placement` input supports `inline` (used — sits in the header's normal flow) and `surface` (absolute top-right disc over a chart; declared, currently unused by any host). Tokens only: glyph `--ac-grey-700` at rest / `--ac-light-blue-400` on hover-focus, popover title `--ac-primary-blue-600`, body `--ac-grey-700`, `surface` disc `--ac-background` (one token, both themes); hit area 32 px (`h-8 w-8`), glyph 16 px.
...
- **Chart explanations:** each `chart-explainer` button carries an accessible name (`"Explain this chart: <title>"`) plus `aria-expanded`/`aria-controls` while open, closes on `Escape` from anywhere via a document-level listener with focus returned to the button, and never auto-focuses into the popover on open; a persistent `sr-only` description is exposed via the host section's `aria-describedby`, so the explanation is announced even to a reader who never opens the popover.
...
- **2026-08-25 — Chart-explainer pattern shipped: one shared "?" popover component, wired at Act-section level (6 instances) instead of per-chart.** Spec: `docs/specs/changes/chart-explainers/`. One `chart-explainer` component (button + non-modal `p-popover` + persistent `sr-only` description) is reused everywhere a chart-bearing section needs a plain-language explanation (D-CXP-1, D-CXP-3 — non-modal because reading help must not trap focus or dim the page, unlike the `all-modals` dialog pattern); 32 px hit area / 16 px glyph was chosen over `ui-ux-pro-max`'s 44 px guidance because 44 px would dominate a 44-px-tall composition strip or a 15 px title row, and 32 px already clears the WCAG 2.2 AA 24 px minimum with margin (D-CXP-4); registry entries carry a `derivedFrom` audit-trail field alongside the copy itself, not just the copy (D-CXP-9). **Mid-spec pivot (D-CXP-10, 2026-08-25):** the originally-approved design placed one explainer per chart (33 surfaces, 38 keys, including new `viz-chart` inputs); the owner judged that density unacceptable once the 20-chart indicator deep-dive grid alone was flagged as a risk, and redirected to one explainer per Act section (6 keys, `project-dashboard.component.html` only) — `viz-chart` and `project-dashboard-card` stayed untouched. Full incident record: that spec's `execution.md` → "Pivot Record: T-02". *Advisory carried to the HITL pass, not yet observed:* PrimeNG's `p-popover` root renders `role="dialog"` with `[attr.aria-modal]="overlayVisible"` (`node_modules/primeng/fesm2022/primeng-popover.mjs:535-536`), and this component's own markup then nests a `role="region"` panel inside that dialog wrapper — so a screen reader may announce the outer `dialog`/modal role before ever reaching the inner `region`, in tension with the non-modal intent above. VoiceOver's actual announcement on this nesting is unverified pending the T-04 HITL keyboard/SR pass.
