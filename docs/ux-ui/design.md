...
- **2026-08-12 — `aligns_with_toc: true` now requires only `level` + `toc_result_id`; `indicator_id` and `quantitative_contribution` become optional (D-C1-3, the Level + HLO floor).** *Rationale:* user decision 2026-08-12, matching AC-1676's shallowest listed stop ("HLO/Outcome only"). Level is implied — it filters the catalog the HLO is picked from. A bare "Yes" without Level + HLO is still invalid; "No" still expresses "not mapping".
