...
- **2026-08-12 — `aligns_with_toc` keeps its column, wire, and draft-type name; the question it backs is reworded to ask whether the contributor wants to complete the ToC mapping (D-C1-2, `docs/specs/bilateral/toc-optional-mapping`).** *Rationale:* stored values stay compatible (`true` = mapped); renaming mid-flight breaks the FE for no user benefit. The meaning shifts from "does this contribute" to "opted into detailed mapping" — documented, not renamed (R-1 in that spec's requirements §9).
