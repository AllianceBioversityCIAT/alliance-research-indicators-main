...
| `/akili-execute` — Leader | **T1** | Writes no production code, but selects each worker's skills, adjudicates FAILs, and decides pivots. |
| `/akili-execute` — Implementer | T2 | Writes the code. |
| `/akili-execute` — Reviewer | T3 | **Must differ from the Implementer model.** |
| `/akili-test` — Leader | **T1** | Suite fan-out orchestration + per-suite skill/effort selection. |
