...
| both paths | `organization_count` — `app-input type="number"`, `[min]="0"`, `[maxFractionDigits]="0"`, **optional** ⚠️ **AMENDED 2026-09-03 by `docs/specs/archive/2026-09-03-changes--innovation-use-organization-count-known-path`:** `organization_count` no longer renders on both paths — it renders on the **unknown path only**. The payload nulls it on the known path (does not clear the rendered `body`). Reason: on the known path the row names one specific CLARISA institution, so "how many organizations?" cannot be meaningfully answered — the field asked a question with only one possible answer. The two Lens B "Fix 3" guards this document's `execution.md:731` added (the known-path presence assertion and its anti-vacuity length guard) were **reworked, not deleted**, to assert absence instead. See that spec's `design.md` `DD-1`–`DD-6` for the full design. |
...
| **DD-4** | 2026-08-20 | `app-input` gains an optional `maxFractionDigits`, forwarded to `p-inputNumber`; the new page passes `0` | **Verified** against `primeng@19.0.6`: with both fraction options `undefined`, `Intl` resolves `maximumFractionDigits: 3`, and `insertText` formats a pasted `2.5` through that resolution — so a fraction reaches the body. `[min]="0"` already blocks negatives on both paths. ⚠️ **AMENDED 2026-09-03 by `docs/specs/archive/2026-09-03-changes--innovation-use-organization-count-known-path` (`R-IUC-001`):** "both paths" here describes **`app-input`'s own generic behaviour** — the guard works wherever the control is rendered — and that remains true. What is no longer true is the implication for **`organization_count` specifically**: that field now renders on the **unknown identity path only** (see the amended §5.5 row), so its `[min]="0"` guard exists on **one** path, not two. The five actor counts are unaffected. *(Caught by `/akili-validate`'s independent audit as finding **F-4**: this spec's own KZ-013 sweep concluded "no referrer asserts a falsehood" after examining only the archived `execution.md`, and missed this line — a **live, already-amended design decision**, not frozen history. Recorded because the miss is the lesson, not the line.)* Optional-with-`undefined`-default keeps all 16 consumers byte-identical (§6.3) ⚠️ **AMENDED 2026-08-27 by `docs/specs/changes/measure-number-signed-decimal` (`S-10`, `DC-12`): "the new page passes `0`" now holds only for the **six** surviving count fields — the five actor counts (§5.4) and `organization_count` (§5.5). The measure Number (`quantification_number`, §5.6) passes `[maxFractionDigits]="4"` with a derived signed `min`/`max`, because it accepts signed decimals (scale ≤ 4). The `undefined` default and every other consumer are unchanged — this amendment narrows only the Innovation Use call site's argument.** |
...
| **2026-09-03** | **External amendment by `docs/specs/archive/2026-09-03-changes--innovation-use-organization-count-known-path` — `organization_count` carved out of the KNOWN identity path.** §5.5's `both paths` field-table row and `tasks.md:309` both asserted, unqualified, that `organization_count` renders on **both** identity paths of the organization card. That is no longer true: the field renders on the **unknown path only**, and `buildOrganizationPayload` nulls it on the known path (at the payload boundary — the card's `body` is deliberately left untouched, so `onKnownToggle`'s no-cross-clear invariant and the known-path restore test both survive). Reason: a row that names one specific CLARISA institution cannot meaningfully answer "how many organizations?". Each affected passage is qualified in place with a dated note; the original sentences are preserved. **The two Lens B "Fix 3" guards this spec's `execution.md:731` added were reworked, not deleted** — the known-path presence assertion is inverted and the `c8` anti-vacuity length guard now pins `0`; both were observed RED against the un-fixed template before being trusted (K-004). `R-IUP-008`'s numeric-hygiene guarantee is **unchanged** and still governs the field on the surface where it survives. **The server tier is untouched** — the DTO still accepts the field on both paths, the column stays `int NULL`, `SP_versioning` is unedited, and `resolveOrganizationCount` already confined the column to the `INNOVATION_USE` role, so Innovation Dev is unreachable from this change. Governing requirements: `R-IUC-001`, `R-IUC-002`. ⚠️ *This document is a point-in-time record and is not otherwise re-verified; only the superseded present-tense claims are qualified. The archived `execution.md`'s three both-paths statements are deliberately left intact as a frozen record of what was verified on 2026-08-26.* |
