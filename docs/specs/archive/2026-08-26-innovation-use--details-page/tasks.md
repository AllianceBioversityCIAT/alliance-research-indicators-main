...
- `organization_count` renders on **both** paths, `[min]="0"`, `[maxFractionDigits]="0"`, **optional**. ⚠️ **AMENDED 2026-09-03 by `docs/specs/archive/2026-09-03-changes--innovation-use-organization-count-known-path`:** the field now renders on the **unknown path only** — the known path names one specific CLARISA institution, so a count question is unanswerable there and the payload nulls it (the rendered `body` is left untouched). The two Lens B "Fix 3" guards `execution.md:731` added were **reworked, not deleted**, to assert absence on the known path instead of presence. See that spec's `design.md` `DD-1`–`DD-6`.
...
- **Status:** done · **11 of 11 discharged 2026-08-26.** Reopened at `/akili-validate` and re-closed after the R1/R2/R3 remediation: `c7` re-discharged against a rendered observation of the remediated surface (incl. the first full sighting of card 1), `c9` against the now-focusable delete control plus R2's automated focusable-and-named positive control. See `execution.md` → *`T-13` CLOSED* · **Size:** M · **Dependencies:** T-09, T-10, T-12, **T-14**
...
- [x] c8 — **T6-Multimodal screenshot review** of the section, **light theme only, at both viewports — two screenshots, not four** (amended 2026-08-21 with c7, same user ruling and same reachability evidence). Per the model registry's *Cross-host dispatch*, the strongest column for T6 may not be this session's host; if no T6-capable reviewer is reachable, record the criterion as **blocked**, not passed.
- [x] c9 — **Keyboard pass:** Tab through the whole page — every control receives focus in document order with a visible ring, **no focus trap inside a repeatable card**, and every icon-only control announces an English name. Human-observed; quote what was observed.
- [x] c10 — **Budget reconciliation against `design.md` §12.** ✅ **RE-RUN and CLOSED 2026-08-26** against the amended baseline (**14 tasks · ~3,400 written / ~4,800 re-baseline · ~31 rounds**), including `T-14`, the three Pivots and the `R1/R2/R3` remediation, after validation `F-3` (the 08-21 table was arithmetically inconsistent) and `F-4` (it predated `T-14`, and no document carried a post-amendment total). **LOC tripwire breached; rounds under budget.** **The figures live in `execution.md` → `T-13` c10, their single home (`KZ-005`) — this criterion points there and deliberately does not carry them.** Recorded and **not absorbed**; not re-escalated, the user having ruled on this same overrun three times with full information. Figures, the deriving command and the per-commit split live in **`execution.md` → `T-13` c10, their single home (`KZ-005`) — re-derive there, do not restate**.
...
| c12 | A ratio computed against the token's **dark** value is the wrong number — DD-14 scopes this to the light theme. **And a ratio computed from the class you *intended* is not evidence of the colour that *renders*:** §5.8 documents two live cascade traps, **in two different files** — `custom-fields.scss:99–101` hardcodes `.description`'s `#777c83` (**4.20:1** on white, fails AA — *ratio corrected 2026-08-26: the `3.91:1` first written here was the `loadFailed` banner's local `--ac-grey-700`, not this rule*) and `styles.scss:193–199`'s nested `.description a` repaints links `#2e2e2e` (passes contrast at 12.62:1, destroys link discoverability). *(File attribution corrected 2026-08-26 at T-14 finalization, together with §5.8's — this line said both traps were in `custom-fields.scss`. Second site of the same error: **KZ-005** is fixing the cited site and missing the rest, so the sweep ran in both directions across the spec folder)*. **Report the computed colour of the rendered element, not the token you meant to apply** |
...
| RB-5 | 2026-08-20 · **reframed 2026-08-26** | **`T-03 c6`** — the promoted `quantification-item` template carries four hex literals into `shared/`; DD-7's "new files" wording does not reach it. ⚠️ **This is not only tokenization debt.** One of those literals is a **live light-theme WCAG AA failure**: `quantification-item.component.html:3`'s eyebrow is `text-[#8D9299]` on `bg-[#F4F7F9]` = **2.911:1** against AA's 4.5:1 — the same role and the same number as the actor and organization eyebrows that validation `F-1` raised and `R1` fixed. Recording it as `DD-7` debt alone meant **nobody was tracking it against `R-IUP-017` AC.3**, which is the reframing `R5` owes (validation `F-2`). Rendered on every Innovation Use page with ≥ 1 measure **and on every OICR details page** | Bounded and named rather than remediated inside a move task. Owed follow-up: detokenize on its own schedule with **OICR in the blast radius**. The AA half is a **one-word swap** (`#8D9299` → `var(--ac-grey-800)`, 2.911:1 → 7.44:1) that also reduces the recorded hex count — but it changes OICR's rendering, so it is the user's decision, not a sweep. Deliberately not taken during the `R1/R2/R3` remediation under the standing *stability-before-deploy* instruction. Carried alongside `OQ-IUP-8`. *(Closes `judgment.md` → `I-4`)* | Engineering lead | **open — follow-up · AA defect + hex debt** |
