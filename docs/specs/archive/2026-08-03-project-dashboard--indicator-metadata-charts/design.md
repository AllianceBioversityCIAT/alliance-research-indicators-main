...
| 4-card band | `minmax(400px, 1fr)` — **not fixed at 2×2** (corrected 2026-07-31; DD-7 in §12 has the full, measured breakdown by width and sidebar state) |
...
| Gender util specs | Pure fixtures — individual + group merge keyed on `gender_id`; NULL→0; zero-total dropped; no double-count; an id present on only one side carried through unchanged; **a group-only fixture (zero individual rows) still yields the summed counts for every category with a non-zero total, zero-total categories dropped per AC.3** (wording corrected 2026-07-31 — see `execution.md` § *Owner escalation*); and the merged result is **sorted `count DESC, id ASC`** with a fixture where summing reorders the SQL ranking. |
...
| **DD-7** *(corrected 2026-07-31)* | 2026-07-30; corrected 2026-07-31 | `auto-fill`, `align-items: start`. **The 4-card band's layout is not fixed at 2×2** — `repeat(auto-fill, minmax(400px,1fr))` (`indicator-metadata-band.component.scss:100-129`) reflows by available container width. **Measured in real Chrome** (`evidence/t16-raw/measurements.json`): at 1440 px, in the app's **default** sidebar-collapsed state (`cache.service.ts:70`, `signal(localStorage.getItem('isSidebarCollapsed') !== 'false')` → `true` fresh), it is **3 columns + 1 wrapped card** (`wideGridComputedColumns: "422.391px 422.391px 422.406px"`, 4th card at `y:374.75`, container `1299.19px`); genuine **2×2** (`"553.25px 553.25px"`, container `1122.5px`) is reachable only if the sidebar is manually **expanded**; at 768 px in the same default state it is **one column, four stacked cards** (`"660.797px"`). The `@media (width < 720px)` collapse is demonstrated as the acting *mechanism* only on the **single-card** band (`minmax(300px,1fr)`) — the isolation pair on the 4-card band (~676 px container) is masked by width scarcity and proves nothing about the media query for that band. | Original claim (measured 500/768/1440, KZ-006) recorded no sidebar-state qualifier and was stated unscoped at old §7.4 / this row. T-16's real-Chrome re-measurement found it **wrong** at both 1440 px and 768 px in the app's default state (`evidence/t16-report.md` §Q1, §Q2). **No CSS changed** — documentation-only correction, owner-authorised 2026-07-31; the owner declined the layout fix and chose to correct the record instead. |
