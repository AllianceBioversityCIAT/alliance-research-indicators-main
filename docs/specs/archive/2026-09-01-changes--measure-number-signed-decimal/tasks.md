...
- **Acceptance / done check:**
  ...
  - [x] A scale outside `0…4` is rejected as a configuration error, not silently clamped (AC.1). ✅ **DISCHARGED by `T-10` and confirmed 2026-09-01.** The guard is in the **real** `@Input` setter (`quantification-item.component.ts:58-64`), and `quantification-item.component.spec.ts:181-203` exercises it through `fixture.componentRef.setInput` — Angular's real input path, not a test helper — for `5`, `-1`, `null` and `2.5`, plus the whole `0…4` domain not throwing. `null` is covered explicitly because `null <= 4` is `true` in JS. Original note follows. ⚠️ **NOT DISCHARGED — deliberately.** T-09 asserted a **test-side helper** throws; `input.component.ts` contains no scale rejection and adding one would be drift (`design.md` §6.1 assigns the guard to `QuantificationItemComponent`). **→ `T-10` must re-prove this against the card's real guard, and must NOT cite T-09's green.**
...
- **Acceptance / done check:**
  ...
  - [x] **HITL visual gate** ✅ **DISCHARGED 2026-09-01** — the user confirmed on Dev: values persist, pre-existing data survived the migration, decimals and negatives store, and **AC.7's untouched-row resave succeeds**. Placeholder and the amber warning both confirmed present. Spinner discharged by inference (negatives store → no clamp at `0`; `step = 1` is PrimeNG's default, unit-asserted). **Dark theme NOT verified and carried forward as `THEME-1`** — not a defect of this spec: the card hardcodes four hex literals from `d2f6a15e` (the *previous* spec), violating root `CLAUDE.md:145`, and app-wide light/dark is being implemented separately. **HITL visual gate** (`DC-11`, `NFR-MSD-004`): the field in both themes and in error state. **No automated substitute exists.**
...
- **Acceptance / done check:**
  ...
  - [ ] ⚠️ **ACCEPTED AS FOLLOW-UP 2026-09-01 (still owed, not waived).** Risk measured and much narrower than feared: the OICR **UI cannot** send negatives or decimals (it passes neither `min` nor `maxFractionDigits`, so it takes the scale-0 defaults — `DD-4`/`DD-13`'s additive-defaults design, confirmed by the user in the running app), and **no internal integration touches quantifications** (`grep src/domain/tools/` → zero). Residual exposure is direct API callers only, which cannot be enumerated from this repo. Draft in `execution.md`. **Nothing has been sent.** Comms record in `execution.md` naming the MEL/product owner, the OICR reporting owner, and any partner-platform contact (`NFR-MSD-005`, `:600`, `RK-12`). ⚠️ **Anchor corrected by `T-12` rework attempt 2; was `:496`, a blank line — see the correction at this task's `Requirements covered` line for why `:600` (the requirement's own heading) is the intended target.** **DELIBERATELY LEFT UNTICKED — see `execution.md` → `### T-12` → Comms draft.** No individual can be notified by an agent; a draft is prepared and the roles are identified from the spec's own sign-off list, but nobody has been notified. This checkbox is the Leader's to flip, after a human sends it.
