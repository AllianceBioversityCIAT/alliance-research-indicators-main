...
> **Archive-amendment policy (added by `T-12` rework attempt 2).** `R-MSD-007` AC.4 and `DC-12` require sweeping the archived `innovation-use--details-page` spec for the superseded claim; the repo's own convention (root `CLAUDE.md` §6) says archived specs are point-in-time records left unchanged. This spec resolves that tension as follows, stated explicitly rather than left implicit (which is how the archived `design.md`'s live claims escaped attempt 1's sweep): a **substantive behavior or design claim this spec reverses** — a rule the reader would otherwise act on today, wrong — gets a dated amendment note in place, in the shape of the existing `R-IUP-008`/`DD-17` amendments. A **coverage or judgment record** (execution logs, round-by-round judgment notes, past-tense "this was verified/measured/decided" text) does not, because it is already accurate for the moment it describes and asserts nothing about today.
...
| `docs/specs/archive/2026-08-26-innovation-use--details-page/design.md` | qualify five live claims that `quantification_number` is guarded against negatives/fractions (§0 findings row 3, §4.3, §6.3 ×2, §10.3), add a §16 revision-log row (`S-10`, `T-12` rework attempt 2 — the backward sweep missed this file first-pass) | modified |
...
> ⚠️ **Anchors regenerated by `T-12` (2026-08-27), LAST, after every other edit in this round, and verified by opening every line in `requirements.md`** — mirroring `tasks.md` §4's own regeneration, which found the same drift independently. `:181`–`:266` (through both `R-MSD-003` scenarios) were unchanged; everything from `R-MSD-004` onward had drifted, `:362` onward by **+22 lines** (the `R-MSD-006` AC.3 amendment). The `Where satisfied` column is otherwise untouched except where noted inline below.
...
| `:299` (was `:297`) must NOT be `3`, `2`, or `2.5000` re-read differently | DD-1 (scale 4) + DD-2 |
| `:300` (was `:298`) AND IT MUST be proven at the fixture tier | DD-9 |
| `:329` (was `:327`) must NOT be evidenced by a routine body diff | DD-9 — "bodies unchanged" is a separate assertion |
| `:330` (was `:328`) AND IT MUST read both rows out of MySQL | DD-9 + **DD-20**'s four-column match |
| `:384` (was `:362`) must NOT be fixed by re-uniting the guard's threshold | **DD-7 withdrawn** — the guard is not touched at all, so neither re-uniting nor removing occurs. `R-MSD-006` is amended accordingly. **Round 4:** the clause's own tail still read *"the guard is **removed**"*; corrected in `requirements.md`, since withdrawal means neither |
| `:385` (was `:363`) AND IT MUST instead be prevented at the control | ⚠️ **Partly false and amended — and Round 4 found the amendment never landed on the clause itself.** `L-07` established that PrimeNG checks `max` only on blur/Tab/Enter/spinner and then **silently clamps**; the per-keystroke path never checks it. So `max` is a *clamp*, not a prevention. The amendment was written as a new AC.6 while this clause kept saying *"not enterable"* — the two contradicted each other for a whole revision. **The clause is now rewritten, not merely annotated.** Asymmetry worth keeping in view: **`min` genuinely IS a prevention** — `allowMinusSign()` is `this.min == null \|\| this.min < 0` (`primeng-inputnumber.mjs:1270`, refused at `:1316`), so with `[min]="0"` the minus key is rejected per keystroke |
| `:452` (was `:430`) must NOT render `-0.7500`, `NaN`, `0`, or empty | DD-2 + DD-3 |
| `:453` (was `:431`) AND IT MUST behave identically for `string` or `number` | DD-3 — the coercion is idempotent |
| `:483` (was `:461`) must NOT be asserted from reasoning about `DECIMAL` formatting | **DD-11** — automated fixture |
| `:484` (was `:462`) AND IT MUST be observed from an executed query | DD-11. ✅ **VERIFIED by execution — `T-08`, §17 `U-1`** (was "still unexecuted" when this row was last written) |
| `:485` (was `:463`) AND IT MUST include the `-10.0000` case | DD-10's seven cases |
| **`:517`** (was `:495`) must NOT be silently rounded to `3` and stored | **DD-13** — service-layer rejection before the upsert |
| **`:518`** (was `:496`) AND IT MUST be announced to consumers before it ships | **DD-13** + §11 Comms + `NFR-MSD-005` |
| **`:571`** (was `:545`) must NOT deactivate either row or insert a duplicate | **DD-2** (§5.3) + DD-13 |
| **`:572`** (was `:546`) AND IT MUST be seeded from a real read, never literals | **DD-19**, extended to the OICR-path fixture (`K-17`) |
...
On the Innovation Use path validation fires first, so the symptom is a `400`. On the **OICR** path there is no validation today, so the symptom is silent row replacement. ⚠️ **CORRECTED 2026-08-27 (T-07 review):** the superseded text read *"DD-13's pipe closes that door too"*, which **overstates it**. `DD-13` closes only the **echoing-client** variant — a client that resends the raw `DECIMAL` string hits `typeof value !== 'number'`, which **both** rule-map branches open with, so it `400`s on **every** role and **both** write paths. But the variant the table above depicts — hydrated `"10.0000"` versus incoming `10` — requires a client that **coerces on read**, which is exactly what `DD-3`/`T-11` guarantees on the Innovation Use path and what any machine client sending JSON numbers does. In that configuration validation **passes** and the row is replaced **silently**. **So `DC-16` is closed by `DD-2` alone**; `DD-13` closes only the echoing variant. Not a product defect — the silent variant needs `DD-2`'s `from` removed, a regression state in which `T-07`'s round-trip fixture is already red — and it is why that fixture asserts the **read-shape invariant** rather than either symptom: the invariant dominates both symptom-specific checks. **DD-2 is what makes the keys agree**, and it is the only fix that serves both paths.
...
**Scale domain 0–4, guarded.** 4 is the column's scale; above it the UI would accept precision the database drops. A value outside the range is a configuration error, not rounded quietly. ⚠️ **Corrected by `T-12` (`T-10` carry, 2026-08-27):** the mechanism is a **synchronous throw in the `maxFractionDigits` setter** (`quantification-item.component.ts:58-65`, `Number.isInteger(value) && value >= 0 && value <= 4`), and it throws **in production too, not only at development time** — nothing in the setter or its callers distinguishes a dev build from a prod one. Every binding in the client is today a static in-domain literal (`STUB-1`, §8), so the throw is unreachable in the current tree — but if a future call site ever derived this input at runtime, the throw would propagate inside Angular's change detection and **abort the tick**, which on an in-progress form is silent data loss, not a caught configuration error. The guard's *intent* — reject rather than silently clamp — still holds; only the "development time" qualifier was wrong.
...
- `R-IUP-008` — `docs/specs/archive/2026-08-26-innovation-use--details-page/requirements.md:450-480` (⚠️ range corrected by `T-12`: was `:450-478`, now +2 lines from the `S-10` amendment note)
...
| ID | Claim | Status |
| --- | --- | --- |
| **U-1** | §9.2's expected renders for DD-10's expression | ✅ **VERIFIED by execution (`T-08`).** All seven cases (the six over `report_oicr`'s real `decimal(24,4)` column plus the `bigint`-column case, seven asserted and count-checked) rendered exactly as §9.2 predicted: `10.0000`→`10`, `-10.0000`→`-10`, `2.5000`→`2.5`, `-0.7500`→`-0.75`, `0.0000`→`0`, `bigint 10`→`10`, `NULL`→`Not provided`. `test/fixtures/innovation-use/report-oicr-number-rendering.fixture-spec.ts`, `npm run test:fixtures` |
| **U-2** | `ALGORITHM=COPY` rebuild cost on the live table | ✅ **MEASURED by `T-01`'s pre-flight (2026-08-27), corrected by `T-12`.** `result_quantifications` on Dev holds **80 rows total** (42 role 1, 37 role 2, 1 role 3) — the copy phase is effectively instant at this size. ⚠️ **Scoped, per `T-01`'s own advisory:** this is the copy phase only. `ALGORITHM=COPY` also needs an exclusive metadata lock whose *acquisition* time depends on concurrent long-running transactions, not on row count — report this to DevOps as *"copy time negligible; MDL wait independent of size,"* not as a blanket all-clear |
...
| **U-5** | §9.2's four **property** claims (Exact / `down()`-safe / Version-portable / Type-stable) | ✅ **Three of four VERIFIED by execution (`T-08`); one named as residue, not an execution gap.** **Exact:** confirmed — `CAST(TRUNCATE(x,0) AS CHAR)` returned plain integer strings (`'10'`, `'-10'`, `'0'`) with no scale/rounding artifact for every integral case, on both the `decimal(24,4)` and `bigint` columns. **`down()`-safe:** confirmed directly by the falsifier — the same `bigint`-column rows give the **bare** trim `'10'`→`'1'`/`'0'`→`''` while `DD-10`'s **guarded** expression on those identical rows still renders `'10'`/`'0'`; `NULL` also confirmed benign under both column types. **Type-stable:** confirmed — both branches executed (`CAST` via every integer case, `TRIM` via the two force-seeded fractional cases) through the live view with no `1267` collation error and no new `SHOW WARNINGS` entries (`U-8`, answered the same way). **Residue: Version-portable is not an execution claim at all** — it asserts `IF`/`TRUNCATE`/`TRIM`/`CAST … AS CHAR` predate MySQL 8.0.4, a syntax-availability fact settled by which functions were used, not by which server ran them; the container (8.0.46) and Dev (8.0.45, `T-06`) both running it is consistent with, but does not by itself prove, the claim for versions below either. `test/fixtures/innovation-use/report-oicr-number-rendering.fixture-spec.ts` |
...
| **U-8** | The charset/collation of DD-10's two `IF()` branches inside `CONCAT_WS` | ✅ **ANSWERED BY EXECUTION (`T-06`, `T-07`, `T-08`) — updated 2026-08-27.** *(Previously read "Unsettled"; `U-5`'s row already referred to it as answered, so §17 contradicted itself. Resolved here.)* **What settles it:** both `IF()` branches executed through the **live view** — `CAST` via every integer case, `TRIM` via two force-seeded fractional cases — with **no `1267`** and **no new `SHOW WARNINGS`** entries. ⚠️ **Attribution matters, and neither raw collation readout is the answer:** the bare-expression readout is the *measuring session's* `collation_connection` (the view's stored one is `utf8mb4_unicode_520_ci`), and the view-column readout is **fixed by `report_field`'s declaration** (`baseline.sql:6560-6563`, parameter and return both `COLLATE utf8mb4_unicode_ci`) and so is **structurally unable** to detect a branch mismatch. **Structurally, there are no two collations to aggregate:** `IF()` resolves its branches *before* `report_field` is called, and both branches take their charset from the connection (`CAST … AS CHAR`, and the implicit numeric→string conversion inside `TRIM`). The `convert(… using utf8mb3)` the live view needs on `impact_area` (`baseline.sql:8080`) is a **wrapper**-level mix — `report_field`'s declared return against view literals — not a branch mix, which is why it was a reasonable worry and is not this expression's problem. ⚠️ **One honest residue:** the fixture's `expect(collationErrors).toHaveLength(0)` **cannot redden for its stated reason** — `1267` *aborts* the statement, so it would surface as a thrown error and never appear in a `SHOW WARNINGS` taken after a successful `SELECT`. The conclusion is carried by the statements **executing without throwing**, not by the empty array |
| **U-9** | That any existing role-3 row sits **inside** DD-14's scale-4 bound | ✅ **MEASURED by `T-01`'s pre-flight (2026-08-27), corrected by `T-12`.** Role 3 holds exactly **1** row on Dev, with `\|value\|` well inside the bound (global `max_v = 87,654` against the bound `549,755,813,887`). Both pre-flight stop conditions were also shown able to fire on seeded scratch input (K-004). **Dev-only** — Prod was not queried and must be re-measured at rollout per `T-01`'s own disclosed limit |
...
| **U-11** | `DD-12`'s claim that defaulting `maxFractionDigits` to `0` leaves OICR *identical or stricter* | **Narrowed at Round 4, not closed.** With `U-4` resolved the **direction** is now established from source rather than assumed: decimals refused before and after, paste rendering goes `2.5` → `3`. Executed alongside it: at `maximumFractionDigits: 0` PrimeNG's `getDecimalChar()` yields `""`, so `_decimal` degenerates to an empty character class that matches nothing and the separator is refused by a *different* branch (`onInputKeyPress`, code 46 ∉ 48–57) than the one that refuses it today. Same outcome, different mechanism. **What is still unverified is the rendered end-to-end claim** — no browser was reachable, and the interaction of `showButtons`, blur/`validateValue` and `ngModel` write-back was traced but never observed firing. `DC-1`'s rendered assertion is the gate ⚠️ **Citation corrected 2026-08-27 (T-10 review):** this row previously named **`DC-3`** as the gate, but `DC-3` is a **server** DTO accept/reject table — as written, `U-11`'s gate did not exist. The client gates are **`DC-1`** (effective value on the real `app-input`/`p-inputNumber` instance — **satisfied by `T-10`**) and **`DC-2`**. `U-11` stays **narrowed, not closed**: the end-to-end DOM half (`input.nativeElement.value` and the `ngModel` write-back) is observed by nothing, and jsdom cannot reach it. |
