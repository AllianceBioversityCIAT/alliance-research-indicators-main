...
- **Falsifying input, named before the test is written (K-012):** ⚠️ **CORRECTED 2026-09-10 during execution, then corrected AGAIN before landing — the first correction carried a false universal. Both rounds are recorded because the second is the instructive one.**
  ...
  - **Dark half:** substituting `--ac-grey-600` **cannot** falsify it — in dark that pair is **4.67:1**, which *clears* 4.5. Nor can `grey-700` (6.24) or `grey-800` (7.95). **Of the three tokens `design.md` §8.3 evaluated, all three clear AA on dark `--ac-grey-100`, so the light half's substitution falsifier has no analogue among the candidates.** The dark half's falsifier is therefore **REMOVAL** of the colour class: an element carrying no token inherits UA black on `#2b2b2b`, measured at **1.4832:1**. Removal reddens the class-presence assertion identically in **both** themes, because that assertion is theme-independent while the token resolves per theme in `colors.scss`.
  - **⚠️ What this does NOT say, and the first correction wrongly did.** It is **false** that no grey in the `[data-theme='dark']` block fails 4.5 on `--ac-grey-100`. That block holds **nine** greys and **`--ac-grey-500` `#7d7d7d` is 3.4397:1 — it fails**, as do 400 and below. A dark substitution falsifier *does* exist; it simply uses a token that **was never a candidate for this text**. The claim is scoped to §8.3's candidates, not to all greys — and the difference matters, because *"no dark substitution can falsify this"* would tell the next maintainer not to look (`judgment.md` **W5**: *"saying otherwise removes the reason to look"*).
  - **Provenance (`KZ-007` — verified against source, and the first attempt shows why that rule exists).** `design.md` §8.3 already recorded `--ac-grey-600` dark as **4.67:1 ⚠️** (*"Fails AA in light; clears it in dark by only 3.7%"*), and `judgment.md` **W1** records both judges independently recomputing **4.6676** after catching §8.3's table carrying `6.2` — grey-700's value, copied one row up. **W1 warned that *"a test written from this table would encode a false expectation"*, and the artifact that warning applied to was this falsifier line, which nobody connected at specify time.** **All eight figures asserted here were recomputed from `colors.scss` with a calibrated interpreter** (verified against the canonical `Y(#808080) = 0.21586`) and **every one matches**: 4.67 (4.66760) · 6.24 (**6.23729**, matching W1's 6.2373 exactly) · 7.95 (7.94901) · 2.91 / 2.9115 (2.91149) · 1.4832 (1.48315) · 3.4397 (3.43968) · 4.6676. **The earlier form of this sentence claimed the Reviewer's independent arithmetic as corroboration and was narrowed after it failed** — the closing Reviewer hand-computed `6.2327` for grey-700 dark and asked that it be confirmed with an interpreter before any edit. It was, and **the Reviewer's figure was the wrong one** (its `Y(#acacac)` was off by 0.00034); `design.md`'s 6.24 and W1's 6.2373 are both correct. **The `KZ-017` reason its check could not have caught itself: it calibrated on three figures that all pass (2.9115 / 4.6676 / 7.9490), and a calibration set of passing values cannot surface an error in a fourth.** Recorded because the near-miss ran the other way from expected — trusting the audit would have "corrected" four correct sites to a wrong value, citing a reviewer as authority. **The `≈ 1.45` in §8.3 and `judgment.md` S6 is itself imprecise (exact: 1.4832) — deliberately not propagated, and the gating test computes it rather than hardcoding it.**
...
| RB-7 | 2026-09-10 | **QA asked for the card to adopt the "OICR selected" style** (eyebrow + icon, bold title, `Label → value \| …` metadata row, status pill) from `oicr-form-fields.component.html`. `/akili-quick` was invoked and **escalated by its own triviality gate**: the pattern has **no slot for the description** (a required field, `R-IUC-002`/`R-IUC-004`), it **reverses `DD-6`/`DD-7`'s "prose, not a property list"** which T-07/T-08/T-09/T-10 all depend on, and it carries **11 hardcoded hex literals** against §4.2's token rule. Measured: four of its six text colours fail AA on `--ac-grey-100`, including the eyebrow at **2.91:1** — the exact pair T-08's falsifier uses as its failure case | **Routed to a new child spec after this one** (user, 2026-09-10), via `/akili-propose` with the QA screenshots as Visual Reference. **Description stays, below the metadata row** (user) — preserving `R-IUC-004`'s clamp and diverging from the OICR card deliberately. **For this spec nothing changes:** `DD-6`/`DD-7` stand, T-10's *"reads as prose"* remains the governing intent, and no task was minted here | Product owner / next spec | **open — carried forward, deliberately not absorbed** |
