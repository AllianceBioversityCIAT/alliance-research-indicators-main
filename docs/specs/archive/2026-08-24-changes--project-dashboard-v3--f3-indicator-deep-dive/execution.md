...
### Task T-10 — Full gates, latency, bundle delta, HITL close (IN PROGRESS)

- **Status:** [~] — automated gates complete 2026-08-24; latency + owner HITL pending
- **Automated gates (Leader-run, sequential per §4.3, all workers idle):**
  - Server unit: `npm test -- --silent` → **338 suites / 2480 passed**. `npm run test:cov` → All files 85.02 / 75.15 / 84.73 / 85.14 (floor 60 ✓).
  - Server integration: `npm run test:integration` → **F3 suite `agresso-contract-indicator-details.integration-spec.ts` PASS (6.6 s)**; the pre-existing `bilateral-primary-contributing-sp` T-13 suite refuses to run without `T13_MYSQL_PASSWORD` (deliberate env gate, local container credential — 9 "failures" are that refusal, not code red). **KZ-017 declared:** this run cannot reach the T-13 DB-invariant probes in this environment; unrelated to F3.
  - Client: full `npm test -- --silent` → **316 suites / 6720 passed**; `npm run test:coverage` → 97.98 / 92.82 / 97.34 / 98.35 (floors 40/20/45/30 ✓); `npx tsc -p tsconfig.spec.json --noEmit` → **937 errors = baseline** (no new); `npm run build` → Output location, **Initial total 1.12 MB — identical to the F2-close baseline** → NFR-DD-003 (±5 kB) met; deep-dive/echarts additions confirmed outside the initial bundle by that identity.
- **Latency (NFR-DD-002) — pending, with the plan recorded:** the process on :3000 serves `/api/configuration/:key` (200) but 404s every `/api/v1/*` route including pre-F2 ones — a stale/foreign build (likely the baked Docker image); measuring against it would be void evidence. Measure during the owner's HITL session against a freshly started local server on this branch: 3× `curl -w '%{time_total}' -o /dev/null -H "Authorization: Bearer <token>" 'http://localhost:3000/api/v1/agresso/contracts/reports/indicator-details?contract-id=A511'` — p95-proxy ≤ 800 ms, spread >±40% → inconclusive.
- **HITL (KZ-014) — pending owner:** light+dark screenshots of every new chart form (donuts, readiness histogram, scalability radar incl. a gap axis if data allows, policy funnel order, gender×youth stacked bars, publications-by-year, velocity strip, quantifications table); lazy fetch verified with the panel BELOW the fold (network shows zero `indicator-details` before scroll, exactly one after); F1 drills + F2 single-`reports/dashboard`-call intact; `/swagger` renders `reports/indicator-details`; carried notes for HITL from T-08/T-09 advisories: mount placement accepted, velocity keeps previous line during retry (expected), sparse copy carries indicator label.

#### T-10 close — owner HITL (2026-08-24)
- **Owner verdict:** approved ("Ya lo revisé y me gustó") with a live light-mode screenshot in-session: velocity strip rendering real monthly data (20 last 24 mo), tab strip with counts (KP 9 · CapSharing 8 · Policy 2 · InnovDev 1), Policy Change tab active showing the implicated-institutions stat tile over sparse satellite data — tri-state rendering consistent with design §5/D-F3-7 on real A511 data.
- **Evidenced:** panel live against the current branch build; visual forms in light mode; velocity + tabs + a sparse-data tab.
- **NOT evidenced, accepted by owner at close (KZ-014 — recorded, not silently absorbed):**
  - Dark-mode screenshots (debt carried since F1; still open — next dashboard HITL owes it).
  - Below-the-fold network check of laziness (attested by specs + the K-004 red only; jsdom-unprovable gap declared at T-08).
  - **NFR-DD-002 latency:** not measured — the only reachable local server runs a stale build (404 on all v1 routes; measuring it would be void evidence). Recorded as **inconclusive/pending**, to be measured post-deploy against dev (3× curl, ≤800 ms, spread rule) or on the next locally-started stack.
- **Final status: PASS — spec complete.** All T-01…T-10 done. Budget vs actual: 10 tasks / 10 · review rounds: 1 rework loop (T-08, 2 attempts) + 1 addendum — within the 2-round budget · runtime incidents: 3 quota exhaustions across two Leaders (Antigravity Leader, 2 Claude Implementer spawns), all recovered with zero lost work.
