...
- **Server:** repository specs asserting **generated SQL + params** per section (KZ-001) with fixtures for multi-row satellites, absent rows, NULL booleans (radar denominators), month bucketing; controller/service specs; **HTTP-path integration spec** (amended 2026-08-23): in-process `TestingModule` with controller + service + global interceptor/filter and the repository `overrideProvider`-mocked — no `AppModule`, no `DataSource`, no network; runs under `npm run test:integration` in seconds. Covers section presence rules (zero-result indicator omitted), partial failure, all-fail 500, 400; 401 is the `JwtMiddleware` unit spec. The full-`AppModule` `app.e2e-spec.ts` is an infrastructure smoke test and never a gate for this spec. **Dev ground-truth check** (requirements defect table): endpoint numbers vs hand-run SQL for ≥2 sections on A511, recorded in `execution.md`.
