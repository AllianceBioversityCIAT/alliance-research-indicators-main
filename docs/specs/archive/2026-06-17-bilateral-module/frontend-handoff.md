...
### 4.7 HLOs/indicators per result (NEW — 2026-05-26; LIVE since 2026-05-27)

> **⚠️ Superseded (2026-06-10):** the contract below (`aow_status` + `pairs[]`, PRMS `(SP, AOW)`-pair fan-out) is **no longer what the endpoint returns**. `GET .../hlos-indicators` was reshaped in place (same `/v1`, D-V2-2) to the lambda-toc **level-based catalog read**: `{ result_code, mapping_status, clarisa_project, result_type, allowed_levels, version_locked, catalogs[] }`. The FE contract of record is now [`./toc-mapping-v2/`](./toc-mapping-v2/) (`design.md` §4–§5; decisions D-V2-1..D-V2-8; OQ-V2-9 → new table `result_pool_funding_toc_alignment`). The section below is kept for lineage only. The PRMS pair machinery (`PrmsTocService`) is still in code but unused — deletion is gated on cutover verification (toc-mapping-v2 T-10, R-BIL-098 AC.2).

> **Supersedes** the 2026-05-26 stub version of this section (which assumed `?sp_codes=` from the FE and a 503 interim path). Sub-spec of record: [`./pending-items/requirements.md` R-BIL-077](./pending-items/requirements.md). Backed by commit `907993e7` (T-15.12) + the AOW-source revision (T-15.12-rev / D-PI-14, 2026-05-28). **No change to the response shape or the FE contract** — only where the backend reads AOW from changed.

For the "Map HLOs and/or indicators" panel. The endpoint walks the same chain as the SP picker (result → AGRESSO contract → `bilateral_project_mapping` → CLARISA project). It takes the **Science Program** from the mapped project, then looks up each SP's **Areas of Work** in CLARISA's `cgiar-entities` catalog, and builds one `(program, areaOfWork)` pair per SP × AOW — the FE passes **no** query params.

| Verb | Path | Auth |
| --- | --- | --- |
| `GET` | `/api/v1/results/:resultCode/pool-funding-alignment/hlos-indicators` | ROAR JWT |
...
```ts
{
  ...
  pairs: {
    ...
    area_of_work: string;   // AOW code, e.g. "AOW03" — from the CLARISA cgiar-entities catalog
    composite_code: string; // "SP01-AOW06" — mirrors PRMS upstream
    outcomes: {             // PRMS OUTCOMEs (HLOs in our domain language)
      ...
      indicators: {
        ...
        targets_by_center?: { targets?: ...; centers?: ... };
        // ...more upstream fields; see PrmsTocIndicator in `dto/prms-toc.types.ts`
      }[];
    }[];
    outputs: { /* same shape as outcomes, category: "OUTPUT" */ }[];
    metadata: { total: number; outcomes: number; outputs: number };
  }[];
}
```

**Where does AOW come from?** It's NOT a STAR-side taxonomy and NOT a separate PRMS endpoint — it's CLARISA's canonical SP→AOW catalog at `GET /api/cgiar-entities?version=2`, where each level-2 "Key Area of Work" names its parent Science Program by **code** (e.g. `compose_code: "SP02-AOW03"`). Backend reads the project's SP(s), pulls each SP's AOWs from this catalog, calls PRMS once per `(SP, AOW)` pair, drops the pairs PRMS has no data for, and returns the rest grouped. No persistence, no schema change on the FE side. (Originally — D-PI-13 — AOW was read from the project's own `project_mappings_array[]`, but every AOW there shares a global `parent_id`, so that yielded only one usable pair catalog-wide; **D-PI-14** switched the source to `cgiar-entities`. See [`./pending-items/execution.md`](./pending-items/execution.md) T-15.12-rev entry.)

UX rules — endpoint **always returns 200**; drive the empty-state UX off `aow_status`:

| `aow_status` | What it means | FE rendering |
| --- | --- | --- |
| `"unmapped"` | No active `bilateral_project_mapping` for this result's contract (or no AGRESSO contract, or CLARISA no longer exposes the linked project). `pairs[]` empty. | Same affordance as the SP picker's `mapping_status: "unmapped"` (see §4.6) — "Contact admin to link this contract." Hide the HLO/indicator panel or show the empty state. |
| `"no_aow_mappings"` | Mapped to a CLARISA project, but there's nothing to show: the project has no Confirmed SP in the active portfolio, the catalog lists no AOWs for those SPs, OR PRMS has no ToC data for any derived `(SP, AOW)` pair. `pairs[]` empty. | Show a distinct empty state — e.g. "This project's HLO/indicator data isn't available yet." Don't ask the operator to do anything; this is an upstream (CLARISA/PRMS) data state, not a missing-mapping error. |
| `"has_aow"` | Mapped + ≥ 1 `(SP, AOW)` pair **with PRMS ToC data**. `pairs[]` populated (pairs PRMS has no data for are omitted, so every entry has at least one outcome or output). | Render the SP → AOW → outcome/output → indicator tree. Each `pair` is one SP/AOW combination; group `outcomes` (HLOs) and `outputs` separately within each pair, or merge them with a category badge if your design prefers. |

Caching + resilience:
- Backend caches each `(program, areaOfWork)` pair for 5 minutes in-memory, keyed by `composite_code`.
- On PRMS upstream error with a **warm** cache → backend serves stale + logs a warn (200 still returned).
- On PRMS upstream error with a **cold** cache → backend returns **503** with `description = "PRMS ToC temporarily unreachable"`. Treat as a transient failure; retry after a short backoff.
- If `ARI_PRMS_TOC_HOST` is unset on the deployed env → backend returns **503** with `description = "PRMS ToC integration not configured (ARI_PRMS_TOC_HOST missing)"`. Operations issue, not an FE one.

`composite_code` always echoes the PRMS upstream value — useful as a stable React key or for cross-referencing PRMS UIs.
...
| 2026-05-27 | **T-15.12 / PRMS ToC live (commit `907993e7`, branch `AC-1594-bilateral-module-v2`).** Single FE-visible delta: **`GET /api/v1/results/:resultCode/pool-funding-alignment/hlos-indicators` is now live** (§4.7 rewritten). Key contract changes vs the 2026-05-26 stub: (1) **no query params** — the FE passes nothing; backend derives `(program, areaOfWork)` pairs from the mapped CLARISA project's `project_mappings_array[]` (AOW entries with `parent_id` → SP). (2) **Always 200**; FE drives empty-state UX off the new `aow_status: "unmapped" \| "no_aow_mappings" \| "has_aow"` discriminator. (3) Response now carries `pairs[]` (grouped by `composite_code`, e.g. `"SP01-AOW06"`) instead of the flat-by-SP shape — each pair has `outcomes[]` + `outputs[]` + `metadata`. (4) 503 still possible on PRMS upstream cold-cache failure or missing `ARI_PRMS_TOC_HOST` env var. Closes OQ-RV-2; answers OQ-IM-2 (the AOW model question the FE asked) — AOW is sourced from CLARISA, not a STAR-side or PRMS-side taxonomy. Decision **D-PI-13** in [`./pending-items/execution.md`](./pending-items/execution.md). | ARI backend team |
| 2026-05-28 | **T-15.12-rev / AOW source switched to the `cgiar-entities` catalog (D-PI-14).** **No FE contract change** — same endpoint, same no-params request, same `aow_status` + `pairs[]` response. Backend-only fix: AOW is now sourced from `GET /api/cgiar-entities?version=2` (parent SP referenced by code) instead of the project's `project_mappings_array[]`, whose AOW entries all share a global `parent_id` and yielded only `SP01-AOW06` catalog-wide (which PRMS test lacks). Net effect for the FE: the panel now actually **populates** for mapped projects (e.g. D527 → SP02 + SP06 → 10 populated pairs) instead of always returning `no_aow_mappings`. Also: a per-pair PRMS 404 is now treated as data-absence (that pair is omitted), not a 503; `no_aow_mappings` now also covers "PRMS has no data for any pair". §4.7 updated in place. | ARI backend team |
| 2026-06-10 | **toc-mapping-v2 / lambda-toc level-based read (branch `AC-1594-bilateral-module-v2`, sub-spec [`./toc-mapping-v2/`](./toc-mapping-v2/)).** Breaking FE-visible deltas, same `/v1` (D-V2-2 — sole consumer is the unshipped STAR FE): (1) **`GET .../hlos-indicators` reshaped in place** — `pairs[]` / `aow_status` / `no_aow_mappings` are gone; response is now `{ result_code, mapping_status, clarisa_project, result_type, allowed_levels, version_locked, catalogs[] }`, sourced per `(SP, level)` from lambda-toc (§4.7 banner; full shape in `toc-mapping-v2/design.md` §4). (2) **PATCH alignment accepts `toc_alignments[]`** — atomic validation (whole PATCH 400s, six error codes, D-V2-8) + 409 `errors.code: 'toc_mapping_version_locked'` gate (D-V2-7). (3) GET/PATCH alignment read-back now carries `version_locked` + snapshot-sourced `toc_alignments[]` (frozen shape D-V2-5). New table `result_pool_funding_toc_alignment` (OQ-V2-9 → proposal Option A, D-V2-1); new tool module `domain/tools/toc-integration/` (env `ARI_TOC_INTEGRATION_HOST`). PRMS pair machinery removal is gated (toc-mapping-v2 T-10). | ARI backend team |
