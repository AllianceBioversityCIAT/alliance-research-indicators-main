...
| HLOs/indicators per SP | Owned by PRMS ToC (`/api/public-results-framework/toc-results`). Endpoint `GET /api/v1/results/:resultCode/pool-funding-alignment/hlos-indicators` takes no params — the **Science Program** comes from the mapped CLARISA project's `project_mappings_array[]` (level-1 SP entries) and each SP's **Areas of Work** come from the canonical `GET /api/cgiar-entities?version=2` catalog (`ClarisaCgiarEntitiesService`), which names each AOW's parent SP by **code**. Each SP × its catalog AOWs is one `(program, areaOfWork)` pair; fans out one PRMS call per pair, cached 5 min per `compositeCode`, warm-on-error / cold-503, and **drops pairs PRMS has no ToC data for** (404 = data-absence, not outage). Response carries `aow_status: 'unmapped' \| 'no_aow_mappings' \| 'has_aow'` + `pairs[]`. Implemented by `PrmsTocService` + `ClarisaCgiarEntitiesService` + `BilateralService.getHlosIndicatorsForResult`. Decision **D-PI-14** (in `pending-items/execution.md` T-15.12-rev entry) supersedes D-PI-13's mechanism: the project-embedded AOW entries are unusable (all share global `parent_id` 267 = SP01, yielding only `SP01-AOW06` across all 299 projects), so AOW is sourced from the catalog instead. AOW remains CLARISA-owned, not FE-supplied. | T-15.12 (done 2026-05-27, commit `907993e7`) + T-15.12-rev (catalog source, 2026-05-28) |

> **Superseded (2026-06-10) — "HLOs/indicators per SP" row only.** The (SP, AOW)-pair ToC read described in that row (`PrmsTocService` fan-out to PRMS public-results-framework; `aow_status` + `pairs[]` envelope) was replaced by the lambda-toc **level-based catalog read**: `GET .../hlos-indicators` now returns `{ result_code, mapping_status, clarisa_project, result_type, allowed_levels, version_locked, catalogs[] }`, sourced per `(SP, level)` from the new `domain/tools/toc-integration/` module (env `ARI_TOC_INTEGRATION_HOST`). See [`./toc-mapping-v2/`](./toc-mapping-v2/) — decisions D-V2-1..D-V2-8 (`design.md` §13); OQ-V2-9 resolved as the new table `result_pool_funding_toc_alignment` (proposal Option A). The PRMS pair machinery (`PrmsTocService`, AOW fan-out) still exists in code but is no longer used by this read — its deletion is **gated** on a recorded cutover-verified note (toc-mapping-v2 T-10, R-BIL-098 AC.2). All other rows in this table (SP picker, mapping table, PATCH validation, renames) remain current.
