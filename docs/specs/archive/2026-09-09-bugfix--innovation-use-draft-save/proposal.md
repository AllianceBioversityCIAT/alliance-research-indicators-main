...
| Fix the duplicate message | ⛔ **Superseded 2026-08-21 at T-02 attempt 1 — this is where the wrong prescription originated.** It read: *"Keep the page-owned block (it uses `.trim()`, so it covers blank and whitespace); **suppress** the shared `app-textarea`'s own message on this field."* Suppression is **forbidden by T-02's own falsifying input**, which requires deleting the page-owned block to fail *only* the whitespace case. The shipped design makes the two messages **disjoint** instead — `app-textarea` owns raw-empty, the page block owns whitespace-only — with the shared component's bindings untouched. See `requirements.md` §6 R-IUD-003. **The constraint that did survive: do not edit `TextareaComponent`** (T-09's blast-radius ruling, still binding) |
