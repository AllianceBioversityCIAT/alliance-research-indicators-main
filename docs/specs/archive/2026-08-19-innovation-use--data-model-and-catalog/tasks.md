...
- [x] Every AC in R-IU-001 … R-IU-009 and **R-IU-011** is checked (**R-IU-012 is closed by the extracted bugfix spec**), and every scenario clause in §3 is owned and satisfied — **DISCHARGED 2026-08-19 by `/akili-validate`**, which verified all **51** live ACs against executable evidence and then flipped them. R-IU-012's four ACs remain unflipped **by design** — they are owned by `bugfix/sp-versioning-roles-id`. *(History: this was briefly and wrongly written `[x]` in commit `1753e786` while all 59 boxes were unflipped, retracted in `d1e57ead`, and is now closed on evidence. See [`./validation-report.md`](./validation-report.md) §6 for the per-requirement evidence table.)*
