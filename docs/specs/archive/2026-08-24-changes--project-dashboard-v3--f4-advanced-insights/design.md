...
| D-F4-8 | 2026-08-24 | **Pivot addendum (owner-approved at T-03 attempt-1 FAIL):** the cycle-time submission anchor is a NEW forward-looking canonical value **`RESULT_SUBMITTED`** added to `REVIEW_EVENT_TYPE`; approval anchor = `REVIEW_DECISION` + `decision = APPROVE`. Audit-edit events (`POOL_FUNDING_ALIGNMENT_CHANGED`, `INDICATOR_MAPPING_CHANGED`) are NOT submission proxies (re-affirms Option B's rejection). The submission/approval mapping lives in the canonical constants file, not as calculator-local literals. The future `reviewDecision` implementation MUST write both a `RESULT_SUBMITTED` event (at submission) and the `REVIEW_DECISION` event (at decision), importing these constants | Cycle time must measure submission→approval, not edit→approval; naming the value now (beyond the archived design's illustrative list) makes the forward contract explicit instead of deferring a second pivot |
