...
| **Inline-style assertions** (new, `DD-10`) | that the border is set as an **inline style**, which outranks every stylesheet rule layered or not — so a passing assertion here does imply the border paints. ~~`element.style.border` is a real, readable string **in jsdom**~~ — **Correction 2026-09-02 (T-04 Reviewer, confirmed from `cssstyle@2.3.0` source):** readable in jsdom **only for a literal colour value**. For a `var()` reference the whole shorthand is dropped atomically — `border`'s `shorthandSetter` returns without writing when any part fails to parse, and `valueType()` has **no `var()` branch**, so the colour falls through to `KEYWORD` and fails `border-color`'s `TYPES.COLOR` check. Result: `element.style.border === ''` **and** `getAttribute('style') === null`, identically whether the binding is right or wrong — and longhands do not rescue it. **The observable is therefore the value Angular hands the CSSOM (an accessor spy on `CSSStyleDeclaration.prototype.border`), not a DOM read-back.** This sub-claim was written by the Leader *while correcting `DD-4` for exactly this failure mode* — `KZ-017` recurring inside its own correction. | the exact composited colour (antialiasing, OS colour management) — but the cascade question, which is what actually failed, becomes decidable |
