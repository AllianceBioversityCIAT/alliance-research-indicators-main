...
| | 34 | invalid `p-select` border — **mechanism REVISED 2026-09-02 (`RB-6`, `DD-10`): an `[style]` object binding, not a Tailwind class.** The class was inert and the border never rendered in any colour. Owned by `T-04` | `--ac-grey-100` |
| | 41 | duplicate-actor-type message | `--ac-grey-100` |
| | 52 | invalid "Specify other" input border — **same revision as `:34`** (`pInputText` loses the same cascade fight; directive-vs-component makes no difference here) | `--ac-grey-100` |
...
- [ ] AC.2 — Each changed site keeps its existing `material-symbols-rounded warning` icon, text, spacing and layout. **Colour is the only delta at the 6 text sites.** *Carve-out added 2026-09-02 (`RB-6`):* at the 2 border sites the **mechanism** also changes, because the original mechanism rendered nothing — see AC.3.
- [ ] AC.3 — **REVISED 2026-09-02 (`RB-6`, `DD-10`).** Originally: *"Border sites (`actor:34`, `actor:52`) keep `border-2`; only the colour token changes."* That is now impossible and was always inert: a Tailwind `border-*` utility on a PrimeNG element is defeated by PrimeNG's own unlayered rule. The criterion is replaced by: **the 2 border sites render a visible 2px `var(--ac-warning-1)` border via an Angular `[style]` object binding**, the dead `border-2 rounded-md border-[…]` fragments are removed from the `class` interpolation, and **no hex literal is introduced** (the codebase exemplar hardcodes `#E69F00`; this spec must not).
...
| **D-8** | **A generated, correctly-placed utility class that loses the cascade and paints nothing.** Distinct from `D-3` (a *misspelled* token, absent from `colors.scss`): here the class is spelled right, the token exists, the class lands on the element that paints — and an unlayered rule beats it. It fails **more silently than `D-3`**, because `D-3` at least leaves a name a grep can flag | ❌ **no automated gate, and no *possible* one at this tier.** Every check in this spec asserts over markup strings; jsdom loads neither PrimeNG's nor Tailwind's real stylesheets, implements no cascade layers, and paints nothing — so even `getComputedStyle` in a jsdom test cannot reproduce the fight. **Mitigation going forward (`DD-10`):** put the border in an `[style]` binding. An inline style outranks every stylesheet rule, so the cascade question becomes decidable. **Corrected 2026-09-02 (`T-04` Reviewer):** the *observable* is NOT `element.style.border` — this row originally claimed it was. `cssstyle@2.3.0` drops the whole `border` shorthand when any part fails to parse, and its `valueType()` has no `var()` branch, so a `var(--ac-*)` colour reads back as `''` whether the binding is right or wrong. The working observable is an accessor spy on `CSSStyleDeclaration.prototype.border`, which sees the value Angular hands the CSSOM. See `design.md` §6 | ✅ **It DID fail — this is the one defect class in this spec that actually shipped.** Caught 2026-09-02 by AC.10's human check after 7 automated gates passed over it |

> **`D-8` was added after the fact, and that is the point.** It is not a hypothetical: it is the defect
> this spec shipped into the working tree and that only a person looking at the screen found. It is
> recorded here so the next spec touching a PrimeNG surface starts with it on the list.
