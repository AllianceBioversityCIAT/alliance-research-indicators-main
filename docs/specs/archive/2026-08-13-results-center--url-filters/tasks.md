...
### T-08 [x] — Write effect + remove **both** wipes
...
- **Implementation notes:**
  ...
  - **Replace the "two ranges" check with two content checks.** After the change: `router.navigate` must appear in `results-center.component.ts` **only** inside the new write effect, and a grep for `\b(indicatorTab|statusTab|statusLabel)\b` in that file must return **zero** hits (the codec owns those keys per R3-2). *(Corrected 2026-08-13, post-T-08.* The check originally specified the **unbounded** substring `indicatorTab|statusTab|statusLabel`, which is **unsatisfiable**: it collides with T-06's `api.indicatorTabs` control-list resource and T-07's `indicatorTabStripSync` effect, returning 8 hits against a tree where the wipe is genuinely gone. The word boundaries are load-bearing. **This is the same defect class as the line-number trap the ⚠️ block above warns about, one layer up — a verification instruction decays against the tree it was written for exactly as a citation does.)*
...
- **Acceptance / done check:**
  - [x] Applying, changing and clearing each sidebar filter (plus `tab`) updates the URL correspondingly (R-RCU-003 AC.1). — six tests, one per canonical parameter, matching R-RCU-001's exact six
  - [x] **Clearing a filter removes its key from the address bar** — the R2-1 guard; assert the resulting URL string, not the serializer output. — asserted on the string the router double's real merge produced
  - [x] Zero `router.navigate` during init when arriving with stale singleton state from `/project-detail/:id` (R2-2 guard).
  - [x] Zero `router.navigate` on a parameter-less visit that restores persisted state, and restore still honoured on the **next** load (R2-5 guard).
  - [x] A filter change produces zero additional results requests (R-RCU-003 AC.3).
  - [x] Browser history depth after N filter changes is unchanged (R-RCU-003 AC.4). — `replaceUrl`, `relativeTo` and `queryParamsHandling: 'merge'` pinned on **every** call
  - [x] `?utm_source=email` survives the first filter change. — falsifiable only because the double now honours `queryParamsHandling`; see the execution log's finding 2
  - [x] `?tab=my` no longer self-destructs (JD-9 guard).
  - [x] **NFR-RCU-003 — the written URL never contains the cached `sec_user_id`, asserted on the resulting URL string for both scopes** *(assigned here 2026-08-12; see below)*. — the sentinel is seeded into `create-user-codes`, the same signal key production writes it to, after attempt 1's version was found structurally incapable of failing
...
- **Implementation notes:**
  ...
  - This is a rewrite of a **~1,550-line** spec — the single largest item in the budget, scoped as its own task deliberately. *(Was ~1,000 when written; T-06 through T-08 added ~550 lines, 424 of them T-08's write-effect block. Attempt 1 landed at 1,639 lines — 1,316 insertions / 1,297 deletions. **Attempt 2 — the one committed as `d934f1fe` — landed at 1,836 lines: 1,514 insertions / 1,298 deletions**, verified at `/akili-validate` 2026-08-13.)*
- **Acceptance / done check:**
...
- [x] **The manual cross-package check (D6) has been performed and recorded.** ✅ **CLOSED 2026-08-13 — performed by the product owner (d.casanas) against a running client; every filter, including the sidebar indicator multiselect fixed under T-13, confirmed applying correctly.** This discharges the spec's one acknowledged blind spot: the twin literals proved the *string* matches on both sides, and the human check proved the parsed filter actually reaches the table, the chips and the tab strip on a real page — which no unit test in either package can observe.

  > **The manual gate earned its place, and it is worth stating why before this spec is archived.** This same human pass is what found the T-13 defect — a filter that 6,479 tests, mutation testing, two reviewer lenses and a full `/akili-validate` all reported as working. D6 was written into the requirements as a *substitute* control for a coverage gap; it turned out to be the only gate that could see the product.

  *(Original wording, kept for the record: "⚠️ OPEN — not a task, and no automated gate covers it.")* Paste the string the server's `buildStarLink` produces into a running client and confirm the filtered view *(this line also named `CAPDEV_INDICATOR_TAB_QUERY`; T-10 deleted that constant — struck at `/akili-validate` 2026-08-13)*. See requirements.md §8 D6: no test in either package crosses the boundary; the twin literals in T-02 and T-10 are the only substitute control. **Human action required before this spec can be archived.**
  ...
  **Substitute control 1 — VERIFIED 2026-08-13 (static, by the closing Leader; does not discharge the checkbox).** The two literals were traced to source and compared byte for byte on the **same** query string:
  ...
  | --- | --- | --- |
  | Server — producer | `capdev-bulk-notification.service.ts:575-579` → `COMPLETE_CLIENT_HOST` (`app-config.util.ts:261-263`, plain concatenation, no slash normalization) | `` `${ARI_CLIENT_HOST}/results-center?indicator=${QueryIndicatorsEnum.CAPACITY_SHARING_FOR_DEVELOPMENT}&contract=${agreementId}` `` |
  | Server — enum | `indicators/enum/indicators.enum.ts:26` | `CAPACITY_SHARING_FOR_DEVELOPMENT = 'capacity-sharing-for-development'` |
  | Server — pin | `capdev-bulk-notification.service.spec.ts:428-433` | asserts href `` `${HOST}/results-center?indicator=capacity-sharing-for-development&contract=A100` `` (and `…&contract=B200`, two groups with different ids per KZ-004) |
  | Client — vocabulary | `results-center-url.vocabulary.ts:48` | `['capacity-sharing-for-development', 1]` |
  | Client — pin | `results-center-url.codec.spec.ts:210-220` | parses `{ indicator: 'capacity-sharing-for-development', contract: 'A100' }` → `filters.indicator === 1`, `filters.contract === ['A100']` |

  The server's asserted query portion and the client's asserted input are the **identical string**, so a slug change on either side reddens that side's pin. **What this does not prove, and why the human check still stands:** both pins are unit-level. Neither renders the Results Center, so neither can show that the parsed filter actually reaches the table, the chip and the tab strip on a real page — which is the half of D6 that only a running client can answer.

- [x] **`docs/ux-ui/design.md` decisions log records the URL vocabulary as a durable contract.** *(Done 2026-08-13 — `docs/ux-ui/design.md` §12.2, entry dated 2026-08-13, **updated after T-13**. Records the **seven** canonical parameters and the `indicator`-vs-`indicators` distinction (D-URL-18) with the defect that distinction exists to prevent, the byte-identical `QueryIndicatorsEnum` cross-package literal and its D6 blind spot, the per-token-class case policy, single-vs-multi-value and bounds, the no-deprecation-date legacy keys, the `sec_user_id` exclusion, and the component-owns-the-write-path structural guard.)*
