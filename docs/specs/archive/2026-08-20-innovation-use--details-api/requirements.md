...
- **Depends on:** [`docs/specs/archive/2026-08-19-innovation-use--data-model-and-catalog/`](../2026-08-19-innovation-use--data-model-and-catalog/) — **satisfied**, status `done` in the family manifest
...
| Blocks | [`innovation-use/details-page`](../../innovation-use/details-page/) (chunk 3) |
...
- [ ] AC.6 — **Every organization row MUST identify its organization**: either `is_organization_known = true` together with an `institution_id`, or an `institution_type_id`. A row supplying neither is rejected `400` naming the offending row index. *(**Added 2026-08-20 at `/akili-validate`, `validation-report.md` FAIL-1.** This is the organization counterpart of R-IUA-004 AC.6, and its absence was the defect: with no identity field, `ResultInstitutionTypesService.constructWhereClause` emits `{ result_id, institution_type_role_id }` and nothing else, so `findOne` returns an **arbitrary** existing row of this result, its primary key is adopted, and it is overwritten with nulls while every sibling row is deactivated — on a `200`. All three id-keyed protections are inert because the payload submits no id. Enforced in `ResultInnovationUseService.validateOrganizationsAreIdentified`, **before `BEGIN`**, so R-IUA-003 AC.2 holds for it by ordering rather than by rollback; not on the DTO, because DD-8 records that this repo has no global `ValidationPipe` and the fixture tier calls the service directly. **Residual, stated:** `customSaveInnovationDev` shares `constructWhereClause` and its own DTO is equally permissive, so the Dev endpoint remains exposed — see `../../innovation-use/family.md` FR-7.)*
...
- [ ] AC.5 — The module is registered in `main.routes.ts` under `results` as `innovation-use`, and the catalog module under `tools/clarisa` as `innovation-use-levels`. **AND each module is additionally registered in the module-graph file that instantiates it** — `ResultInnovationUseModule` in `entities.module.ts`'s `imports`, `ClarisaInnovationUseLevelsModule` in `clarisa.module.ts`'s `imports` — asserted over `Reflect.getMetadata('imports', <GraphModule>)`. **Corrected 2026-08-19 (T-07 Pivot, `design.md` DD-15).** The original wording named only the route files, and that wording is what produced the defect: `RouterModule.register()` stamps a path prefix onto a module constructor and never instantiates it, so **both** modules satisfied this AC literally while every one of their four endpoints returned `404`. A route node alone does not discharge AC.5, and neither does a spec asserting the shape of the `route` array.
...
- [ ] Security review — ⚠️ **REQUIRED — this row's premise was falsified by this spec's own remediation (2026-08-20).** It read *"not required: no auth path, secret, or exclude-list change"*, which was true when written and stopped being true when FAIL-1/FAIL-B added `assertInnovationUseOwnership` to `ResultActorsService` and `ResultInstitutionTypesService` — an **authorization control** on two tables shared with Innovation Dev, deciding by `(result_id, role)` whether a caller-supplied primary key may be written. `JwtMiddleware`'s exclude list and the secrets path are still untouched, so the *original* three clauses hold; the review is required because a **new** authorization boundary now exists that the row does not contemplate. See also **FR-7** (→ [AC-1718](https://cgiarmel.atlassian.net/browse/AC-1718)) in `../../innovation-use/family.md`: the sibling `customSaveInnovationDev` endpoint over the same tables has **no** such control, so the platform's exposure is asymmetric — which is precisely the kind of thing a security reviewer, and not an implementer, should sign off on.
