...
- **Status:** **all 13 tasks `[x]`** (T-01 … T-13; T-13 closed 2026-08-20 when the human `/swagger` observation landed). **NOT archive-ready** — see [`./validation-report.md`](./validation-report.md) (second run, 2026-08-20): 7 FAIL, of which two were product defects **now fixed** (FAIL-1, an identity-less organization row overwriting an arbitrary sibling; FAIL-2, a phantom-collision reconcile inside this spec's own PK remediation) and five are record defects, most now swept. **Two human gates remain open:** the security review (`requirements.md` §15) and **FR-7** (→ [AC-1718](https://cgiarmel.atlassian.net/browse/AC-1718)) in [`../../innovation-use/family.md`](../../innovation-use/family.md) — `customSaveInnovationDev` shares the same tables and has none of this spec's four protections, so the platform exposure is **asymmetric**; do not summarise this archive as "authorization fixed". Review rounds **≥ 26** (see §7). *(⚠️ **This line was rewritten 2026-08-20 at the second `/akili-validate`.** It had carried five stale or retracted claims at once — T-13 as `[~]`, the `it.failing` quarantine as live, options A/D as open, "role isolation itself is proven" (a phrase retracted in T-10's own status below), and a T-12 blocker long closed — while being the first thing `/akili-archive` and any resuming session reads.)*
- **Owner:** David Felipe Casañas Hernández
- **Linked requirements:** [`./requirements.md`](./requirements.md)
- **Linked design:** [`./design.md`](./design.md)
- **Parent spec:** [`../../innovation-use/family.md`](../../innovation-use/family.md) — chunk 2 of 3
- **Last updated:** 2026-08-20 (second `/akili-validate`)
...
- [ ] `GET /api/v1/tools/clarisa/innovation-use-levels` returns ten rows in a `ServerResponseDto`, each carrying `id`, `level`, `name`, `definition` *(R-IUA-010 AC.1, AC.2)*
  > ⚠️ **UN-TICKED 2026-08-20 at the second `/akili-validate` (FAIL-3).** It had been ticked as "released by T-13 c6's human observation". **That observation does not cover this criterion.** `execution.md` scopes it, in its own words, to *"the one thing static evidence cannot — that the page **renders** as intended"*, and `test-report.md`'s G-3 states that *"the `ServerResponseDto` envelope on the wire"* is proven at **no tier**. A rendered Swagger page is not a live `200` carrying ten rows; T-01's own limits say *"No live `200`"*. The ten rows and the four columns **are** proven at the service tier — only the live HTTP response is not. **Class: BLOCKED ON G-3** (an e2e project pointed at the scratch container), not blocked on T-13 c6. Sibling c4 — the *rendered* surface — **is** genuinely discharged by that observation and stays `[x]`.
...
- [x] The endpoint renders under the `Clarisa` Swagger tag with the bearer lock *(the **rendered** surface is exactly what T-13 c6 observes; **released 2026-08-20**)* *(AC.5)*
...
Add `customSaveInnovationUse(resultId, data, manager)`, modelled on `customSaveInnovationDev` (`result-actors.service.ts` → `customSaveInnovationDev`) with role `ActorRolesEnum.INNOVATION_USE` and the five `int` count columns instead of the four booleans.
...
Add `customSaveInnovationUse(resultId, data, manager)` modelled on `customSaveInnovationDev` (same file, that method) and its five private helpers, role `InstitutionTypeRoleEnum.INNOVATION_USE`, carrying `organization_count` through `buildUpdateData` and `buildDataTemplate`.
...
- **T-06's validation runs entirely before `BEGIN`** — the payload-shape, duplicate-actor and level-explanation rules. That is what makes "a failure persists nothing" a property of ordering rather than of rollback **for those rules**. *(Scoped 2026-08-20: the **ownership check** added by the FAIL-1 remediation runs **inside** the transaction as the first statement of each `customSaveInnovationUse`, and holds by **rollback** — `design.md` DD-3's one recorded exception, and §5.1 steps 7a/8a. This sentence was unqualified and became false when that gate landed; found by the two-direction sweep on the §5.1 correction, not by reading T-06.)*
...
- [x] `results.updated_at` advances across the save *(AC.7)* — *(corrected 2026-08-20: this criterion read `results.last_updated_date`, **a column that does not exist**. `AuditableEntity` has only `updated_at`, and the fixture had silently re-mapped it. The AC was corrected in `requirements.md` at the previous validation; **this inheriting site was missed**, which is the sweep failure Auditor A named as this spec's dominant pattern.)*
...
- [x] **No row belonging to result 2 changed after a save that submits result 2's row ids inside result 1's payload** *(✅ **closed 2026-08-20** — the FAIL-1 remediation added `assertInnovationUseOwnership`, scoped by `(result_id, role)`, to both `customSaveInnovationUse` paths. **The two `it.failing` markers inverted to passing**, which is exactly what choosing `it.failing` over `.skip` at the Pivot was for: the fix announced itself. A **third** case now also passes — the same-result **cross-role** variant that `/akili-validate` found un-gated and that this criterion never covered)* — whole-row `SELECT *` diff, all three shared tables *(AC.3, second half; **added 2026-08-19 at T-10's dispatch** — see the Scope note. This is the only criterion that reaches `buildUpdateData`/`buildNewData`'s id-present branch, which the empty-array save never touches. **If result 2's rows ARE modified, report it as a `PRODUCT_BUG`-class finding and stop — do not weaken the assertion to make it pass.**)*
...
- **Status:** ~~todo~~ → ~~`[~]` 2026-08-19~~ → **`[x]` DONE 2026-08-20** — PASS on attempt 1, zero rework. The tenth criterion, the human `/swagger` check, was **closed 2026-08-20 by the user's own observation** (recorded verbatim in `execution.md`), releasing T-01 **c4** with it *(and, as recorded then, c1 — **retracted 2026-08-20**: c4's rendered surface is what the observation covers; c1's live `200` is not, and c1 is re-opened as BLOCKED ON G-3)*. Originally: **PASS with ONE criterion outstanding BY DESIGN.** Nine of ten Done criteria met. **Criterion 6 — the human `/swagger` check — awaits the user's own observation and cannot be discharged by any agent**; it is the only gate `R-IUA-013 AC.3` has, and the task requires it be *"reported as a human observation, never as a command result"*. Per Step 2.3.0 a task with an outstanding gap never reaches `[x]` even on a Reviewer PASS. C-4 cleanup: **2 dead sites removed, 10 live guards kept**, classification enumerated by grep over all 14 files and ruled **conclusive** *(the tree now has **15** fixture files; the sixth Innovation Use fixture adds two further `*Seeded` guards outside that enumeration. Both are correctly **live**, so no regression — but the enumeration no longer covers the tree and the word "conclusive" is scoped to 2026-08-20's 14)*. NFR-IUA-001: **exactly 5 queries** with 52 active actor rows — and it **corrects this spec's own T-05 estimate of 4**, since `findOne` with a relation join costs 2 (TypeORM's id-picking subquery). Fixture suite 14/49 green **twice consecutively**; unit 336/2264; coverage 89.69/75.61/85.13/89.14. *(Point-in-time at T-13's close, 2026-08-19 — superseded four times since; current figures live in §7 only.)* Evidence: [`./execution.md`](./execution.md) → *T-13*
...
- [x] **Human check, recorded verbatim in the report:** *(✅ **closed 2026-08-20 by the user's own observation** — recorded verbatim in `execution.md` → *Human `/swagger` Observation*. No agent attempted, simulated or substituted it)* `/swagger` shows the section `GET` and `PATCH` — the two **own-declared** handlers — each with tag, `@ApiOperation` summary, bearer lock, and — for the `PATCH` — the `@ApiBody` schema. The catalog `GET` — the one **inherited, unmodified** `BaseController` handler — shows tag and bearer lock but carries **no** `@ApiOperation` summary; confirm this as the exemption ruled in `design.md` DD-13 / `requirements.md` D-IUA-10, not as a defect *(R-IUA-013 AC.3)*
...
**Closure statement:** 13 functional + 3 non-functional requirements · 11 scenarios · 22 `BUT`/`AND IT MUST` clauses — **21 owned; one is not**. ⚠️ *(Corrected 2026-08-20 at the second `/akili-validate`. This read "all owned", contradicting `test-report.md` G-2, `validation-report.md` and `execution.md`, which all record `R-IUA-011`'s* "the submit transition is permitted" *as owned by no task. The clause count of 22 is correct. Note also that this matrix's own preamble scopes it to `BUT it must NOT` / `AND IT MUST` clauses — **22 of the 43** assertive scenario clauses — so the 21 plain `THEN`/`AND` clauses are invisible to it by construction, which is the mechanism behind every orphan in §7's open item, not an oversight in any one row.)*. No clause is discharged by citing a different requirement. Two ACs are discharged by a **human** check (R-IUA-013 AC.3) or carry a **declared-weak** gate (R-IUA-010 AC.3 via F-D); both are named as such above and in their tasks, not counted as ordinary passes.
...
- [x] All 13 tasks `done` *(T-13 closed 2026-08-20 when the human `/swagger` observation landed)*
- [x] Every **task-level Done criterion** in this document checked — **verified by grepping the unflipped-checkbox count, not inferred from "the tasks are done"**. ⚠️ *(**Wording corrected 2026-08-20 at `/akili-validate`.** The item originally read "Every AC in `requirements.md` §14 checked", which is false against the document it names: **§14 contains no checkboxes at all**, and all **73** AC checkboxes in `requirements.md` §7 remain unflipped. The 2026-08-19 audit verified the **61 task-level criteria in this file** and ticked this item on that basis — defensible in substance, false as written. Ticking `requirements.md`'s own 73 boxes is separate, unstarted work. This is the same mechanism the spec's own Kaizen names — "a checkbox that says 'verify by grepping' is a checkbox, not a gate" — one level up.)* Audited 2026-08-19: **61** task-level criteria were unflipped; **57 ticked against quoted evidence**, and **exactly 4 remain open by design** — enumerated below. *(**Carve-out added 2026-08-19 at the archive-readiness audit.** The original demanded the count reach **zero**, which is **unachievable**: two of the four record a defect and a human gate that must NOT be ticked, and two are transitively gated on the third. A Done definition that can only be satisfied by a false tick is the mechanism of **KZ-002's fourth recurrence** — the pressure to zero the count is exactly what substitutes "the tasks are done" for "the checkboxes are checked". The count is now a named exception list, not a zero.)*

  **⚠️ THREE OF FOUR ARE CLOSED. The unflipped count is ONE, not zero** — corrected 2026-08-20 at the second `/akili-validate` (FAIL-3). The claim below read *"ALL FOUR ARE NOW CLOSED. The unflipped count is zero, reached honestly rather than by a forced tick."* Three of the four were closed honestly. **T-01 c1 was not:** it was released by a human `/swagger` observation that covers the *rendered* surface, while the criterion asserts a live `200` carrying ten rows in a `ServerResponseDto` — the exact substitution of an adjacent satisfied thing that this methodology's validate phase exists to catch, committed in the sentence declaring the count zero. c1 is re-opened above and re-classed as **BLOCKED ON G-3**. The carve-out is retained below as the record of what was open and why — it was the right structure at the time, and the reason it can now be retired is that each item was resolved on its own terms, not waived:

  | Was carved out | Closed by |
  | --- | --- |
  | **T-10 c5** | The **defect was fixed** (option A), not the criterion softened. Both quarantined tests inverted to passing, plus a third for the cross-role variant |
  | **T-13 c6** | The **user's own `/swagger` observation**, recorded verbatim |
  | **T-01 c1** | ⚠️ **NOT closed — re-opened 2026-08-20.** T-13 c6 covers the rendered page, not a live `200` in a `ServerResponseDto`. Re-classed **BLOCKED ON G-3** |
  | **T-01 c4** | Released by T-13 c6 — the rendered surface *is* what that observation covers |

  **The original carve-out, as recorded 2026-08-19:**
...
- [x] `npm test -- --silent` green · `npm run test:cov` ≥ 60% · `npm run test:fixtures` green **twice consecutively** — **336 suites / 2296 tests** · coverage **89.80 / 75.82 / 85.31 / 89.27** (floor 60) · fixtures **15 suites / 71 tests, 0 failed**, twice on the same container. **All four figures re-derived by ONE run on 2026-08-20 at the second `/akili-validate`, after that round's FAIL-1/FAIL-2 fixes added 11 unit tests.** Re-run by the Leader rather than relayed. Recorded in `test-report.md` and `validation-report.md` — **grep all three before trusting any**; this cell has now been restated five times (2264 → 2275 → 2279 → 2285 → 2296) and the durable fix is fewer sites asserting it, not a better number here. They were deliberately withheld through two remediation rounds because the earlier values (336/2264 · 89.69/75.61/85.13/89.14 · 14/49) had gone stale twice; writing them once, from one verified run, is the fix for that class
...
- [x] Budget variance recorded against **13 tasks / ~2,400 LOC / ~24 rounds** — **ACTUALS: 13 tasks · **≥ 26** review rounds (was recorded as "24, exactly on budget"; a review died on a session limit and was re-dispatched, and three commits landed after the closing run — the exact figure is not derivable from the tree, so it is stated as a floor rather than a false exact) · fixture tier **4,619 LOC** for this spec's six fixtures plus the harness (was 3,225, measured before the last four commits) · spec-wide new+modified ~6,450 LOC, itself now a floor**, measured against the tree 2026-08-20 and re-measured at the second `/akili-validate`. Escalated, not absorbed: the fixture-tier overrun was reported at T-12 and the top-line actual is recorded here. *(Previously un-ticked 2026-08-20 at `/akili-validate`, because Rounds are recorded exactly (24/~24). **LOC is not.** T-12 escalated the fixture tier at ~2,545 (~2.8×); an independent measurement of the tree gives the fixture tier **3,225 (~3.5×)** and a spec-wide new+modified total of **~6,450 (~2.7× the top line)**. **No document records a top-line actual at all**, so this item was ticked against a sub-line escalation. Recording the measured figures is the remaining work.)* *(rounds re-baselined from 6–8 on 2026-08-19 by user ruling)*, and any overrun escalated rather than absorbed
