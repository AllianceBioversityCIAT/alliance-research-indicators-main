...
CapdevBulkNotificationService.dispatch(processId, fileContacts)
  ...
  4b. countTotalResults(processId)— ungrouped scalar, 1 read                    [R-CBU-008 / §4.1 total_results]
...
- **Stays unversioned**, where the handler already is: the change is purely additive and backward compatible, so it introduces no version segment where there is none. *(Corrected 2026-08-11 by `/akili-validate` — this bullet read "Stays on `/v1`" and survived D-T11-b's forward sweep, which grepped `api/v1` and therefore could not match a line carrying only `/v1`. It contradicted the correction note 11 lines above it.)*
...
**Query budget — `4 grouped reads + 1 ungrouped scalar read + 2 writes`, plus `2 config reads per dispatch` and `1 template read per group`.** That is **O(groups), never O(results)** — which is what NFR-CBU-001 and §11 actually require. *(An earlier draft claimed a constant "3 reads + 1 write"; that was false — `template.service.ts:11-22` issues a `findOne` on every `_getTemplate` call, each `EnvAppConfigUtil` accessor issues its own, and there are two writes. JD-04.)*

> **Corrected 2026-08-11 by `/akili-validate`.** This line read "`4 grouped reads + 2 writes`, plus `1 template read + 1 config read per group`" and was wrong in two places once T-09 landed. **Reads are five, not four:** `countTotalResults` was added for §4.1's `total_results` (the one column filtered by neither indicator nor contract) and runs unconditionally at `capdev-bulk-notification.service.ts:195`. **The config reads are per dispatch, not per group:** `CAPDEV_BULK_UPLOAD_ENABLED` and `CAPDEV_BULK_UPLOAD_CC_EMAIL` are each read once, outside the per-group loop (`:242`, `:260`). `execution.md` → T-10 already recorded the correct figure ("5 reads + 2 writes run per upload regardless of the flag"); this section was the stale side. **NFR-CBU-001 is unaffected** — the added read is one unjoined scalar query, so there is still no per-result fan-out, and the old per-group config figure was pessimistic rather than optimistic.
