...
> **Supersedes** the 2026-05-26 stub version of this section (which assumed `?sp_codes=` from the FE and a 503 interim path). Sub-spec of record: [`./pending-items/requirements.md` R-BIL-077](./pending-items/requirements.md). Backed by commit `907993e7` (T-15.12).

For the "Map HLOs and/or indicators" panel. The endpoint walks the same chain as the SP picker (result → AGRESSO contract → `bilateral_project_mapping` → CLARISA project) and derives `(program, areaOfWork)` pairs from the mapped CLARISA project itself — the FE passes **no** query params.
...
```ts
{
  ...
  pairs: {
    ...
    area_of_work: string;   // AOW code, e.g. "AOW06" — derived from CLARISA
...
**Where does AOW come from?** It's NOT a STAR-side taxonomy and NOT a separate PRMS endpoint — it's already inside CLARISA's `/api/projects` response, as level-2 entries in `project_mappings_array[]` (`global_unit_type_id: 26`, `cgiar_entity_type_object.prefix: "AOW"`) whose `parent_id` points back to the level-1 SP entry's `id`. Backend reads this at request time, builds `(parent_SP_smo_code, AOW_smo_code)` pairs, calls PRMS once per pair, and returns them grouped. No persistence, no schema change on the FE side — see decision **D-PI-13** in [`./pending-items/execution.md`](./pending-items/execution.md) T-15.12 entry.
...
| `"no_aow_mappings"` | Mapped to a CLARISA project, but that project carries only SP-level entries (no AOWs). PRMS cannot answer without an AOW. `pairs[]` empty. Today this is the case for ~90% of Bilateral projects in TEST (28 of 31). | Show a distinct empty state — e.g. "This project's HLO/indicator data isn't yet defined in CLARISA at the AOW level." Don't ask the operator to do anything; this is a CLARISA-side data state, not a missing-mapping error. |
| `"has_aow"` | Mapped + ≥ 1 `(SP, AOW)` pair derived. `pairs[]` populated. | Render the SP → AOW → outcome/output → indicator tree. Each `pair` is one SP/AOW combination; group `outcomes` (HLOs) and `outputs` separately within each pair, or merge them with a category badge if your design prefers. |
