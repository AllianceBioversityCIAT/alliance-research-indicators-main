...
| HLOs/indicators per SP | Owned by PRMS ToC (`/api/public-results-framework/toc-results`). Endpoint `GET /api/v1/results/:resultCode/pool-funding-alignment/hlos-indicators` takes no params — `(program, areaOfWork)` pairs are derived from the mapped CLARISA project's `project_mappings_array[]` (level-2 AOW entries paired with their `parent_id` → SP). Fans out one PRMS call per pair, cached 5 min per `compositeCode`, warm-on-error / cold-503. Response carries `aow_status: 'unmapped' \| 'no_aow_mappings' \| 'has_aow'` + `pairs[]`. Implemented by `PrmsTocService` + `BilateralService.getHlosIndicatorsForResult`. Decision **D-PI-13** (in `pending-items/execution.md` T-15.12 entry): AOW derivation comes from CLARISA, not FE-supplied, since PRMS exposes no `/aow-by-program` listing endpoint and CLARISA already carries the data. | T-15.12 (done 2026-05-27, commit `907993e7`) |
