...
- [ ] Budget variance recorded against **13 tasks / ~2,400 LOC / ~24 rounds** ⚠️ *(**Un-ticked 2026-08-20 at `/akili-validate`.** Rounds are recorded exactly (24/~24). **LOC is not.** T-12 escalated the fixture tier at ~2,545 (~2.8×); an independent measurement of the tree gives the fixture tier **3,225 (~3.5×)** and a spec-wide new+modified total of **~6,450 (~2.7× the top line)**. **No document records a top-line actual at all**, so this item was ticked against a sub-line escalation. Recording the measured figures is the remaining work.)* *(rounds re-baselined from 6–8 on 2026-08-19 by user ruling)*, and any overrun escalated rather than absorbed
