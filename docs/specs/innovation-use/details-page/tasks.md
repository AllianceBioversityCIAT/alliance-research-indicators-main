...
- **Status:** `[~]` **REOPENED 2026-08-26 at `/akili-validate`** — c7 and c9 discharges **retracted**: two independent T3 auditors found light-theme AA failures (`R-IUP-017` AC.3) and an unfocusable, unnamed delete control (`R-IUP-018` AC.1/AC.2) inside the exact regions those human checks structurally could not see. See `execution.md` → *`/akili-validate` — FAIL* · **Size:** M · **Dependencies:** T-09, T-10, T-12, **T-14**
