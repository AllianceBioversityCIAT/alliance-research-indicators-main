...
- [x] c10 — **Budget reconciliation against `design.md` §12.** ✅ **RE-RUN and CLOSED 2026-08-26** against the amended baseline (**14 tasks · ~3,400 written / ~4,800 re-baseline · ~31 rounds**), including `T-14`, the three Pivots and the `R1/R2/R3` remediation, after validation `F-3` (the 08-21 table was arithmetically inconsistent) and `F-4` (it predated `T-14`, and no document carried a post-amendment total). **Result: 6,133 LOC / 24 of ~31 rounds** — 6,328 / 25 with `RB-9`'s stylesheet. **LOC tripwire breached (+80.4% written / +27.8% re-baseline); rounds under budget at 77%.** Recorded and **not absorbed**; not re-escalated, the user having ruled on this same overrun three times with full information. Figures, the deriving command and the per-commit split live in **`execution.md` → `T-13` c10, their single home (`KZ-005`) — re-derive there, do not restate**.
