...
| **FR-11** | A saved Innovation Use row whose effective catalog `level` is below 6 (or absent) can still hold a leftover `innovation_use_level_explanation`: STAR re-sends the hidden text, and omitted-key preserve (DD-14) keeps it | none | [`docs/specs/bugfix/innovation-use-stale-justification/`](../bugfix/innovation-use-stale-justification/) — server write-time clear when effective catalog `level` is `< 6` or absent. Not a family child. |
