...
| 5 | Innovation Dev card details (readiness, description, geo scope) | [`dev-card-details/`](dev-card-details/) | none (children 1–4 done + archived; child 4 shipped the card this chunk extends) | no | **`pending`** — proposal only, 2026-09-10. Extends the card child 4 delivered with three read-only fields. Origin: a reviewer comment on Innovation Use, not a defect. |
