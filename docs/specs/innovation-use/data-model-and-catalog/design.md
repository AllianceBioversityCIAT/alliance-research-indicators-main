...
| Harmonize the two hard-delete routines | Transcript §4.1 records a pre-existing divergence. **Out of scope** — "fixing" it changes delete behavior for every indicator. ⚠️ **Corrected 2026-08-18** *(residual site of correction item 1, KZ-005; this formerly listed the divergence as "(`result_impact_outcomes`, `result_strategic_objectives`, and `SIGNAL` vs `RETURN FALSE`)")*: **the `result_impact_outcomes` / `result_strategic_objectives` half is already CLOSED** by the extracted bugfix's T-02b (migration `1784250000000`, on this branch). Only **`SIGNAL` vs `RETURN FALSE`** is still an open divergence to leave alone. **Read this the right way round:** `SP_delete_result_version` now *does* delete those two tables, and M6 must **keep** those two `DELETE`s — removing them would be a regression, not a refusal to harmonize (R-IU-011 AC.8, as amended) |
