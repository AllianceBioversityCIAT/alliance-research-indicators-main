...
- [x] All T-01 … T-14 are `done` (T-03 extracted — verify instead that both `sp-versioning-roles-id` repair migrations are present and ordered before M6) — true at the **task-status** level. ⚠️ Recorded at the same gate: **T-12 and T-13 carry `[x]` status lines while their own Done-item checkboxes (lines ~388–392 and ~415–419) remain unticked** — a pre-existing 2026-08-18 bookkeeping gap, not a work gap; both tasks have full Reviewer-PASS evidence in `execution.md`. Left as found rather than back-filled, for the same reason as the row below.
- [ ] Every AC in R-IU-001 … R-IU-009 and **R-IU-011** is checked (**R-IU-012 is closed by the extracted bugfix spec**), and every scenario clause in §3 is owned and satisfied — **NOT met as written; corrected 2026-08-19 by the Leader at the archive-readiness gate.** This was briefly written `[x]` in the T-14 finalize commit (`1753e786`) and is **false**: `grep -c '^- \[ \]' requirements.md` returns **59** — every AC checkbox in `requirements.md` is unflipped. The ACs are *substantively* discharged by T-04…T-14 (fixtures, migration specs, Reviewer PASSes — see [`./execution.md`](./execution.md)), but discharge was never **recorded** at the checkbox. Flipping 59 boxes on the Leader's own authority would manufacture exactly the unverified completeness this spec spent 13 review rounds eliminating. **Owned by `/akili-validate`**, whose job is per-AC conformance. Same class as **KZ-002** — an aggregate claim that grep falsifies.
