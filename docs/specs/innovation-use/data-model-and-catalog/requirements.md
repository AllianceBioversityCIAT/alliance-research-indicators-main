...
| F19 | Owned by the extracted `sp-versioning-roles-id` spec, not this one (DC-13 row above). Running it **before** the repair: `CALL SP_versioning` raises MySQL 1054 and the fixture errors rather than asserting. *(Corrected 2026-08-19 by T-14 attempt 2, M0-token sweep: formerly "before M0", retained here for the mechanism description only.)* |
...
- All four routines are amended by migration M6 via `DROP` + `CREATE` reproducing each body in full. **M6 depends on the extracted bugfix's repair migration** (R-IU-012): it reproduces `SP_versioning`'s body and must inherit the repaired one. *(Corrected 2026-08-19 by T-14 attempt 2, M0-token sweep: formerly "M6 depends on M0" — the repair was extracted to the `sp-versioning-roles-id` spec and is not one of this chunk's six migrations.)*
...
| ~~Repair~~ | ~~**`SP_versioning`** — blocks 3–4 only~~ | **EXTRACTED** → [`../../archive/2026-08-18-bugfix--sp-versioning-roles-id/`](../../archive/2026-08-18-bugfix--sp-versioning-roles-id/) as `repairSpVersioningObjectiveBlocks` (R-IU-012) — pre-existing defect, not an Innovation Use change; retained here as the record | n/a — owned by that spec |
...
| **RB-11** | **`SP_versioning` is non-executable in `main` today** (DC-13, R-IU-012), for all six indicators, with no test that would notice. | **High** | **Shipped** in the extracted [`sp-versioning-roles-id`](../../archive/2026-08-18-bugfix--sp-versioning-roles-id/) spec as `repairSpVersioningObjectiveBlocks`, committed on this branch and timestamp-ordered before M1–M6; **F19** is that spec's red-before-green regression gate. |
...
| ~~**D-11**~~ | ~~The `SP_versioning` repair (M0) is carried in this chunk by default, with the option to extract it into its own bugfix spec.~~ **RULED 2026-08-14 — option B.** The repair shipped as `repairSpVersioningObjectiveBlocks` in the extracted [`sp-versioning-roles-id`](../../archive/2026-08-18-bugfix--sp-versioning-roles-id/) spec, committed on this branch and timestamp-ordered before M1–M6 (`design.md` §12 "✅ Resolved escalation"). | Pre-existing, cross-indicator, and **blocking**: M6 reproduces the body, and no versioning fixture could run until it was repaired. Recorded as R-IU-012 / DD-13; F19 is that spec's regression gate. |
