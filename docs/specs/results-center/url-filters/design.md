...
Meeting §10.1 means dropping the template override, using the real `ResultsCenterService`, providing real control-list services for the child multiselects, and switching to a real router harness. **This was a rewrite of a ~1,550-line spec and was its own task in the budget** — the previous draft's estimate did not carry it. *(Post-T-11 the committed file is **1,836** lines: **1,514 insertions / 1,298 deletions** — commit `d934f1fe`, verified at `/akili-validate` 2026-08-13. The figures **1,639 / 1,316 / 1,297** that stood here were T-11 **attempt 1**'s; attempt 2 added a further ~197 lines and is what actually shipped. `execution.md` §9 recorded the attempt-2 numbers correctly — this section and `tasks.md` T-11 did not, which is the same stale-figure class this spec has now been bitten by four times.)*

> *Corrected 2026-08-13, post-T-08.* Three things in this section had decayed. **(1)** The line citations `101-108` / `112-117` now miss by ~70 lines (the real positions are `:170-188` and `:192-197`), so they are replaced with content-based pointers above — this spec has been bitten by stale line numbers three times now. **(2)** The template literal is written with **backticks** in the source, so the previously quoted `template: '<div></div>'` matches nothing under a literal grep. **(3)** T-06 already replaced the *canned snapshot* with a real `convertToParamMap`-built `ParamMap`, and T-08 added a derived `queryParams` getter over it — so what T-11 must remove is the **fabricated service mock**, not the param map. The file was **1,550 lines** when that note was written (~1,000 when the section was), and is **1,836** after T-11 *(corrected from 1,639 — an attempt-1 figure — at `/akili-validate` 2026-08-13)* — which matters because T-11 was the budget's single largest item.
...
| Review rounds | 2 | 3 | 3 | 3 | **3** | Unchanged. *(This cell read "still holding at 10 of 12 tasks: only T-08 consumed rework" — true when written, before T-11. **Final: 9 of 12 on attempt 1**; T-08, T-11 and T-12 each consumed exactly one rework round. The per-task budget of 3 was never exceeded. Corrected at `/akili-validate`, 2026-08-13.)* |

> **Final measured LOC — 2026-08-13, at `/akili-validate`.** **5,810 code insertions** (`*.ts` + `*.html`) across the spec's 15 commits, versus the **~4,600** carried by re-baseline #2 — **+26%**. The overage itself was already caught in flight: `execution.md` §11 recorded the spec passing ~4,889 at 11 of 12 tasks and **declined a fourth re-baseline on purpose**, routing it to Kaizen instead. What that record still under-called is the size: it projected **~5,100 final** on a T-12 estimate of ~200, and T-12 landed **607**. So the estimation-basis error named in re-baseline record #2 recurred **a fourth time, in the very note that diagnosed it** — the projection was corrected in its total while its per-task basis (~200 for T-12) was carried forward unexamined. Task count (12) and review rounds (3) never moved in any revision. **This is the spec's single most valuable Kaizen datum and is deliberately left for `/akili-archive`, not resolved here.**
