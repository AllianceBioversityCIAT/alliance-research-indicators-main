...
The effect writes signals transitively through `lazy()`, so it requires **`allowSignalWrites: true`**. The singleton's self-destructing `onChangeList` is left untouched (out of scope) and nothing here depends on it.
