...
| **OQ-F8** | **STAR API keys — CRITICAL PATH, partially addressed 2026-09-14.** Keys are issued by the PRMS team, **one per tool per environment**, and the key *is* the platform identity. **Config slot created:** `AppConfig.ARI_CLARISA_API_KEY` (`domain/shared/utils/app-config.util.ts`, sent as `x-api-key`) — distinct from `ARI_CLARISA_USER`/`ARI_CLARISA_PASS`, which authenticate STAR against CLARISA's own API. **Still missing: the key VALUES**, one for TEST and one for PRODUCTION. Nothing in this family can be verified end-to-end until the TEST value exists. |
