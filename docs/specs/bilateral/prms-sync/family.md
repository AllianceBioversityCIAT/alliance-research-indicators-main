...
| OQ-F2 | **Type coverage — REVISED 2026-09-14 (D-4 + P-1).** v1 syncs **`capacity_sharing`** and **`innovation_development`** fully, and **`policy_change`** for 2 of its 3 subtypes. Excluded, each refused up front with a stated reason: **OICR** (no PRMS type), **Knowledge Product** (STAR cannot author KPs — D-4), **Innovation Use** (D-1 + P-1, and see OQ-F11), and PRMS **policy type `1`** (D-2). See `sync-engine/homologation.md` §1.2. |
