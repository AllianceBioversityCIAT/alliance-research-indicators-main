...
| **OQ-F8** | **STAR API keys — NEW, CRITICAL PATH 2026-09-14.** Keys are issued by the PRMS team, **one per tool per environment**, and the key *is* the platform identity. STAR needs a **TEST** key (blocks the spike, which gates every field claim) and later a **PRODUCTION** key. Request via PRMS Tech Support. Nothing in this family can be verified end-to-end until the TEST key exists. |
| **OQ-F9** | **Proposed child #5 (`bilateral-project-investment`) — awaiting HITL.** The 2026-09 contract made `usd_budget` / `is_determined` mandatory per `contributing_bilateral_projects[]` entry for **Innovation Use**, and STAR stores no per-result, per-contract USD contribution (homologation **G-1**). This needs a new column **and** a client UI, so it does not fit inside the server-only `sync-engine`. Per §4's closed-set rule the row does **not** exist until approved; see [`sync-engine/proposal.md`](sync-engine/proposal.md) §12. |
