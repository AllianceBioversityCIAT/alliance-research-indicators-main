...
### T-03 — Registration surface: module, controller, service, route, env var, Swagger  `[ ]`
...
- **Falsifier:** **two, and both are required** — DC-11 is only falsifiable in both directions. (a) Remove the redaction call at `response.interceptor.ts:50` → the callback's `data.path` carries the secret and the "no segment beyond the prefix" assertion reddens. (b) Widen the redaction predicate to match **every** route → the assertion that a **non-callback** route's `data.path` is unchanged reddens. A change that passes (a) but not (b) has hidden `path` app-wide.
- **Red run:** apply mutation (a), run `npm test -- --silent`, **observe assertion (a) fail**; restore. Apply mutation (b), **observe assertion (b) fail**; restore; re-run green. Both reds recorded in `execution.md` — this row's history is why (K-004 / KZ-014: if the red has not been *seen*, it may not be asserted).
...
- **Done criteria:**
  ...
  - [ ] **Both** falsifiers **observed red**, then green
  - [ ] All five pre-existing envelope-path consumers green, under **both** `npm test` and `npm run test:e2e`
- **Effort:** M · **Depends on:** — · **Est. LOC:** ≈ 240 (≈ 60 prod, ≈ 180 test) · **Skills:** `nestjs-expert`, `error-handling-patterns`, `systematic-debugging`
