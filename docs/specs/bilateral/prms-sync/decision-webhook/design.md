...
6. **Comms:** PRMS technical team (destination registered, per environment) · the family owner (children **2, 3 and 4** gain `last_decision`) · Security (first public write endpoint). *(Corrected 2026-09-23 at T-10 — this step read "children 3 and 4" and omitted child 2. `last_decision` is additive on `GET /api/results/:result-code/prms-sync` (R-PWH-008), so every child whose surface reads that endpoint gains it. Child 2 `sync-button-ux` owns the result sidebar, and `git grep -rln "prms-sync" -- client/research-indicators/src` shows `result-sidebar.component` as the client consumer today — the same surface its still-owed synced badge lives on. `tasks.md` T-10 already said "2, 3, 4"; this line was the stale one.)*
...
| **DD-12** | **2026-09-25** | **The inbound ingest populates `status` and `reviewer_name` from the callback body, and every PRMS numeric field is read leniently (number OR canonical base-10 string).** `reviewer_role`, `science_program_code`, `changes` and `actor_user_id` stay NULL on the inbound path. | **Forced by the first real PRMS callback (observed 2026-09-24, result `11873` / official code `19949`).** Until then no task populated the six Pivot columns from a body — `execution.md` T-01b states exactly that, and T-11 covers only the outbound write — so an approved delivery landed with a NULL badge and a nameless reviewer. Three findings from the live payload: (a) **PRMS is not type-consistent with its own ids** — `result_id` arrives as the number `11873` but `data.result_code` as the **string** `"9405"`, and the number-only reader silently stored `prms_result_code = NULL` on a row that looked well-formed; the reader now accepts a canonical base-10 string and still refuses `''`, `'1e3'` and `'abc'`, which `Number()` would coerce. (b) The reviewer arrives as a top-level `reviewed_by` **object** (`first_name` + `last_name` → `"Cristian Gamboa"`); `data.reviewed_by` is PRMS's **user id** (`612`) and is deliberately not read — `reviewer_name` stores a name and no id (§4). The reviewer block is **best-effort and never a shape violation**: a delivery without one is still a valid delivery, so it cannot make a body `MALFORMED`. (c) `status` is derived from the already-verbatim `decision` (`APPROVE`→`APPROVED`, `REJECT`→`REJECTED`) and is therefore NULL on a `MALFORMED` row, which has no decision to derive it from. **Not populated, and why:** the payload carries no reviewer role and no `changes` document, and its `official_code` fields (`SP06`) are the **result's** science program, not the reviewer's — guessing either would put fabricated data in a column whose whole point is that it is filterable. |
