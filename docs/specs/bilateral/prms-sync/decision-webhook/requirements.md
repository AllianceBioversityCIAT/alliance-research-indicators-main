...
| **OQ-5** | **Who may read the history, and at what grain?** Contributor, Center Admin, PI and `SYSTEM_ADMIN` do not obviously get the same view. v1 exposes only `last_decision` on the existing per-result status endpoint; no global history endpoint is built. | Product owner | Before the UI family row |
