...
- **Done check:**
  ...
  - [ ] **Carried from T-09's audit (A-1) — delete one duplicate test.** `sp-toc-alignment-block.component.spec.ts:1080` (`'unit, target, and the contribution panel do not render …'`) **overlaps** T-02's committed AC.3 test at `:1023`. *(Corrected 2026-08-13 by independent validation: the earlier "byte-identical, merely permuted" characterisation was **wrong** — `:1080` uses `reloadedPartialDraft()` where `:1023` uses `emptyDraft({...})`, and carries an extra `sp-toc-contribution-SP01` assertion. Overlapping, not identical.)* `tasks.md` §3 assigns R-BIL-116 AC.3 to **T-02 alone**. Not gated (3 of 4 T-09 tests were genuinely new, materially unlike T-06 where 2 of 2 were fake-new) — fold the deletion into this sweep rather than spending a rework attempt.
