...
6. Collapse everything and confirm **each row's two cards are equal height again**, exactly as before the change.

> **Step 6 wording corrected 2026-07-30.** It previously read *"the **four** cards are equal height again"*, which is **literally false and would produce a false failure report.** The four ranked cards sit in a nested 2-column grid whose two rows are **separate `auto` tracks**, so cards are equal height *within* each row but not *across* rows — measured 366 / 366 / 405 / 405 in both the corrected mockup and the product. That is correct, pre-existing behaviour, not a regression. The check is per-row equality, and that the collapsed state is **identical to before the change**.

Reference: the GATE-2 mockup at `./mockup/index.html` — **corrected 2026-07-30 to model DD-14**; see the warning below for why its prior state mattered.
...
| **GATE-2** | Collapsed + expanded mockup at `./mockup/` | ✅ **CLOSED — re-closed 2026-07-30 on the corrected DD-14 mockup** | Umbrella and proposal §8 marked it *needed*: it is the input for the bounded-height decision (OQ-3), the reference for the DC-8 human check, and the layout contract Chunk B inherits. **Its first closure (2026-07-29) was unsound** — the mockup then in use could not model the defect it was used to settle, which is what produced DD-13 and forced the T-06 pivot. **Re-closed on measurement:** the mockup now models DD-14 mechanism (ii) and was verified by an independent probe across seven viewports, reading **zero delta on all four links in both directions**, while the `max-height` control reproduced the real component's **+52px / +13px** failures to the pixel. Full record: `design.md` §13 *Gates*; raw run at [`./evidence/mockup-dd14-measurements.json`](./evidence/mockup-dd14-measurements.json). ⚠️ **GATE-2 ≠ NFR-PDB-004 acceptance** — the six-step check in §7 is still **UNRUN** and NFR-PDB-004 stays **UNVERIFIED** (RB-2). |
