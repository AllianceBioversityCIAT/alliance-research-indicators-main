...
NFR-IMC-001 bounds the result at **1.5× the current p95**, which therefore requires `T_metadata ≤ 0.5 × T_existing`. That inequality is **unverified** — the baseline needs a live database (`judgment.md`), and Q1/Q2 are two multi-branch UNION aggregations over four fact tables joined to `results` through a CTE. It is plausible, not established.

**NFR-IMC-001 is consequently the binding risk of DD-11, not a footnote.** Measure it early (§10, and see the task ordering note in §11). **If the measurement breaches 1.5×, the fallback is `Promise.all` *plus* the `poolSize` prerequisite — a genuine architecture change, not a tolerance to absorb silently.**
...
- **Measure NFR-IMC-001 early.** It gates DD-11 (see §1): a breach of the 1.5× bound invalidates sequential composition and forces `Promise.all` + an explicit `poolSize` change. Sequencing that measurement late turns an architecture decision into late rework.
- **Migrations:** none.
...
| **DD-11** *(rev 4)* | 2026-07-30 | Compose the two repositories **sequentially**, not with `Promise.all`. | Peak becomes `max(8,2) = 8` — today's value — so no `poolSize` change, no shared-infra edit, no DevOps sign-off. **Rejected alternative:** racing both peaks at 10/10, monopolising the pool and forcing an infrastructure prerequisite. Peak concurrency, not total query count, is the quantity that matters. **This decision buys pool safety with latency, and the price is not small change — see the cost model below. Its acceptance is contingent on the NFR-IMC-001 measurement.** |
