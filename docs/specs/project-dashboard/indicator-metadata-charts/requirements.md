...
- **Target:** p95 **≤ 1.5×** the pre-change p95 on the largest available contract; absolute p95 ≤ 3 s.
- **How verified:** timed runs before and after against the same contract. **Inconclusive is a valid outcome** — see §9 DC-9's no-pass clause.
- **This NFR gates a design decision, so it must be measured early, not at the end.** Design DD-11 composes the new aggregation batch *sequentially* to hold pool concurrency at 8, which makes the added latency additive (`T_total = T_existing + T_metadata`) rather than overlapped. Meeting the 1.5× bound therefore requires `T_metadata ≤ 0.5 × T_existing`, which is unverified. **If the measurement breaches the bound, DD-11 is invalidated** and the fallback is parallel composition plus an explicit `poolSize` change — a change of architecture, so discovering it late is expensive.
...
| **R-1** | **Corrected twice.** The baseline is **8** concurrent queries, not 6 — `getGeoScopeReport` runs a nested `Promise.all` of 3 (`agresso-contract.repository.ts:739-743`). The design consolidates the 10 aggregations into 2 queries **and composes them sequentially** (DD-11), so **peak concurrency remains 8** and no connection-pool change is needed. Total queries issued rises 8 → 10; peak does not move. **The cost is additive latency, not a round trip:** `T_total = T_existing + T_metadata`, so NFR-IMC-001's 1.5× bound requires `T_metadata ≤ 0.5 × T_existing` — plausible but **unverified**. This makes NFR-IMC-001 the binding risk of DD-11; measure it early. If it breaches, the fallback is `Promise.all` **plus** an explicit `poolSize` change — an architecture change, not a tolerance. | NFR-IMC-001 + DC-9. |
...
| DC-3 | Gender combination arithmetic wrong — **including the asymmetric case where group-only data is dropped** | Dedicated fixtures: one mixing individual records and group sums (R-IMC-005 AC.1), and one **group-only** (zero individual records) that must still yield all three categories. A merge that subordinates group rows to individual rows passes the first fixture and fails the second |
...
| DC-10 | Swagger drift | Manual `/swagger` inspection (R-IMC-012 AC.1) |
...
**Accepted risk:** DC-8 has no mechanical gate at all. If neither the human check nor a T6 review happens, this spec ships with its dominant visual defect class unguarded — recorded here so that is a decision, not an oversight.
