...
- **Consumed by later tasks:**
  ...
  - **Open advisory for the owner's copy pass, surfaced early on purpose:** the empty-state string binds its claim to `N` (`count_results`) — **precisely the population that diverges** from the `is_primary = TRUE` aggregation — so in the all-non-primary case it is arguably false in a *second* way even though it asserts no reason. A separated form (*"No data is recorded for this field on this project. (N results.)"*) avoids that. **Decide before T-14 writes assertions against the current wording.**
...
- **⚠ HARD PREREQUISITE, found by T-10's review before this task started.** `createReportsMock` / `applyFixtureToReportsMock` (used at `project-dashboard.component.spec.ts:108-133`) **do not carry the 10 new sections.** Until they do, this task's ten per-instance DC-5 assertions — which its own acceptance box below already requires — would **bind to `undefined` and pass vacuously.** That is T-07's failure shape exactly: a green suite over nothing. **Extend the mock helpers first, then write the assertions.** This is not new scope; it is the mechanical prerequisite for a box this task already owns.
