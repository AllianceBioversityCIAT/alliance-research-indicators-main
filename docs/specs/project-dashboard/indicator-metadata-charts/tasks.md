...
- **Status:** `[~]` **PIVOT RESOLVED 2026-07-30 — NFR-IMC-001 amended, owner-approved. Two of three new bounds MET; the third is unmeasured, so this task stays `[~]` rather than `[x]`.**
  ...
  - **(c) Server-side execution ≤ 50 ms p95 per query, isolated from network** — ⬜ **NOT MEASURED.** This is the only part that says anything about the SQL rather than the link, and it **still gates T-10 … T-16** per design §11. Needs a measurement co-located with the DB, or network cost subtracted via a same-session `SELECT 1` baseline.
