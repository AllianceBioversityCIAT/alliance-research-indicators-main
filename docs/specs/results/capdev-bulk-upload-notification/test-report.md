...
| --- | --- | --- | --- |
| **R1** | `test/jest-e2e.json` has no `testTimeout`, so Jest's 5,000 ms default governs a `beforeAll` that boots `AppModule` in ~3.6 s quiet and **78.5 s under load**. Fix is one line: `"testTimeout": 120000`. | Spec owner | **Open — decision escalated.** Not applied: framework config is out of scope for `/akili-test` by rule, the flake is not caused by this spec, and the file carries T-11's load-bearing `forceExit`. |
