...
> **Re-measured over the FULL corpus by T-14 (2026-08-05) — the 277 was a sample, and the agreement is not total.** `execution.md` records the 277 figure honestly as *"measured over 400 live items (277 KP, 123 non-KP), 8 pages"*, but this paragraph and `tasks.md` T-14 both carried it forward as **the** baseline with the sampling caveat dropped — so "277/277" read as a population fact. The full searcher corpus, fetched with the sync's own parameters, is **5,180 items · 2,388 KP · 2,387 carrying a handle-format `knowledge_product_summary.handle`**. Agreement against the stored side is **2,386 / 2,387 (99.96%)**, not 100%.
>
> **The one divergence is real and it is recorded here rather than smoothed away.** PRMS result `23607` (official code **7232**, year 2023, KP, active) stores evidence handle `hdl.handle.net/10568/**131655**` while the live payload supplies `hdl.handle.net/10568/**131889**`. Both handles exist as live TIP rows — TIP `29293` (code 27262) and TIP `29307` (code 27276) — carrying the *same* title as the PRMS row. So the **sweep and the sync path would place this PRMS row in two different groups**: the sweep matches it to TIP 29293 on the stored handle, the sync path matches it to TIP 29307 on the payload handle. TIP wins under RULE_1 either way, so this is not a path to deleting a distinct publication, but the audit record would name a counterpart the other path disagrees with, and DC-2's post-run check keys on the group's normalized link. **This is an owner decision before `apply`, not a defect the build can close.**
...
- **`rowsRefusedMultiIdentity` on the plan is non-zero on dev** → expected to be exactly 0 (D-dup-20); a non-zero count means live data has moved into the shape the branch refuses. **Renamed during T-15 (2026-08-05), and the old name was unreadable:** this tripwire said "`UNRESOLVED_CONFLICT` from the multi-group branch", but `byClassification` is keyed by **group** classification while AC.8's refusal is per-**result**, so the multi-identity branch could never raise that count — it was **zero by construction**, which is DC-7's pathology aimed at the one branch standing between an ambiguous identity and an irreversible delete. Folding a per-result refusal into `byClassification` was rejected as the fix because the map would stop summing to `groupCount` and would corrupt the neighbouring `CROSS_YEAR_REVIEW`/`SAME_SYSTEM_IGNORED` counts this same section relies on. The group's own classification correctly stays `RESOLVED`. **Read the count as "refusals exercised", not "multi-identity rows present"** — an ambiguous row inside a `CROSS_YEAR_REVIEW` or `SAME_SYSTEM_IGNORED` group has empty `losers`, so nothing is refused and this reads 0 while the row exists. The population question is the separate tripwire below and belongs to T-14.
