...
| `ON DELETE CASCADE` | **1** |
| `full_delete_result_version` DELETE targets | **35** |
| Body length (bytes) | 7325 |
| **Uncovered, `NO ACTION`** (raise errno 1451 → T-02 must add) | **7** |
...
1. `bulk_upload_results` — column `result_id`
2. `result_cap_sharing_ip` — column `result_cap_sharing_ip_id`
3. `result_pool_funding_alignment` — column `result_id`
4. `result_pool_funding_indicator_mapping` — column `result_id`
5. `result_pool_funding_toc_alignment` — column `result_id`
6. `result_review_history` — column `result_id`
7. `temp_result_ai` — column `result_id`
