...
- **Left on disk, untracked and unreviewed:** `test/duplicate-resolution.e2e-spec.ts` (590 lines) + `test/support/duplicate-resolution-seed.util.ts`, authoring three rolled-back-transaction cases. No production source touched. **`npm run test:e2e` was never run, the seeded table count was never checked against T-01's enumeration, and the Reviewer never audited it** — so none of the three ACs are met and this code is scaffolding, not proof. Full detail in [`./execution.md`](./execution.md).
