...
- **Acceptance / done check:**
  - [x] Mixed-year batch: each item holds exactly its own ids, at the role its own portfolio dictates.
  - [x] Reversing item order yields the identical per-item outcome.
  - [x] No item's levers are attached to another item's result id.
  - [x] After an item fails mid-way, no `result_levers` row survives for it, and later items resolve on their own year.
  - [x] The three-field item ends with every `result_sdgs`, ALIGNMENT `result_contracts`, `result_strategic_objectives` **and** new lever row `is_active = true`.
  - [x] Reversing the two alignment steps changes nothing observable (R-RES-010's `AND IT MUST`) — **MODELLED, NOT BEHAVIOURAL.** The reversed arm invokes the two narrow-save doubles directly and **exercises no production code**, so it has *zero detection power over a production defect*; its only reddening mutant is a mutation of the test's own double. Accepted on the Reviewer's ruling because the clause is **not behaviourally provable at the seam T-06 was scoped to** — `design.md` §5.4 fixes production's call order and T-06 may write no production code. The substance is carried by production-exercising tests elsewhere (`portfolio-2-alignment.handler.spec.ts:699-745` and `:325-494`: each narrow save calls `create` exactly once with exactly its own rows at its own role, and neither touches the other's table — disjoint effects ⇒ order-independence). **Behavioural closure would be a task-scope amendment (a handler-seam composition test), not rework.** Recorded on the register rather than laundered as an ordinary green — the treatment `R-RES-007` AC.2's residual already gets.
  - [x] The alignment step is reached only for a result created in the same call (R-RES-007 AC.1) — discharged by a **source-text assertion** over `formalizeResult`'s body, which the AC's own wording (*"asserted against the code path, not inferred"*) calls for: it enumerates **every** `saveLeversForPortfolio` call site via `matchAll` and pins the first argument to `newResult.result_id`, which a behavioural test cannot do (a mock shows one execution's id *happened* to match). Premises verified at source (`results.service.ts:899`, `:900`, `:1027-1028`). Brittleness to renames is cost, not invalidity — it fails **loud**, never silently green — and the behavioural half is pinned separately in the same file.
  - [x] **The suite FAILs when run against an implementation that resolves the portfolio once per request** — staged using the second form the task names (the `findByYear` double returning its first answer for every subsequent year); **all three routing tests observed red.** The probe exposed a real gap in the Implementer's own first draft: the orchestrator double **ignores `_portfolioId` by construction**, so row-content assertions are portfolio-blind and every written row is byte-identical under the mutant. Only the added `toHaveBeenCalledWith(resultId, portfolio, ids)` pin can see it.
...
- **Estimated effort:** M (run at `xhigh`) · **Status:** **done** — PASS on attempt 1, unanimous across a two-lens parallel review (G routing/leakage/mid-batch · H techniques/inertness/`Not Done`). **No production code**; 610 lines, **zero deletions**. Two unusual techniques adjudicated: the `R-RES-007` AC.1 source-text assertion **discharged**; the `R-RES-010` reversed-order arm accepted as **MODELLED, not behavioural** — see the qualifier on that check and `execution.md` → T-06
...
- **Acceptance / done check:**
  ...
  - [x] Migration `1783029013035` confirmed applied (RK-6 closed) — **or the rollout is blocked and escalated**. — **CLOSED for `alliancereportingdb` (owner-run 2026-09-04).** TypeORM reports *"No migrations are pending"*; the migration file is present in the repo among 306, with later timestamps also not pending, so it is necessarily applied. The §11 FK-1451 rollout hazard is retired for that environment. **Deliberately not inferred** from T-06's green rollback suite (which models *post*-migration semantics) nor from Lens D having read the migration's *body* — content and deployment are separate claims, each now with its own evidence. ⚠️ **RB-3 remains partially open:** its wording is *"every target environment"*, and this covers one database.
...
| RB-3 | 2026-09-04 | Migration `1783029013035` not applied in a target environment | **PARTIALLY CLOSED 2026-09-04 (owner-run).** TypeORM reports *"No migrations are pending"* against `alliancereportingdb`, and the migration file is present in the repo — so it **is** applied there, and the §11 FK-1451 rollout hazard is retired for that environment. **Still owed: confirmation for every *other* target environment** (Production, if a separate schema) | D. Casañas | **partially closed — Prod unconfirmed** |
...
| RB-5 | 2026-09-04 | ~~**Q-6 unanswered** — DD-10 widens scope by one expression~~ | **CLOSED 2026-09-04.** Accepted, then amended post-execution: the expression is behaviour-neutral and kept for type honesty | D. Casañas | **closed** |
...
| RB-9 | 2026-09-04 | **The mixed-year suite's sharpest discriminator is undocumented.** The reversed fixture's `2027 → PORTFOLIO_1` deliberately contradicts `requirements.md` §5's documented P2 range (2026–2030), and is the **only** entry where a table-driven answer and a hardcoded `year >= 2026` threshold disagree — i.e. the only thing making a threshold router (a DD-2 violation) go red. A maintainer will read it as a typo and silently delete the discriminator | One comment line in `results.service.spec.ts`. **Not actioned in T-06** — an advisory may not widen a task; flagged for the owner | D. Casañas | **open** |
| RB-8 | 2026-09-04 | **Discovered, out of scope — `PATCH .../alignments` wipes a whole role when its key is absent.** `create` is invoked unconditionally, and `formatDataToArray` maps `undefined` and `[]` alike to `[]`, so an absent `research_areas` / `strategic_objectives` / `impact_outcomes` deactivates every active row for that role | **None in this spec.** DD-10 does not close it (behaviour-neutral). Recorded by owner decision 2026-09-04 and routed to its own proposal; no task minted here | D. Casañas | **open — out of scope** |
