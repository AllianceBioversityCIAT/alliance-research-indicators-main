...
| **R-5** | `payload?.research_areas?.map(...)` is unguarded (`portfolio-2-alignment.handler.ts:61`) — the same class of bug the predecessor's R-RES-010 hardened for `strategic_objectives` and `impact_outcomes`, which left `research_areas` out | Low; pre-existing, not caused here | **Addressed 2026-09-04 as DD-10, then amended after execution:** the alignment is **behaviour-neutral** (`formatDataToArray` maps `undefined` and `[]` alike to `[]`), so it buys type honesty and sibling consistency, not wipe prevention. The real hazard is pre-existing, sits in `PATCH .../alignments`, and is recorded as `tasks.md` **RB-8** for its own proposal |
