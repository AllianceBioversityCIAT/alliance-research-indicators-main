...
- **Done:** [ ] methods compile + typed; [ ] `by-delegate` uses `?delegate_user_id=`; [ ] DELETE carries a body (2 shapes).
...
- **Done:** [ ] after a write, state comes from a refetch (not an optimistic divergent copy); [ ] loading/error modeled.
- **Dep:** T-UI-01 · **Effort:** M · **Skills:** angular-developer
...
- **Done:** [ ] entry renders + routes; [ ] no existing nav item renamed/removed.
- **Dep:** T-UI-04 · **Effort:** S · **Skills:** angular-developer
...
- **Done:** [ ] opens on By-project; [ ] counters correct + reactive; [ ] no hex.
- **Dep:** T-UI-02 · **Effort:** M · **Skills:** angular-developer, ui-ux-pro-max
...
- **Done:** [ ] table renders enriched fields; [ ] row X revokes + refetches; [ ] search filters; [ ] no-colour-only cue.
- **Dep:** T-UI-04 · **Effort:** M · **Skills:** angular-developer, ui-ux-pro-max
...
- **Done:** [ ] renders person→projects; [ ] row X revokes; [ ] no unmanaged projects shown.
- **Dep:** T-UI-04 · **Effort:** M · **Skills:** angular-developer, ui-ux-pro-max
...
- **Done:** [ ] reuses `app-modal` (no bespoke modal); [ ] Accept gated; [ ] pre-load prevents accidental revoke; [ ] confirm shows add+remove delta; [ ] POST sends per-project full lists.
- **Dep:** T-UI-02, T-UI-08 · **Effort:** XHIGH · **Skills:** angular-developer, ui-ux-pro-max
...
- **Done:** [ ] multiselects render from the stub; [ ] one documented swap point; [ ] no component depends on stub-specific shape.
- **Effort:** S · **Skills:** angular-developer
...
- **Done:** [ ] `npm run test:coverage` green + floors met (40/20/45/30); [ ] `npx tsc -p tsconfig.spec.json --noEmit` clean.
- **Dep:** T-UI-05, T-UI-06, T-UI-07 · **Effort:** M · **Skills:** angular-developer, tdd
...
## 6. Done definition
- [ ] T-UI-01…09 done.
- [ ] Two tabs (By-project default + By-person), enriched tables, per-row "X" revoke, search by person/project.
- [ ] Assign/Edit reuses `app-modal` + two `app-multiselect`s; SYNC-aware with pre-load + add/remove delta confirm; Accept gated.
- [ ] STAR tokens only, no hex; non-colour a11y cues; loading/empty/error/success states.
- [ ] Single signals cache feeds all views (no drift). Coverage floors met.
- [ ] Picker sources stubbed behind one swap point (user endpoints pending).
