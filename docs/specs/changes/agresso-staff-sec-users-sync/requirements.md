...
| ~~**RSK-9**~~ | ~~The Agresso `status` field is never branched on~~ | **CLOSED BY CONSTRUCTION 2026-09-15** | The feared scenario — *"if the employees endpoint returns terminated or on-leave staff, this spec creates accounts and grants `CONTRIBUTOR` to departed people, and `R-AGS-007` reactivates them"* — is now unreachable: the fetch itself is filtered with Agresso's documented **`?status=active`**, so departed staff never enter the payload. Measurement that made the old mitigation moot: `status` holds only `{'N', NULL}` in `alliance_user_staff`, i.e. it carries no signal to branch on. ⚠️ **The parameter lives in `private query()`, which BOTH `findNumberOfPages()` and the page loop call** — adding it at only one site desynchronises pagination (the count would report every employee while the pages return only actives). Asserted by two unit gates, observed red when the parameter is removed. *(The `M-6` correction stands: the field IS read and indexed by OpenSearch; what was never true is that any business logic branches on it.)* |
...
| ~~**OQ-8**~~ | ~~Should the Agresso `status` field gate account creation and reactivation?~~ | — | ✅ **RESOLVED 2026-09-15 — and dissolved rather than answered.** The pre-flight finally ran: `SELECT DISTINCT status FROM alliance_user_staff` returns **`{'N', NULL}`** — two values, neither of which distinguishes an active employee from a departed one, so the column **could never have gated anything**. The risk is instead closed **upstream**: Agresso's documented **`?status=active`** query parameter was added to `agresso-staff-tools.service.ts`'s `private query()`, so the payload now contains only active staff. There is nothing departed left to provision or reactivate. *(Superseded default, for the record: "Not in v1; a pre-flight is owed before Prod.")* |
