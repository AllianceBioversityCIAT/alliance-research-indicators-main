...
/ ~~~24~~ **~37** review rounds** — the round figure **re-baselined 2026-09-07 at the T-10 gate by user ruling** (`RB-9`; derivation and single home in `design.md` §11). The task and LOC figures are unchanged. A further overrun escalates.
...
### `RB-9` — review-round budget re-baselined to ~37 (user ruling, 2026-09-07, at the T-10 gate)

**Trigger:** the Step 2.4 tripwire fired exactly at the boundary — **24 of ~24 rounds consumed with 7
of 20 tasks remaining.** `execution.md`'s own clause said *"a further overrun escalates"*, so this was
the escalation, not a warning.

**Derived from measurement, not re-estimated.** 24 rounds / 13 completed tasks = **1.85 rounds per
task**. Of the seven remaining, **T-20 is a human migration step that consumes no review round**, so
6 × 1.85 ≈ 11 more → **~35**, rounded to **~37** for margin on T-13 (the heaviest remaining task, and
the one carrying `R-IUR-014` AC.4b's owed discriminating red). Against the original ~24 that is
**+54%**; task count and LOC are **unchanged** and remain on plan.

**Single home (`KZ-005`):** `design.md` §11's budget table. `tasks.md` §1.1 and this file's §Budget
now **link** there instead of restating the number. Historical mentions of `~24` survive in dated
entries in this append-only log, in `tasks.md`'s account of `RB-8`, and in `judgment.md`'s `P-8` row —
all point-in-time records of what was true when written, deliberately not rewritten. A **backward
sweep confirmed no site still asserts `~24` as the current budget.**

**What the measurement actually shows, recorded because the original estimate was not simply low.**
Zero rounds were spent on production-logic defects. Every rework in this spec landed in the in-code
claim layer, and three of those were generated by the Leader's own briefs. The ADVISORY policy adopted
today addresses the largest cause; T-09 (2 rounds) and T-10 (3, one of which delivered a
user-authorized bug fix) are the first data under it. The re-baseline therefore prices the *remaining*
work honestly rather than blessing the historical rate as inevitable.

---

## Pivot Record: T-13 — the save-blocking gate is withdrawn; report instead of block (2026-09-07)

**Status: T-13 `[~]`. Rework loop stopped, not exhausted** — attempt 1 was built and reviewed
(`STATUS: FAIL`, one `K-004` issue on unobserved mutations, fully remediable). It is superseded, not
failed. **No code has been changed in response to this pivot; documents first, per the protocol.**

### What triggered it

The T-13 Reviewer's escalation, not a defect. It constructed a fully reachable sequence showing that
`DD-8`'s save block, combined with the **pre-existing unconditional** `if (page) this.navigateTo(page)`,
converts a **partial** data loss into a **total** one:

| Step | Before T-13 | After T-13 |
| --- | --- | --- |
| Row 1 saved (type + counts); user sets level 4→7, writes the justification; adds a row, types counts, forgets the type; clicks **Next** | `PATCH` fires — **level 7 and the justification persist**; only the new row is dropped | **No `PATCH` at all** — level, justification **and** the row are lost on component destruction |

The toast named one row while everything else the user typed was also discarded, and pointed at a card
the user had already navigated away from. `DD-8`, §8 and `tasks.md` §T-13 are **silent on navigation**,
and §1's non-goal (*"Making the save gate stricter than `DD-8` requires"*) argued against the
Implementer resolving it alone. Escalated to the user rather than filed as an advisory (`KZ-008` — this
spec family has already lost a silent-data-destruction finding into an advisory register once).

### The user's ruling, and the premise the Leader had to correct first

The user's direction: **prevent duplicate actor types at source by making an already-used type
un-selectable, and stop blocking the save for anything else** — *"el usuario podría seguir editándolo
más adelante, entonces no tiene sentido bloquear al usuario con que todo tiene que estar completo."*

**The Leader verified that premise against the code and it did not hold as stated.** `saveData()` calls
`await this.getData()` on success, and `getData()` does `this.body.set(...)` from the server response.
So a dropped row does not survive as an editable draft — **it disappears from the screen**, and the
user would have to re-add the row and retype the counts. Reported before the decision, not after.

**Ruling taken (user, informed):** **save everything, and tell the user what was not saved.** No
gate. Plus the dropdown rule, with `OTHER` exempt.

### Why this is coherent rather than a retreat

- **`R-IUR-014`'s headline sentence survives untouched** — *"The system SHALL NOT discard user-entered
  data on save without telling the user."* It never said *block*; blocking was `DD-8`'s chosen
  mechanism. **Telling** satisfies the requirement directly. Its scenarios and ACs change; its intent
  does not.
- It **resolves the navigation trap by removing its cause.** Nothing blocks, so no block-plus-navigate
  path exists. The Reviewer's escalation is discharged rather than deferred.
- It **restores `R-IUD-001`** (a permissive draft save) and **un-narrows `NFR-IUR-003`**, whose
  *"one deliberate, named exception"* was precisely `DD-8`'s client-side block. That narrowing —
  added at Judgment Day round 2 as `S-6` — is no longer needed. The requirement gets **simpler**.
- **`P-3` already withdrew site 2** for the same class of reason: a gate the user could not escape.
  This withdraws site 1 on the same principle, one level up.
- **`RSK-1` already accepts the consequence.** With `R-IUR-011` removing the zero-actor backstop, a
  dropped row can leave a result with no content that is nonetheless submittable. That risk is
  **already accepted by user decision `D-1`** and carries an open MEL sign-off (`RB-1`). No new risk
  is introduced by this pivot.

### The duplicate-actor rule — Leader's reading was WRONG; corrected by the user

The Leader read *"todo lo demás"* as the incompleteness rules only, and proposed **keeping** the
duplicate-actor save block as a backstop for pre-existing duplicate data. **The user rejected that
reading** (2026-09-07): *"lo de actores duplicados lo que debe pasar para evitar actores duplicados no
es bloquear el save sino en el drop down de actores si ya se seleccionó ese actor entonces en el nuevo
chip de actores ese actor debe salir inseleccionable, como desactivado del listado del dropdown."*

**The user is right and the Leader's backstop argument was weak.** Prevention at source plus
server-side authority is the correct split: the client prevents for UX, the server validates for
correctness. The client block existed only because the prevention did not. And the risk the Leader
raised barely exists — **if the server rejects duplicates, they cannot have entered through the API**,
only by a direct database write, and that case is already handled by the existing server-error toast
(`extractErrorMessages` + error toast).

**Ruling: `hasDuplicateActorType()` stops gating `saveData` entirely. NOTHING blocks the save.**

### Two implementation details the Leader is deciding, with reasons

Neither is a direction the user needs to set, but both are traps if left implicit:

1. **`OTHER` stays selectable, always.** Duplicates there are keyed on `type + trimmed lowercase
   custom name`, so several `OTHER` rows with different names are legitimate. Disabling it after the
   first use would break a valid case.
2. **The disable is per-row.** A row's own currently-selected type must stay visible and selectable
   in **its own** dropdown — otherwise the row would render its own value as disabled.

### Revised technical direction

| Item | Change |
| --- | --- |
| **`R-IUR-014`** | Headline kept. Both scenarios rewritten: the save **proceeds**; the user is **told** which rows were not saved and why. AC.1/AC.2 change from *"save blocked, user informed"* to *"save proceeds, row dropped, user informed"*. AC.6's falsifying input keeps its shape but inverts its expectation. **AC.4b becomes trivially satisfied** (no gate exists anywhere) and its co-assignment to T-10+T-13 collapses |
| **`DD-8`** | *"Block the save only when it would destroy typed data"* → **"Never block the save; report what was dropped."** Site 1 withdrawn on `P-3`'s principle; site 2 already withdrawn. The decision table becomes a **reporting** table |
| **`NFR-IUR-003`** | The `S-6` narrowing is **removed**; the requirement returns to *"every rule gates submission, not saving"*, with no exception. Its two-part verification collapses to one part |
| **`R-IUR-017`** *(new)* | An actor type already used by another row is **not selectable**, `OTHER` exempt (duplicates there are keyed on `type + trimmed lowercase custom name`, so multiple `OTHER` rows are legitimate) |
| **`T-13`** | Rewritten: remove the gate, keep and re-scope the message. Most of attempt 1's predicates (`actorRowWouldLoseData`, `organizationRowWouldLoseData`, `blockedSaveRowMessages`) **survive** — they compute *what would be dropped*, which is exactly what the message needs. `saveData`'s `if/else` inverts |
| **`T-21`** *(new)* | `R-IUR-017` in `innovation-use-actor-item` — the parent passes the set of used types; the card disables those options; `OTHER` always selectable |
| **Budget** | Tasks **20 → 21**. Third declared breach. Rounds already re-baselined to ~37 (`RB-9`); T-21 is small and T-13's rewrite reuses attempt 1's work, so no further round re-baseline is proposed |

### What the Leader will NOT do without a further ruling
