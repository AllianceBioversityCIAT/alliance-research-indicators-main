...
- **Verify:** `npm test -- --silent -- innovation-use-organization-item.component.spec`. `:302-316` must be **rewritten, not deleted** — it breaks identically under suppression and removal.
...
- **Verify:** `npm test -- --silent -- innovation-use-organization-item.component.spec`.
...
- **Implementation notes / the three gates:**
  ...
  | 3 | **Human browser check**, light theme, real browser | Every new border confirmed **painted**. Record **which fields were checked**, in words (`KZ-002`) — a human "looks fine" that covers an adjacent property is not evidence for this clause. Include T-02's `text-sm`/`fs-[14]` line-height decision |

- **Verify:** all three, with gate 2's two error sets attached and gate 3's field list quoted.
...
- [ ] **T-16 gate 3** — human browser check performed, with the checked fields quoted in words.
