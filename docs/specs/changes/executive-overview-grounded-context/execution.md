...
**Owner decisions (2026-08-25, hero pass):** three connected findings from owner screenshots — dashboard-tab hero dead space, hero height JUMP on tab switch (results tab reveals the meta grid), and tab controls lacking affordance. Direction chosen: **stable identity hero** (identical both tabs; contacts horizontal; meta grid relocates into the Results tab body — each tab owns its context, Act 1 keeps the dashboard's) + **improved underline tabs** (icons, hit area, hover pill, focus ring, tablist aria). Spec amended: R-EOC-010/011 + T-09 (depends on T-08, done).

---

## T-09 — Stable identity hero + tab affordance — PASS

- **Date:** 2026-08-25 · **Status:** PASS · **Attempts:** 1 · **Covers:** R-EOC-010 (AC.1–4), R-EOC-011 (AC.1–4)
- **Implementer (sonnet, medium):** HTML-only + spec (component .ts untouched — all bindings pre-existed). Hero: zero route-keyed conditionals; aside column + divider removed; contacts horizontal wrap row (initials + name · role). Both `<dl>` grids (8 fields intact) relocated below the tab bar under the same guard. Tabs: icons, min-h-44px (real improvement — old was ~38px), cursor-pointer, hover pill, focus-visible ring, tablist/tab + aria-selected; routing untouched. K-004: 4 DOM tests observed RED against old markup (verbatim in report), green after. Route-scoped 352/352.
- **Reviewer (opus): STATUS: PASS on all 8 ACs.** Extra verifications: the kept `!== 'project-dashboard'` guard is EXACTLY equivalent (two children; `project-results` is a redirect shim — no third value); all 4 newly-used tokens defined in both theme blocks; no coverage lost in the replaced describe (legacy R-HL-001/D-F1-1/RC-4 behaviors still guarded, incl. document-order via compareDocumentPosition); native buttons = keyboard order.
- **Leader re-measurement (clean scope, chart-explainer excluded):** 320 suites / 6869 PASS.
- **Concurrent-session ledger:** foreign session committed `5fcc730b` (chart-explainers T-01) to this branch — cleanly scoped, none of our files swept. It now edits `project-dashboard.component.*` and `project-dashboard-card.*` in the working tree. Our T-09 commit is pathspec-scoped to `project-detail.component.{html,spec.ts}` — disjoint. ng lint project-wide red on THEIR viz-chart unused imports at measurement time.
- **Advisories (recorded, never gate):** (a) a11y — ARIA tabs pattern incomplete (no aria-controls/tabpanel/arrow-key roving); (b) inactive tab grey-600 on hover pill grey-100 ≈2.9:1 — PRE-EXISTING deficit now on a designed background; joins the R-EOC-008 AC.2 contrast ledger for T-07; (c) dead `min-w-0 flex-1` on header; (d) `truncate` on inline span is a no-op (tooltips still work); (e) replaced describe dropped legacy traceability IDs from its title; (f) "no height jump" proven structurally (innerHTML equality — sound given zero route-keyed bindings), not by rendered-height measurement.
