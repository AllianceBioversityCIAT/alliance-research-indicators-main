...
- **Migration order:** the new migration must be applied (Dev → Staging → Prod, per the existing CI/CD pipeline) before or alongside the client/server code deploy — but note root `CLAUDE.md` §4.3 (K-015): **the pipeline deploys code, not migrations.** Whoever merges this must explicitly confirm the migration was applied in each environment before relying on R-ALN-003's behavior there; until applied, R-ALN-001/002 (display + persistence) work but R-ALN-003 (enforcement) silently does not.
