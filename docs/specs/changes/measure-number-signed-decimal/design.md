...
SERVER  server/researchindicators/src/
  ...
  .../result-quantifications/result-quantifications.service.ts
        override createCustomValidation — per-role rule map + default    DD-13
        (base-service.ts:134,345 call it on BOTH upsert paths)
...
| Unit specs (`input.component.spec.ts`, `quantification-item.component.spec.ts`, `oicr-details.component.spec.ts`, `innovation-use-details.component.spec.ts`, the IU DTO spec, `result-oicr.service.spec.ts`) | own `DC-1`, `-2`, `-3`, `-6`, `-9`, `-10`, `-15`. Listed because §2.1 claims "every new file" and round 1 omitted them entirely (`K-17`) | modified |
...
| `.../result-quantifications/result-quantifications.service.ts` | override `createCustomValidation` with the per-role rule map (DD-13). **No OICR file is touched at all** | modified |
...
| --- | --- |
| `:164` must NOT round, clamp, or drop the sign | DD-5 (scale 4, symmetric derived bounds) |
| `:165` AND IT MUST treat `0` as a value | **DD-2's null contract** — `null` maps to `null`, never `0` |
| `:172` must NOT stop at `0` | DD-5 — `min` is negative |
| `:173` AND IT MUST step by a whole unit | DD-6 — PrimeNG's `step` default of `1`, no binding |
...
| `:203` AND IT MUST hold for **both** OICR blocks | Holds **by construction** — both render the same card and both pass nothing. Asserted once on the card's default and once per block on the rendered value |
| `:236` must NOT `400` — the value originates from the read path | DD-2 + DD-15 |
| `:237` AND IT MUST use a value from a real read, never a literal (**K-012**) | **DD-19** |
| `:245` must NOT name `quantification_number` in the `400` | DD-8 / DD-17 — one field |
| `:246` AND IT MUST keep rejecting on the **six** count fields | DD-8 keeps all six; DD-13 covers OICR's two |
| `:277` must NOT be `3`, `2`, or `2.5000` re-read differently | DD-1 (scale 4) + DD-2 |
| `:278` AND IT MUST be proven at the fixture tier | DD-9 |
| `:307` must NOT be evidenced by a routine body diff | DD-9 — "bodies unchanged" is a separate assertion |
| `:308` AND IT MUST read both rows out of MySQL | DD-9 + **DD-20**'s four-column match |
| `:341` must NOT be fixed by re-uniting the guard's threshold | **DD-7 withdrawn** — the guard is not touched at all, so neither re-uniting nor removing occurs. `R-MSD-006` is amended accordingly |
| `:342` AND IT MUST instead be prevented at the control | ⚠️ **Partly false and amended.** `L-07` established that PrimeNG checks `max` only on blur/Tab/Enter/spinner and then **silently clamps** — the per-keystroke path never checks it. So `max` is a *clamp*, not a prevention. `maxFractionDigits` **is** enforced per keystroke. `R-MSD-006` is amended to claim only what the control does |
| `:409` must NOT render `-0.7500`, `NaN`, `0`, or empty | DD-2 + DD-3 |
| `:410` AND IT MUST behave identically for `string` or `number` | DD-3 — the coercion is idempotent |
| `:440` must NOT be asserted from reasoning about `DECIMAL` formatting | **DD-11** — automated fixture |
| `:441` AND IT MUST be observed from an executed query | DD-11. **Still unexecuted — §17 `U-1`** |
| `:442` AND IT MUST include the `-10.0000` case | DD-10's seven cases |
| **`:472`** must NOT be silently rounded to `3` and stored | **DD-13** — service-layer rejection before the upsert |
| **`:473`** AND IT MUST be announced to consumers before it ships | **DD-13** + §11 Comms + `NFR-MSD-005` |
| **`:521`** must NOT deactivate either row or insert a duplicate | **DD-2** (§5.3) + DD-13 |
| **`:522`** AND IT MUST be seeded from a real read, never literals | **DD-19**, extended to the OICR-path fixture (`K-17`) |
...
| Where the rule lives | `ResultQuantificationsService.createCustomValidation` — **below** this endpoint and below the Innovation Use one, on the path both traverse (DD-13) |
...
**The client suite runs in full, never targeted** (**KZ-003**, `innovation-use` lineage; family `FR-4`) — the shared card renders on every OICR details page, and this change now edits OICR deliberately.
...
| Step | Detail |
| --- | --- |
| 1 | **Client + server code merges first.** Behaviour does not change yet — the DTO relaxation cannot produce a decimal while the column still rounds |
...
| Backout | code rollback + `migration:revert` **plus restore from the backup table** (DD-18). A bare revert rounds fractions and can **fail** on a wide value |
...
| **DD-8** | Relax `quantification_number`'s validation, **this field only**; the **six** sibling count fields keep their decorators | The superseded draft said "seven" — the DTO has seven `@Min(0)` total, one of which is this field (`J-05`). Requirements' own contradiction (six / seven / eight) is swept in `S-10` |
...
| **DD-12** | **The shared card's `maxFractionDigits` default becomes `0`. `oicr-details.component.html` is NOT touched.** | **Rewritten at the product owner's additive-defaults ruling.** Round 1 pinned OICR's two call sites; round 2's judges then spent three separate findings on *which* call sites pass *what* (`J-21` → `C-1` → `L-05`, three wrong figures for one number). **Changing the default deletes the question.** OICR passes nothing and receives `0`, so its template is not edited, its behaviour is fixed **by construction**, and no enumeration of call sites is load-bearing anywhere. **This is the one default that changes value** (`undefined` → `0`), and it is safe in both readings of the contested `U-4`: if the product owner's live test is right (decimals already refused) it *matches* observed behaviour; if the judges' source reading is right (PrimeNG resolves 3 digits) it *tightens* an unintended permissiveness toward what every other consumer already asserts. Either way OICR ends stricter or identical, never looser. `min` and `max` keep today's values as defaults |
| **DD-13** | **Override `createCustomValidation` on `ResultQuantificationsService`, with a per-role rule map and a default entry. `result-oicr.service.ts`, `update-oicr.dto.ts` and `result-oicr.controller.ts` are NOT touched.** | **Rewritten at the product owner's additive-defaults ruling — the third version of this decision, and the first that touches no OICR file.** Round 1 added a `ValidationPipe` (`K-01`: would delete ~14 undecorated properties and deactivate every tag, link and impact-area row on the first save). Round 2 moved the rule into `ResultOicrService` — correct but still an edit to OICR's own code, which left `R-MSD-011` true only **by enumeration** of today's writers. `base-service.ts:278-283` exposes `protected async createCustomValidation(dataArray)` — *"Override this method to add custom validation"* — and the base calls it at **`:134` and `:345`**, i.e. on **both** upsert paths, inside `ResultQuantificationsService`, the one service every writer traverses. Nobody overrides it today. **Rule map:** the **default** entry (roles 1, 2, and any future role) is *non-negative integer* — today's effective behaviour, previously enforced only by the `bigint` type `DD-1` removes; role **3** opts in to *signed, ≤ 4 decimals, bounded*. Consequences: `R-MSD-011` becomes true **by construction** — the check sits below every writer, so a future caller cannot bypass it — a new role is one map entry, and `K-01`/`K-04` are closed permanently because no pipe, no DTO decorator and no nested DTO exist to get wrong |
...
| **DD-16** | **WITHDRAWN. No scale message is added; the local rule is prevention, and the digit guard is REMOVED** | **Reversed at the round-2 re-judgment (`K-10`, `K-09`).** Round 1 claimed paste was the reachable route for an over-scale value. It is not: `p-inputNumber`'s paste path runs `Intl.NumberFormat`, which **silently rounds** — executed, `format(2.55555)` at `maximumFractionDigits: 4` → `"2.5556"` — and `app-input`'s own paste handler returns unless `type === 'text'`. So no client state exists in which an over-scale value is present to report, and a message for it would be dead UI. Separately (`K-09`), DD-7's digit guard becomes **unreachable at every call site** once DD-14 caps every configuration below 18 digits: counting digits kills the true positive along with the false one. **So the guard is removed, not re-united** — a gate that can no longer fire is dead code that a future reader will trust. The server's `400` stays the authority |
...
| Signal | R0 | R1 | R2 | **Now (additive-defaults)** | Why it fell |
| --- | --- | --- | --- | --- | --- |
| Tasks | 11 | 14 | 15 | **11** | `DD-7` withdrawn (−1), `DD-12`'s OICR-template task gone (−1), `DD-13` collapses from *DTO + controller + service* to **one method override** (−2) |
| LOC | ≈ 700 | ≈ 1,410 | ≈ 2,400 | **≈ 1,500** (≈ 330 production, ≈ 1,170 test/fixture) | production drops ~37%: no OICR files, no `app-input` guard surgery. Test stays high — the fixture tier is where this spec's real risk lives, and R2's ≈1,880 was calibrated against measured family actuals |
| Review rounds | 16 | 25 | 30 | **≈ 22** | 11 × 2.0, holding chunk 2's measured rate (the only comparable that also carried a migration) |
...
**The additive-defaults ruling is why this section moves down instead of up for the first time.** Three of the four accepted defects (`L-02`, `L-03`, `L-04`) and two chronic findings (`L-05`, `L-09`) were not fixed — they were made **unreachable**, by removing the edits that produced them. Fewer files stating a decision is fewer places for propagation to fail, which is the root cause 78 findings kept pointing at.

**≈ 1,500 LOC still wants two PRs**, not three: (1) server — the entity transformer, the shared validator override, both migrations; (2) client — the card's inputs and defaults, the Innovation Use call site, the read/write type reconciliation. The client depends on (1)'s transformer existing.
...
| **U-4** | PrimeNG's effective fraction limit when `maxFractionDigits` is `undefined` | **Contested** — judges from source say 3, product owner from the running app says decimals are refused. DD-12 makes it moot by pinning, which is why the contradiction is recorded rather than resolved |
...
| **U-11** | `DD-12`'s claim that defaulting `maxFractionDigits` to `0` leaves OICR *identical or stricter* | Rests on `U-4`, which is contested and unresolvable without a browser. The **direction** is safe in both readings; the claim that nothing observable changes is not verified |
...
| **U-12** | That `ResultQuantificationsService` is the only path reaching `result_quantifications`, making DD-13 structural | **Verified by two judges** against the current tree (no OICR creation path, no AI/import path, no cascade, an empty controller). It is a property of **today's** code; `createCustomValidation` sitting below every writer is what makes it hold for future ones |
