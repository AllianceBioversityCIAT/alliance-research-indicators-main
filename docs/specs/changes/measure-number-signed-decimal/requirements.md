...
> | A `ValidationPipe` (`K-01`: data destruction), then OICR's own service | Override **`createCustomValidation`** on `ResultQuantificationsService` — `base-service.ts:134,345` calls it on **both** upsert paths. **No OICR file is edited** |
...
| S-7b | Server — **override `createCustomValidation` on `ResultQuantificationsService`** with a per-role rule map whose **default** is today's behaviour (`R-MSD-011`). Deliberately **not** a `ValidationPipe` and **not** OICR's own service — `design.md` DD-13 |
...
#### Scenario: OICR keeps its floor because its call site says so
...
- [ ] AC.4 — The migration's `down()` restores `bigint NULL` and is executed to prove it. **A fractional value rounds, and a value wider than a signed `bigint` makes `down()` FAIL, not round** (MySQL `1264`/`1406`) — corrected at the round-2 re-judgment (`K-07`). A test asserting only rounding will read a range error as a test bug.
...
- **Reworked at the round-2 re-judgment.** Two claims this requirement rested on were falsified:
  ...
  - **`K-09`** — the `Maximum reached` guard fires at 18 units. Counting **digits** instead of characters makes it unreachable at every call site, because the scale-derived bound caps every configuration below 18 digits. Counting **characters** keeps it reachable but produces the false positive this requirement was written to remove.
- Resolution: the local rule is **prevention, not messaging** — the control refuses the fifth digit and the paste rounds it, and the server's `400` remains the authority for anything that reaches it. The digit guard is **removed rather than re-united**, because a guard that can no longer fire is dead code that future readers will trust.
...
- [ ] AC.3 — No value within the scale-derived bound raises `Maximum reached`, at any configured scale. *(Was "a signed two-decimal value of 16 digits" — a value the derived bound now rejects for magnitude, so the old AC described an unreachable subject: `K-09`.)*
- [ ] AC.5 — The removal of the digit guard is asserted: no configured call site can produce the warning, and the code that produced it is gone rather than merely unreachable.
...
- THEN no `Maximum reached` warning appears
- BUT it must NOT be fixed by re-uniting the guard's threshold — the guard is **removed**, because under the derived bound no configuration can reach 18 digits and a gate that cannot fire is dead code a future reader will trust (`K-09`)
- AND IT MUST instead be prevented at the control: a value beyond the derived bound is not enterable, asserted on the rendered value rather than on the absence of a message.
...
- Behavior: both OICR blocks accept only non-negative integers, enforced at (1) the call site's bound inputs and (2) **`ResultOicrService`, immediately before the upsert**. **Revised at the round-2 re-judgment (`K-01`):** a `ValidationPipe` was the round-1 answer and is now explicitly rejected — `UpdateOicrDto` has no decorators on ~16 properties, so `whitelist: true` would delete every undecorated one, nulling `sharepoint_link` and deactivating all tags, links and impact areas on the first save. `design.md` DD-13 carries the reasoning.

**Acceptance criteria**

- [ ] AC.1 — Both call sites pass an explicit scale of `0` and a floor of `0`; asserted at **each** call site, not once for the component.
- [ ] AC.2 — **`ResultQuantificationsService.createCustomValidation` rejects** a negative, fractional, or out-of-range `quantification_number` for roles 1 and 2, on **both** upsert paths, with `400`. *(Was "`UpdateOicrDto` rejects" — **corrected under the additive-defaults ruling**. An undecorated DTO cannot reject anything, and decorating it would re-introduce `K-01`'s data destruction. `update-oicr.dto.ts` is not edited.)*
- [ ] AC.3 — **`ResultOicrService` rejects a violating row before the upsert runs**, and the rejection surfaces as a `400` through `GlobalExceptions`. **No `ValidationPipe` is added to this controller** — `design.md` DD-13 records why: `UpdateOicrDto` carries no decorators on ~16 properties, and `whitelist: true` would delete every undecorated one, nulling `sharepoint_link` and deactivating all tags, links and impact areas on the first save (`K-01`).
...
- [ ] AC.3 — The bound cannot be set to a value inconsistent with the scale.
...
- **How verified:** `SELECT COUNT(*), MIN(quantification_number), MAX(quantification_number), MAX(LENGTH(TRIM(LEADING '-' FROM quantification_number))) FROM result_quantifications;` — output pasted verbatim
...
| **DC-10** | **Dead or false `Maximum reached`** | *(reframed at round 2, `K-09`)* the guard either fires on a legitimate value (character counting) or can never fire at all (digit counting under the derived bound) — both are defects | client unit test proving no configured scale/bound pairing can produce the warning, **and** that the producing code is removed | yes — reinstate the guard, the "no call site can produce it" assertion reddens |
