...
- **Acceptance / done check:**
  ...
  - [~] **HITL visual gate** ⚠️ **PARTIAL 2026-09-01** — the user confirmed on Dev: values persist, pre-existing data survived the migration, decimals and negatives store, and **AC.7's untouched-row resave succeeds**. Remaining: placeholder with the field cleared, the amber warning triggered with `-549755813886.9999`, the spinner stepping through `0`, and **dark theme**. **HITL visual gate** (`DC-11`, `NFR-MSD-004`): the field in both themes and in error state. **No automated substitute exists.**
