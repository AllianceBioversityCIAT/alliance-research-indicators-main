...
| --- | --- | --- |
| Static class attribute | `details:107,147` · `actor:3,41` · `org:3` · `stepper:4` | `text-[var(--ac-red-1)]` → `text-[var(--ac-warning-1)]` |
| Interpolated class string | `actor:34`, `actor:52` | `border-[var(--ac-red-1)]` → `border-[var(--ac-warning-1)]` **inside** `{{ cond ? '…' : '' }}` |

The interpolated pair is the higher-risk shape and is called out in `DD-4`.
...
| --- | --- | --- |
| Jest class assertions | the token **name** reached the markup (`D-1`, `D-2`) | that the rendered pixel is amber |
...
| **DD-5** | Dark-mode value is `#e69f00`, identical to light — no lightening | Today the amber is hardcoded and therefore does not flip; matching that keeps Innovation use identical to the 22 unmigrated files in dark mode. It also needs no lightening: measured **6.29:1** on the dark card, comfortably AA |
...
Carried from `requirements.md` §9 — `OQ-1` (token name), `OQ-2` (classification of `actor:41` / `details:114`, defaulted to amber and confirmed at the Phase 1 gate), `OQ-3` (dark value, settled by `DD-5`). None blocking.
