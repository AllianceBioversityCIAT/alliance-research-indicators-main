...
- `shared/interceptors/http-error.interceptor.ts` — when the response carries `X-Impersonation-Error: SESSION_INVALID` (**value-matched, not presence** — a `NESTED` code must not end a still-valid session; reconciled with R-IMP-010 at T-08 review): skip the generic 403 toast, call `impersonation.end('server-invalid')` then toast 'Simulation expired'. Other code values: suppress the generic toast only.
...
2. On `201`: `impersonation.start(res)` → stores `{session, actor: dataCache().user}`, sets `dataCache.user = res.user` (with `roleName` via the existing preferred-role rule) and persists `data`; the calling component then runs `websocket.configUser(target)`, `cache.bumpContentReload()` (remounts the current page’s `<router-outlet>` so loaders re-fetch under the target identity — **no** `location.reload()`, **no** forced `/home`), `actions.showToast(...)`.
...
**Client end (R-IMP-010)** — `impersonation.end(reason)`: best-effort `api.endImpersonation()` (3 s cap), restore `dataCache.user = actor`, rewrite `data`, clear `impersonation`, resolve `{ actor }`. Callers (banner, navbar, `httpErrorInterceptor`, `logOut`) then run `configUser(actor)`, `cache.bumpContentReload()`, toast. `logOut()` awaits it first when active.
...
| `SimulateProfileModalComponent` (`simulateProfile`) | content of the shared `app-modal` wrapper (title "Simulate another profile" rendered ONCE by the wrapper at wrapper tokens — D-imp-18; cancel only); hosts the two steps; `document:keydown.escape` closes (wrapper supplies the Tab trap only) | helper `.description` in the content |
...
| D-imp-14 | 2026-08-25 | Banner inside the navbar component; platform padding bound to `navbarHeight()`. **Amended 2026-08-27:** navbar host raised to `z-index: 1001` (above the section-header's `!z-10`) so the account dropdown paints over the page. **Amended 2026-08-28:** the sidebar and section-header `marginTop`, and the content `paddingTop` (= `navbarHeight() + headerHeight()`, since the section-header floats `fixed` above the content), all bind to the measured signals — the original `navbarHeight()`-only content offset hid the page filters under the header, with or without the banner. | Only measured path (J-04); post-launch regressions caught live |
| D-imp-18 | 2026-08-26 | Modal title rendered once by the `app-modal` wrapper (16/500 wrapper tokens); content carries only the helper `.description`. Escape handled by the modal component (`document:keydown.escape`) — the wrapper implements the Tab trap only | T-09 review: duplicated title vs mockup; wrapper has no Escape handling; changing the wrapper would affect all 11 modals. Residual 18/600-vs-wrapper-title visual delta goes to T-12 fidelity |
| D-imp-17 | 2026-08-25 | Endpoint paths are `/api/impersonation/*` — the impersonation endpoints mount unversioned; platform versioning is **opt-in per handler** (`main.ts` enables `VersioningType.URI` with no `defaultVersion`, and no impersonation controller declares `@Version`). This is not the whole app: `bilateral.controller.ts`/`agresso-contract.controller.ts` opt into `/api/v1/...` and one `results.controller.ts` route opts into `/api/v2/results`, so `v1`/`v2` segments are real elsewhere — just not for `/impersonation/*`. The blanket `/api/v1` wording in the baseline TRD §6.2 was drift for the general claim — flagged for T-13/archive | T-04 review; root guide §5: fix the doc, never let docs and code drift; narrowed at T-13 rework (Reviewer attempt 2) after the wider `@Version` sweep |
