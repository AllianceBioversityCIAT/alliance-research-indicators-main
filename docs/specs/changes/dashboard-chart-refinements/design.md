...
- **Role dimension PRESERVED (challenge #1 — a11y non-color-alone, archived R-DA-009 AC.3):** links arrive role-grouped from the server; rendered with `lineStyle.type` solid (PRIMARY) / dashed (CONTRIBUTING) / dotted (UNKNOWN) — Sankey honors lineStyle.type — and the component's existing DOM legend markup is KEPT verbatim. Legacy null roles render as UNKNOWN (archived "Legacy honesty" scenario stays satisfied).
...
| DD-4 | Role preserved as a LINK GROUPING KEY rendered with the same solid/dashed/dotted `lineStyle.type` + kept legend | Challenge #1: role is the non-color-alone a11y mechanism (archived R-DA-009 AC.3), not decoration; Sankey honors lineStyle.type, so the encoding survives 1:1. **Challenge outcome: §9.1** |
