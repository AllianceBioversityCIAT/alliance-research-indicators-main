...
- **Scope:** add `leverLabel(link)`; use it for node name (with ` (<id>)` suffix on label collision between distinct ids), node tooltip, link `leverFullName`, and `tableModel` lever cell. Keep `No lever`/`Other levers` buckets and every other option untouched.
...
- **Done check:** `npx jest src/app/pages/platform/pages/project-detail/components/sp-alignment-graph --coverage=false --silent` → 23 existing + new green (record the new count); `npm run build` green; `npx tsc -p tsconfig.spec.json --noEmit 2>&1 | grep -c 'error TS'` = 938 baseline; bare `npx eslint` on the component.
