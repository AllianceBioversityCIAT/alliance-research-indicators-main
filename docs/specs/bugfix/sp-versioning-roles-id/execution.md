...
| # | Lens | Finding |
| --- | --- | --- |
| **B-12** ✅ | conformance / risk (**both lenses**) | **The spec said 37 child deletes; the routine has 32 + 1 parent.** A Leader error from the T-02 Pivot. **FIXED this turn** in `requirements.md` AC.4 and `design.md` §3.1. **Addendum, 2026-08-18 validation pass:** `tasks.md:155` carried the identical "37" and was a **third** site — missed by this remediation because the forward sweep at the time searched for the *idea*'s two known homes, not every occurrence of the string. Found by the 2026-08-18 validation report (F-2) and corrected then |
...
## Constitution Impact: T-01b / T-03

**Added 2026-08-18 by the validation-remediation pass (W-2).** No block like this existed before this spec's Task Completion showed PASS 5/5 — T-01b built a new `src/db/baseline/` directory, five npm scripts, a Docker compose file, and a second Jest runner; T-03 committed a ~423 KB schema artifact. None of that is reflected in the constitutional docs. This is **`/akili-archive`'s Constitution & Graph Sync work, not unfinished T-03 scope** — it is recorded here so archive inherits a handoff list instead of none.

Each row below was re-verified against the actual file during this remediation pass, not copied from the validation report on trust:

| # | Document | Drift | Severity | Verification |
| --- | --- | --- | --- | --- |
| 1 | `docs/infrastructure.md:98` — *Data & migrations* / Seed-reset paragraph | States *"no seed or reset script exists in the repo … there is nothing an agent may safely reset."* This spec built exactly that — a disposable local container (`docker-compose.test.yml`), a loader (`scripts/load-baseline.js`), and a committed snapshot (`src/db/baseline/baseline.sql`). The statement is now **false** | **high** | Confirmed verbatim at `docs/infrastructure.md:98` |
| 2 | `docs/infrastructure.md:120` — **OI-7** | *"Define how a developer obtains a working local dataset…"* — **partly answered** by `src/db/baseline/`, not recorded | medium | Confirmed verbatim at `docs/infrastructure.md:120`, unchanged since T-01b landed |
| 3 | `docs/infrastructure.md:89-96` — *Data & migrations* command table | Lists none of `compose:test:up`/`:down`, `migration:test:execute`/`:revert`, `baseline:test:load`, `migration:test:bootstrap`, `test:fixtures`. Root `CLAUDE.md` §4.3 directs agents to read this contract and **never guess the commands** — so an agent following the guide cannot discover this harness | medium | Confirmed — the table at `:91-96` lists only the four `ARI_MYSQL_*`-targeted scripts; none of the five TEST-targeted scripts appear anywhere in the file |
| 4 | `server/researchindicators/src/CLAUDE.md` | No `db/baseline/` node in the source map, none of the five scripts in the commands section, and no mention of the fixture runner or its `*.fixture-spec.ts` convention — which advisory C-6 shows is a silent-zero-collection trap for the next fixture author | medium | Confirmed — a full-file grep for `db/baseline`, `baseline:test`, `migration:test`, `compose:test`, and `fixture-spec` returns zero hits |
| 5 | Root `CLAUDE.md` §3 repo tree | Lists `db/config/mysql/`, `db/config/dynamo/`, and `db/migrations/` but not `db/baseline/` | minor | Confirmed at `CLAUDE.md:89-92` |
| 6 | `docs/trd/trd.md` | `requirements.md` OQ-3, `design.md` §4.1, and `src/db/baseline/README.md` all point at an owed TRD note for the non-replayable migration history. None exists and no ticket id is recorded anywhere | **high** | **Confirmed, with one nuance:** `docs/trd/trd.md` has no note on migration replayability at all — verified by full-file grep. `requirements.md:169` (OQ-3) and `design.md:145` do explicitly say a TRD note is owed. `README.md:28` is slightly weaker than "all three" implies — it cites `RB-1d`/`OQ-3` as a repo-level finding out of scope for this bugfix, but does not itself use the words "TRD note owed". The substance holds regardless: no TRD note exists, and no ticket id is recorded anywhere |

**`.codegraph/` re-index is pending.** `src/db/baseline/` and the five npm scripts postdate the last index (`.codegraph/codegraph.db`, last modified 2026-08-17, before T-01b's baseline artifact and T-02b's second migration landed on this branch); the two new migration files are also unindexed. Recommend a re-index before the next spec that queries this area through `codegraph_explore`.
