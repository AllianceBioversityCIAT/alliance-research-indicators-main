...
- BUT it must NOT send `reportYear` on the live view — with no `?version` the request carries no `reportYear`
- AND the live request **does** additionally carry `reportingPlatforms=<platform derived from the route>`, which it does not carry today. *Corrected 2026-09-16 during T-04 — **pending owner ratification**. The original clause read "the request must be **byte-identical** to today's" and is **false**: `result.interceptor.ts:25-31` appends `reportingPlatforms` behind its own `if (platform)`, independent of the year, whenever `router.url` matches `result/<code>` — which the real route (`app.routes.ts:82` + `:112`) does. The mandated design (DD-6) cannot satisfy the original clause; no implementation choice avoids it short of abandoning the interceptor.* **Accepted as resolved-behaviour-neutral, measured not assumed:** `ResultsUtil.setup()` (`server/.../results.util.ts:31-38`) applies `platform_code = ReportingPlatformEnum[reportingPlatforms] ?? STAR` **unconditionally**, so a STAR result resolves identically with or without the param; the section is STAR-only (`bilateral.service.ts:156-160` gates every alignment fetch on `isPoolFundingCapable()`); there is no global `ValidationPipe` with `forbidNonWhitelisted`, so an extra query param cannot 400; and **`PATCH_PoolFundingTag` (`api.service.ts:811`) already ships this exact shape in production today** with no incident. For a non-STAR result it would be strictly more correct than today's silent STAR fallback
