...
- **Model:** **snapshot, not replay.** See _Why replay was abandoned_ below — this is a pivot from this artifact's first version, not the original design.
...
| `migrations` table rows                  | **348** | the one deliberate data exception — bookkeeping, not business data (see _The one data exception_) |
...
**164 migrations remained unexercised.** The migration history assumes a pre-existing environment in both schema _and_ data — it cannot be replayed from empty, full stop. This is now a repo-level finding (`requirements.md` RB-1d, OQ-3), out of scope for this bugfix to fix. Attempting to patch `CreateStaffGroups1759786024597` or any other unreplayable migration is explicitly **not** this task's job.
