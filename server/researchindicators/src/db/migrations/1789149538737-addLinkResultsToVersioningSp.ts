...
// @akili-spec bugfix/sp-versioning-link-results
/**
 * Adds a single `link_results` block to `SP_versioning`, immediately
 * after the `result_innovation_use` block, so a version snapshot carries
 * the source result's active, owned `link_results` rows (R-SPL-001).
 *
 * The rest of the procedure is untouched (R-SPL-002) — this file's `up()`
 * body is the currently-deployed body from
 * `1788878752646-UpdateVersionSp.ts` `up()`, copied verbatim, with
 * exactly one inserted block. `down()` restores that pre-change body
 * verbatim (NFR-SPL-001). No schema change (NFR-SPL-002).
 *
 * See `docs/specs/bugfix/sp-versioning-link-results/` (T-02) for the full
 * design (DD-1 through DD-4).
 */
export class AddLinkResultsToVersioningSp1789149538737
