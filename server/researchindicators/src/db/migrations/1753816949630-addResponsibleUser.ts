...
export class AddResponsibleUser1753816949630 implements MigrationInterface {
  name = 'AddResponsibleUser1753816949630';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`app_secrets\` ADD \`responsible_user_code\` varchar(10) NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`app_secrets\` DROP COLUMN \`responsible_user_code\``,
    );
  }
}
