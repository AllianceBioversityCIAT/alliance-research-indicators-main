...
/**
 * Adds usd_amount + amount_status to result_policy_change for Policy Type
 * "Program, Budget, or Investment" (policy_type_id = 3). Updates
 * policy_change_validation (required when type is 3) and SP_versioning
 * so snapshots copy the new columns.
 *
 * SP_versioning body is the currently-deployed text from
 * 1789586388552-addPoolFundingCopyToVersioningSp, with only the
 * result_policy_change INSERT/SELECT lists extended.
 */
export class AddPolicyChangeUsdAmountAndStatus1789700000000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ALTER TABLE \`result_policy_change\`
        ADD COLUMN \`usd_amount\` decimal(15,2) NULL,
        ADD COLUMN \`amount_status\` varchar(20) NULL
    `);

    await queryRunner.query(`DROP FUNCTION IF EXISTS \`policy_change_validation\``);
    await queryRunner.query(`
        ...
        begin
            ...
            SELECT
                (rpc.policy_type_id IS NOT NULL
                    ...
                    AND (
                        rpc.policy_type_id <> 3
                        OR (
                            rpc.usd_amount IS NOT NULL
                            AND rpc.usd_amount >= 0
                            AND rpc.amount_status IN ('Confirmed', 'Estimated', 'Unknown')
                        )
                    ))
                INTO
                validation
...
BEGIN
                                    ...
                                    DECLARE new_alignment_id BIGINT DEFAULT NULL;
                                    ...
                                    SELECT
                                    toc.created_at, toc.created_by, toc.updated_at, toc.updated_by, toc.is_active, toc.deleted_at,
                                    new_result_id AS result_id,
                                    ...
                                    WHERE toc.is_active = TRUE
                                        AND toc.result_id = temp_result_id;
                                    ...
                                    SELECT
                                    im.created_at, im.created_by, im.updated_at, im.updated_by, im.is_active, im.deleted_at,
                                    new_result_id AS result_id,
                                    im.lever_code, im.indicator_code, im.indicator_type,
                                    im.result_capacity_sharing_id, im.result_knowledge_product_id,
                                    im.result_policy_change_id, im.result_innovation_dev_id,
                                    im.other_contribution_narrative, im.is_stale
                                    FROM result_pool_funding_indicator_mapping im
                                    WHERE im.is_active = TRUE
                                        AND im.result_id = temp_result_id;
