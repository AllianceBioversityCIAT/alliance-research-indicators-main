...
/**
 * T-02 / R-PFV-001 / R-PFV-002 / DD-1 / DD-2 / DD-3 — re-declares
 * `SP_versioning` so it copies the four pool funding tables onto the
 * snapshot and empties the source (soft-deactivate).
 *
 * The body is copied verbatim from
 * `1789149538737-addLinkResultsToVersioningSp.ts` `up()`, the
 * currently-deployed text. The only edits are: one DECLARE, four INSERT
 * blocks (alignment, _sp, toc, mapping), and one deactivation block,
 * keyed on `temp_result_id` / the new `alignment_id`. `down()` restores
 * that pre-change body verbatim (NFR-PFV-001). No schema change
 * (NFR-PFV-002). New blocks sit after the SIGNAL 45001 guard
 * (NFR-PFV-003).
 *
 * Out of scope: any schema/DDL change; `delete_result`;
 * `SP_delete_result_version`; `full_delete_result_version` (T-03);
 * the other 30 blocks.
 *
 * See `docs/specs/bugfix/pool-funding-alignment-versioning/` (T-02).
 */
export class AddPoolFundingCopyToVersioningSp1789586388552
