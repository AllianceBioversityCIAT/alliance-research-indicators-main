...
/**
 * Repairs `SP_versioning`'s `result_impact_outcomes` and
 * `result_strategic_objectives` copy blocks.
 *
 * `1783029013035` introduced these two blocks naming a column,
 * `roles_id`, that `1783022620616` had already dropped from both tables
 * (it copied `role_id = roles_id` then dropped `roles_id`). The procedure
 * has been unable to execute since: MySQL 1054, Unknown column 'roles_id'
 * in 'field list' — masking two further defects in the same two blocks
 * (an 11-column / 10-SELECT-expression mismatch, and copying the source
 * row's AUTO_INCREMENT `id` into the same PK column of the new row).
 *
 * Fix (docs/specs/bugfix/sp-versioning-roles-id/design.md §2.2, DD-2):
 * drop `roles_id` and `id` from both the column list and the SELECT list
 * of the two blocks — 9 columns / 9 SELECT expressions, matching every
 * other copy block in the procedure. FROM/WHERE unchanged. The other 27
 * copy blocks, the guard clauses, and `full_delete_result_version`
 * (untouched here — see DD-4 / OQ-2) are byte-identical.
 *
 * `down()` restores `1783029013035`'s body verbatim, defects included
 * (DD-3) — a reversal must be faithful, not an improvement.
 */
export class RepairSpVersioningObjectiveBlocks1784300000000
