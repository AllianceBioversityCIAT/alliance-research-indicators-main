  ...
  async findAll() {
    ...
    const cleanGroup = (group: GroupItem) => {
      return {
        ...
        items: group.childGroups
          ?.filter(child => child.is_active)
          .map(cleanGroup) || [],
        indicators: group.indicatorPerItem
          ?.filter(ipi => ipi.projectIndicator && ipi.projectIndicator.is_active)
          .map(ipi => ({
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const parentPayload of dto.structures || []) {
        ...
        if (payloadParentId && existingParentsMap.has(payloadParentId)) {
          ...
          await this.syncGroupItemIndicators(parent.id, parentPayload.indicators || [], manager);
          ...
          if (parent) {
            const needsUpdate = parent.name !== parentPayload.name || 
                              parent.officialCode !== parentPayload.code ||
            ...
            parent = savedParent;
            await this.syncGroupItemIndicators(savedParent.id, parentPayload.indicators || [], manager);
          }
        } else {
        ...
        for (const childPayload of parentPayload.items || []) {
          if (childPayload.id != null) {
            ...
            if (child) {
              ...
              const needsUpdate = child.name !== childPayload.name ||
              ...
              await this.syncGroupItemIndicators(child.id, childPayload.indicators || [], manager);
            } else {
              ...
              await this.syncGroupItemIndicators(saved.id, childPayload.indicators || [], manager);
            ...
            if (existingChild) {
              ...
              await this.syncGroupItemIndicators(existingChild.id, childPayload.indicators || [], manager);
            } else {
  ...
  private async syncGroupItemIndicators(
    ...
    for (const ind of indicatorsPayload) {
      ...
      if (ind.id && !String(ind.id).startsWith('indicator_')) {
        ...
        await manager.update(ProjectIndicator, { id: indicatorId }, {
          name: ind.name,
    ...
    for (const existingId of currentIds) {
      if (!payloadIds.includes(existingId)) {
        await manager
          ...
          .andWhere('project_indicator_id = :indicatorId', { indicatorId: existingId })
          .execute();
      }
    }
  }
