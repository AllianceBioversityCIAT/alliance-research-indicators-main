...
export class SaveResultService {
    ...
    public async bulkSaveAllSections(results: ExternalMappersDto[], extraData?: ExtraData) {
    ...
    public async saveAllSections(
        ...
        try {
            ...
            const snapshotMessage =
                (!isNewCode && extraData?.appliedVersion ? 'is a snapshot' : 'is a live version') +
                ' from year ' +
            ...
            this.logger.error(`Error processing ${this.platformCode(extraData?.platformCode)} result: ${error.message}`);
    ...
    private platformCode(platformCode?: ReportingPlatformEnum) {
        if (!platformCode) throw new BadRequestException('Platform code is required');
