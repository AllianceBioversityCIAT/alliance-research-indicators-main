...
- **FP-51 — NO DDL against the shared scratch schema while `test:fixtures` runs (added 2026-08-27, `docs/specs/changes/measure-number-signed-decimal` T-08, measured).** `test/jest-fixtures.json` sets no `maxWorkers` and no `runInBand`, so fixture files run **in parallel against one MySQL container**. A fixture that issues DDL on a shared table — `ALTER`, `DROP`, `TRUNCATE` — bumps the table definition version and any sibling that has already opened that table inside a transaction dies with **`1412 ER_TABLE_DEF_CHANGED`**, *in the sibling*, not in the file that caused it. **Measured, not reasoned:** a draft T-08 fixture that `ALTER`ed `result_quantifications` to reach a `bigint` branch failed **4 of 5** `test:fixtures` runs, always in `innovation-use-level-boundary.fixture-spec.ts`; replacing the `ALTER` with a session-scoped `CREATE TEMPORARY TABLE` gave **8 consecutive clean runs** (5 by the implementer, 3 re-measured by the Leader).
  ...
  - ⚠️ ~~**Do not "fix" this by setting `maxWorkers: 1`** on `jest-fixtures.json`: that taxes all 17 suites to guard a hazard none of them currently trips, and the committed fixtures are clean.~~ **AMENDED AND REVERSED 2026-09-07** (`docs/specs/changes/innovation-use-required-fields` T-19, measured). **`maxWorkers: 1` IS now set.** The original prohibition rested on a premise that turned out to be incomplete: that **DDL is the only shared-schema hazard**. It is not. **Cross-file contention over shared *real catalog rows*** is a second mechanism this rule never anticipated — `innovation-use-validation.fixture-spec.ts` and `institution-type-subtype-catalog-equivalence.fixture-spec.ts` both write `clarisa_institution_types` codes 37/38/39, one deliberately never deleting them and the other deleting them with its failures swallowed by `.catch()`. **Measured, not reasoned:** **3 spurious FK failures in 9 runs**; after serialising, **6 consecutive green runs**. The failures *throw* rather than pass, so they produced false **reds**, never false greens — but a gate that fails one run in three gets worked around, and it was the only executed proof of a validation function about to reach a shared database.
  - **The DDL finding above stands and is unaffected** — it was independently measured and its `1412` mechanism is real. **Keep the fixtures DDL-free anyway:** serialising this runner does **not** protect against DDL from a *concurrent command*, e.g. `test:integration`'s committed `DROP TABLE … results` in `test/support/t13-schema.ts`. Serialisation closes the cross-file class **within** `test:fixtures`; it closes nothing outside it.
  - **The generalisable lesson, which is why this bullet is amended rather than deleted:** *a prohibition derived from one measured hazard inherits that hazard's blind spots.* FP-50 above was amended for exactly the same reason, one rule earlier.
