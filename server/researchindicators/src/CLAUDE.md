...
- **FP-50 — cite same-file targets by anchor, never by line number; a cross-file line citation is fine (added 2026-08-19, same spec).** This is a measurement, not a style preference: six of that spec's seven inaccurate citations were same-file line numbers invalidated by the very edit that introduced them (an eighth was averted only by switching to an anchor). A line number drifts the moment its own document is edited again; an anchor (a heading, a table's `#` column, a distinctive quoted phrase) survives.
