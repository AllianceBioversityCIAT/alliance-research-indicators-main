...
describe('GetContractInsightsService', () => {
  ...
  describe('invalidate(contractId?) — changes/dashboard-refresh T-01, R-DRF-001', () => {
    it('issues a fresh HTTP request after invalidate(id) clears the cache for that id', async () => {
      ...
      httpMock.expectOne(contractInsightsUrl('A1676')).flush({ data: mockReport, successfulRequest: true });
      await first;
      ...
      const req2 = httpMock.expectOne(contractInsightsUrl('A1676'));
      req2.flush({ data: mockReport, successfulRequest: true });
      await second;
    ...
    it('does NOT invalidate a different contract id currently loaded (no HTTP re-issue for it)', async () => {
      ...
      httpMock.expectOne(contractInsightsUrl('B-2')).flush({ data: mockReport, successfulRequest: true });
      await load;
      ...
      httpMock.expectNone(contractInsightsUrl('B-2'));
      expect(service.loadedContractId()).toBe('B-2');
    });
  });
});
