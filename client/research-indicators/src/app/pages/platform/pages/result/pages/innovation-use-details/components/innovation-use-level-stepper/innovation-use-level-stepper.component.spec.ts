...
describe('InnovationUseLevelStepperComponent', () => {
  ...
  describe('c3 — resolves selection by id, then compares level (the inverse trap)', () => {
    it('selectedLevelId = 7 highlights the button labelled 6 and shows the level-6 callout', () => {
      ...
      // quick/innovation-use-level-fill (2026-09-03) REVERSED R3 remediation F-1 by human decision:
      // the selected fill is --ac-light-blue-300 (#1689ca) again, white-on-fill 3.84:1, a
      // documented AA exception pinned in innovation-use-details.component.spec.ts. This assertion
      // tracks the winning class only — the contrast judgement lives with that pinning test.
