...
### 4.3 Shared
- **K-004 binds the ARGUMENT as tightly as the command (KZ-014).** If the red has not been *seen*, it may not be asserted — not in a code comment, not in a dispatch brief, not in a review verdict, not in a budget note. In one 2026-08-20 spec three Leader falsifier arguments reasoned from the design's own frame were wrong (one exactly backwards), and two reached committed test descriptions before review caught them.
...
- **Deployment & Access Governance:** Zero manual deployments by developers/agents. Remote releases are automated via CI/CD pipelines (pushes/merges to `dev` deploy to On-Premise Dev; pushes/merges to `main` deploy to AWS Production). **But the pipeline deploys code only — it does NOT apply database migrations (K-015, measured 2026-08-18).** A merged migration can sit unapplied indefinitely with nothing surfacing it: `8431dc4b` went 4 days and several deploys untouched, and a baseline/post measurement around one deploy showed zero delta. Applying one is a separate, human-decided step against a shared database — check pending state before assuming a merge shipped a schema or seed change. **`migration:show` is NOT an npm script** (verified 2026-08-20); it exists only as a typeorm passthrough: `npm run typeorm migration:show -- -d ./src/db/config/mysql/orm.config.ts`. **Correction (2026-08-27, Jenkins build #72 console, `profile-simulation` T-06):** the K-015 claim above no longer holds for the Dev pipeline — the on-prem deploy runs `npm run migration:execute` *inside the container, against the host env,* on every deploy (`"No migrations are pending"` observed in the console), so a migration merged to `dev` is now applied automatically as part of that deploy. The human-decision rule is unchanged for applying a migration **out of band** (ahead of merge, for local/disposable testing) and for **Prod** (`main`) — verify the Prod pipeline behaves the same way before assuming parity; it was not re-measured there.
- **CodeGraph:** the live index lives at the **repo root** `.codegraph/codegraph.db` (machine-local, gitignored) and it covers **both** packages — server *and* client. Pass the repo root as `projectPath`. *(Corrected 2026-08-18 by the `/akili-constitution` factual-claims sweep, and this line has now been wrong in **both** directions — it first read "`.codegraph/` is initialized", was over-corrected on 2026-08-13 to "lives at `server/researchindicators/.codegraph/` — **not** at the repo root", and that is what is false today. Evidence: the root holds a 174 MB `codegraph.db` re-indexed 2026-08-18; `server/researchindicators/.codegraph/` holds a `config.json` and **no database**; a probe reports **2,330 files indexed** and returns client symbols — `rolesGuard`, `jWtInterceptor`, `ApiService` (250 callers) — with their co-located specs. The companion claim "the client package has **no** index" was false for the same reason and is withdrawn.)* Prefer `codegraph_*` tools for symbol lookup, callers/callees and impact analysis before broad file scanning, and note the index reflects the last re-index, not the working tree. **Re-verify this line before trusting it** — it has drifted twice; a one-call probe settles it.
...
- **Guardrail hook (committed; binds every session in this checkout).** `.claude/hooks/akili-tasks-gate.sh` is a Claude Code `PreToolUse` gate on `Edit|Write`: it **blocks** any write that *increases* the `[x]` count in a `docs/specs/**/tasks.md` unless that spec's sibling `execution.md` exists **and** contains `PASS`. This is `/akili-execute` Step 3's evidence-before-checkbox rule moved from prose into the harness. A block is not a bug — record the Reviewer PASS in `execution.md` first, then flip the checkbox. **Scope and honesty:** the PASS check is a deliberate v1 heuristic (any `PASS` anywhere in the file, not per-task-ID), it needs `jq` on PATH, and it is **enforced only on Claude Code** — on OpenCode and Antigravity the same invariant stays instructional. *Proven able to fail per K-004 (2026-08-18): 6 probe cases, 3 of them observed BLOCKING with exit 2 — missing `execution.md`, present-but-no-`PASS`, and a whole-file `Write`.*

- **A filtered view of a command's output is not the output (K-014).** Never truncate (`tail`/`head`) or filter a **discovery** search without checking the total first — a silent cap makes *absent* and *excluded* indistinguishable, and one `| tail -6` over nine matches dropped the single file that falsified a whole spec's premise. Normalize before counting: the typeorm `migration:show` passthrough emits ANSI escapes, so `grep '^\[ \]'` matched nothing and read as "zero pending" while one migration was pending. **Check for an error in the raw output before you count it** — a count over a failed command is a confident zero. Same family as K-003.

- **A verification must declare what it CANNOT reach (KZ-017).** K-014 governs a command's *output*; this governs its **scope**. Five checks in one 2026-08-20 spec were narrower than the claim they backed and each returned a confident green: `grep "'Confirmed'"` cannot match `'Confirmed,Pending'`; `--include="*.ts"` cannot see `.tsx`; a `grep` over `src` cannot see `test/`; `npm test` (`rootDir: "src"`) never runs the **three** other jest configs (`test:e2e`, `test:integration`); and a mocked query builder cannot represent SQL operator precedence, so `A OR B AND C` passed as `(A OR B) AND C`. **Before trusting a check, name the region it structurally cannot inspect** — and when a property lives in generated output (SQL, rendered DOM, `dist/`), assert it there, never on the call sequence that produced it. Two of the five were the auditor's own.

- **Config behind a TTL cache is not in effect when it is saved (K-016, 2 occurrences).** `MappingPhaseResolver` and `ClarisaProjectsService` each cache for **5 minutes**, so an `app_config` save or an `ARI_CLARISA_HOST` switch is invisible until the TTL lapses or the process restarts — and re-saving *restarts* the window, which makes the emptiness look permanent. Both occurrences were reported as "returns nothing", and nothing was broken either time. Any spec that changes `app_config`-driven behavior MUST state the TTL/restart window in its verification steps, and no UI may imply the save took effect immediately.
...
| Antigravity | `agy` (**not** `antigravity`) | **Installed and confirmed working** at `~/.local/bin/agy` (2026-08-14) — this line previously said "not installed", which would have ruled the host out unexamined. Headless needs `--dangerously-skip-permissions` (even a file read is auto-denied without it), and `--model`/`--effort` are session flags. **`--effort` accepts only `low\|medium\|high`** — `xhigh` is silently not honored, so escalate by **tier** above `high` (2026-08-14). `agy models` — **re-probed 2026-09-09, and the previous list had drifted in THREE directions at once** (`docs/specs/innovation-use/link-innovation-dev` T-13). It now returns **14** slugs: `gemini-3.8-flash-{high,medium,low}` · `gemini-3.7-flash-{high,medium,low}` · `gemini-3.6-flash-{high,medium,low}` · `gemini-3.1-pro-{high,low}` · `claude-sonnet-4-6` · `claude-opus-4-6-thinking` · `gpt-oss-120b-medium`. So: **3.8-flash was missing** from this line (it is the newest flash tier); **3.5-flash no longer exists** and was still listed; and **`gemini-3.1-pro` has only `-high` and `-low` — there is NO `-medium`**, so the otherwise-uniform "effort is baked into the slug" pattern has a hole exactly where a T1/T3 task would reach for the middle. `gemini-3.1-pro-high` was the model that executed this spec's whole client lane. **Re-probe before planning a dispatch** — this list has now been wrong twice, in both directions (a model listed that was gone, and a model omitted that existed) — and **effort is baked into the slug** (`gemini-3.7-flash-high`), so `--model gemini-3.7-flash-high` is the whole selection. `worker-start --agent gemini` re-confirmed **still disabled** (`agent_unconfigured`) on 2026-08-20; the `terminal create` + `dispatch --inject` fallback works and preserves full provenance. **Quota is per-model, not per-account (2026-08-18):** `claude-sonnet-4-6` exhausted mid-run (`Resets in 2h54m`) while every gemini model kept answering. Plan the fallback by model, and expect `author ≠ auditor` to degrade to same-family separation when the Claude tier is the one that runs out. In Orca, `worker-start --agent gemini` is **disabled** on this install — launch via `terminal create --command "agy …"` then `orchestration dispatch --inject`, which preserves full Run/Task/Dispatch provenance |
