...
- **Routing:** global `/api` prefix; URI versioning enabled with **no `defaultVersion`**, so it's opt-in per handler, not blanket — most controllers mount unversioned at `/api/<resource>`; only `bilateral.controller.ts` / `agresso-contract.controller.ts` opt into `/api/v1/...` and one `results.controller.ts` route opts into `/api/v2/results` (`docs/trd/trd.md` §6.2). Never assume a `v1`/`v2` segment without checking the controller.
