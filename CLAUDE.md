...
- **`npm run test:e2e` is an infrastructure test, not a code gate (K-021, measured 2026-08-23).** Its only suite (`test/app.e2e-spec.ts`) imports the full `AppModule`: TypeORM against the **shared dev MySQL**, RabbitMQ, OpenSearch. A spec that copies that pattern hangs in connection retries (26 min observed on `f3-indicator-deep-dive` T-05) and touches a database that is not disposable. To exercise the real HTTP path (envelope, `ResponseInterceptor`, `GlobalExceptions`) use `npm run test:integration` with a `TestingModule` that mounts only the controller/service under test and replaces repositories via `overrideProvider` (template: `test/bilateral-primary-contributing-sp.integration-spec.ts`). **A spec that asks for an "e2e"/"integration" test MUST name the bootstrap scope** — which modules it mounts, which providers it overrides, and what it may not touch; a bare "supertest e2e" reads as the repo's only example. *(Applied on the spec branch by owner mandate, 2026-08-23; the digest apply on the default branch confirms the ID.)*
