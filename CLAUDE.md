...
- **A filtered view of a command's output is not the output (K-014).** Never truncate (`tail`/`head`) or filter a **discovery** search without checking the total first — a silent cap makes *absent* and *excluded* indistinguishable, and one `| tail -6` over nine matches dropped the single file that falsified a whole spec's premise. Normalize before counting: `migration:show` emits ANSI escapes, so `grep '^\[ \]'` matched nothing and read as "zero pending" while one migration was pending. **Check for an error in the raw output before you count it** — a count over a failed command is a confident zero. Same family as K-003.
